  ...
  if ($HAXCMS->validateJWT()) {
    ...
    if ($site = $HAXCMS->loadSite($HAXCMS->safePost['siteName'])) {
      ...
      if (isset($gitSettings)) {
        ...
        if (isset($gitSettings->url) && $gitSettings->url != '' && $gitSettings->url != false && $gitSettings->url != '/' . $site->manifest->metadata->siteName . '.git') {
        ...
        // implies the domain is actually on the system locally
          ...
          $domain = $HAXCMS->basePath . $HAXCMS->publishedDirectory . '/' . $site->manifest->metadata->siteName . '/';
        }
        ...
        // sanity check
        if (file_exists(HAXCMS_ROOT . '/system/boilerplate/cdns/' . $cdn . '.html')) {
          ...
          if ($cdn == 'custom') {
            ...
            $GLOBALS['fileSystem']->mirror(HAXCMS_ROOT . '/build', $siteDirectoryPath . '/build');
          }
          ...
          foreach ($templates as $path) {
            ...
            if ($path === 'index.html') {
              $boilerPath = HAXCMS_ROOT . '/system/boilerplate/cdns/' . $cdn . '.html';
            }
            else {
          ...
          // custom isn't a regex by design
            ...
            $templateVars['cdnRegex'] = "(https?:\/\/" .str_replace('.', '\.', $cdn) . "(\/[A-Za-z0-9\-\._~:\/\?#\[\]@!$&'\(\)\*\+,;\=]*)?)";
          }
        ...
        $index = str_replace('<base href="/' . $site->manifest->metadata->siteName . '/"', '<base href="/_published/' . $site->manifest->metadata->siteName . '/"', $index);
        // replace if it has a vanity domain
        $index = str_replace('<base href="/"', '<base href="/_published/' . $site->manifest->metadata->siteName . '/"', $index);
        // rewrite the file
        ...
        if (!is_link($siteDirectoryPath . '/node_modules')) {
          @symlink(HAXCMS_ROOT . '/node_modules', $siteDirectoryPath . '/node_modules');
        }
        @unlink($siteDirectoryPath . '/assets/babel-top.js');
        @unlink($siteDirectoryPath . '/assets/babel-bottom.js');
        @unlink($siteDirectoryPath . '/build');
