<?php
  ...
  if ($HAXCMS->validateJWT()) {
    ...
    if (isset($_POST['manifest']->title)) {
      ...
      $site->manifest->metadata->image = filter_var($_POST['manifest']->image, FILTER_SANITIZE_STRING);
      if (isset($_POST['manifest']->hexCode)) {
        $site->manifest->metadata->hexCode = filter_var($_POST['manifest']->hexCode, FILTER_SANITIZE_STRING);
      }
      $site->manifest->metadata->cssVariable = filter_var($_POST['manifest']->cssVariable, FILTER_SANITIZE_STRING);
      // update these parts of the manifest to match POST
      if (isset($_POST['manifest']->icon)) {
        $site->manifest->metadata->icon = filter_var($_POST['manifest']->icon, FILTER_SANITIZE_STRING);
      }
      if (isset($_POST['manifest']->domain)) {
        $domain = filter_var($_POST['manifest']->domain, FILTER_SANITIZE_STRING);
        // support updating the domain CNAME value
        if ($site->manifest->metadata->domain != $domain) {
          $site->manifest->metadata->domain = $domain;
          @file_put_contents($site->directory . '/' . $site->manifest->siteName . '/CNAME', $domain);
        }
      }
      // look for a match so we can set the correct data
      foreach ($HAXCMS->getThemes() as $key => $theme) {
        if (filter_var($_POST['manifest']->theme, FILTER_SANITIZE_STRING) == $key) {
    ...
    if (isset($_POST['manifest']->license)) {
      $site->manifest->license = filter_var($_POST['manifest']->license, FILTER_SANITIZE_STRING);
      $site->manifest->author = filter_var($_POST['manifest']->author, FILTER_SANITIZE_STRING);
    }
    ...
    if (isset($_POST['manifest']->git)) {
      if ((filter_var($_POST['manifest']->git->url, FILTER_SANITIZE_STRING)) &&
      (!isset($site->manifest->metadata->git->url) || $site->manifest->metadata->git->url != filter_var($_POST['manifest']->git->url, FILTER_SANITIZE_STRING))) {
        $site->gitSetRemote(filter_var($_POST['manifest']->git->url, FILTER_SANITIZE_STRING));
      }
    }
    header('Status: 200');
