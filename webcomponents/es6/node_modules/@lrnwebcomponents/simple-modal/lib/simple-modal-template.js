import{html,PolymerElement}from"../../../@polymer/polymer/polymer-element.js";import"../simple-modal.js";class SimpleModalTemplate extends PolymerElement{static get tag(){return"simple-modal-template"}static get properties(){return{modal:{type:"Object",computed:"_getModal()"},title:{type:"String",value:""}}}static get template(){return html`
    ...
    `}associateEvents(target,evt="tap",bubbles=!0,cancelable=!0){let root=this;target.addEventListener(evt,()=>{root.openModal(target,bubbles,cancelable)});return root.modal}_getModal(){return window.SimpleModal.requestAvailability()}openModal(target,bubbles=!0,cancelable=!0){const evt=new CustomEvent("simple-modal-show",{bubbles:bubbles,cancelable:cancelable,detail:{title:null!==this.title?this.title:!1,elements:{header:this._getSlot("header"),content:this._getSlot("content"),buttons:this._getSlot("buttons")},invokedBy:target}});["--simple-modal-width","--simple-modal-height","--simple-modal-titlebar-color","--simple-modal-titlebar-background","--simple-modal-header-color","--simple-modal-header-background","--simple-modal-content-container-color","--simple-modal-content-container-background","--simple-modal-buttons-color","--simple-modal-buttons-background","--simple-modal-button-color","--simple-modal-button-background"].forEach(prop=>{this.modal.style.setProperty(prop,getComputedStyle(this).getPropertyValue(prop))});window.dispatchEvent(evt)}_getSlot(slotName){let slot=this.querySelector("[slot=\""+slotName+"\"]");return null!==slot?slot.cloneNode(!0):!1}}window.customElements.define(SimpleModalTemplate.tag,SimpleModalTemplate);export{SimpleModalTemplate};