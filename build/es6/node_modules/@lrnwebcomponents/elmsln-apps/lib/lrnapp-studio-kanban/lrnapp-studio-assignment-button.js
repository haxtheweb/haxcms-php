...
*/class LrnappStudioAssignmentButton extends PolymerElement{static get template(){return html`
    ...
    </template>`}static get tag(){return"lrnapp-studio-assignment-button"}static get properties(){return{auto:{type:Boolean,reflectToAttribute:!0,value:!1,notify:!0},displayErrors:{type:Boolean,value:!0},projectId:{type:String},createAssignmentUrl:{type:String,value:null},iconClass:{type:String,value:null},alt:{type:String,value:null},classes:{type:String,value:null},hoverClass:{type:String,value:null},icon:{type:String,value:null},endPoint:String,csrfToken:String}}ready(){super.ready();// @todo need to load a assignment if one exists ahead of time
this.createAssignmentUrl=this.endPoint+"/api/assignments/create-stub?token="+this.csrfToken}_createAssignment(){this.shadowRoot.querySelector("#ajaxCreateStub").generateRequest()}_ajaxCreateStubHandler(e){var status=e.detail.response.status,response=e.detail.response;if(201===status){var assignment=e.detail.response.data;if(assignment){this.dispatchEvent(new CustomEvent("assignment-created",{bubbles:!0,cancelable:!0,composed:!0,detail:{assignment:assignment}}))}}// if we have errors to display
