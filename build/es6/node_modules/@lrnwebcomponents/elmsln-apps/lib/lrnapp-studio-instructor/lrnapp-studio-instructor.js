import{html,Polymer}from"../../../../@polymer/polymer/polymer-legacy.js";import{dom}from"../../../../@polymer/polymer/lib/legacy/polymer.dom.js";import"../../../../@polymer/app-route/app-location.js";import"../../../../@polymer/app-route/app-route.js";import"../../../../@polymer/iron-ajax/iron-ajax.js";import"../../../../@polymer/paper-toggle-button/paper-toggle-button.js";import"../../../../@polymer/paper-item/paper-item.js";import"../../../../@polymer/paper-badge/paper-badge.js";import"../../../../@polymer/paper-button/paper-button.js";import"../../../../@polymer/paper-input/paper-input.js";import"../../../../@polymer/paper-icon-button/paper-icon-button.js";import"../../../../@polymer/paper-tooltip/paper-tooltip.js";import"../../../../@vaadin/vaadin-grid/vaadin-grid.js";import"../../../../@vaadin/vaadin-grid/vaadin-grid-filter.js";import"../../../../@vaadin/vaadin-grid/vaadin-grid-sorter.js";import"../../../../@vaadin/vaadin-grid/vaadin-grid-column-group.js";import"../../../../@vaadin/vaadin-grid/vaadin-grid-selection-column.js";import"../../../elmsln-loading/elmsln-loading.js";import"../../../lrndesign-chart/lib/lrndesign-bar.js";import"../../../lrndesign-chart/lib/lrndesign-line.js";import"../../../lrndesign-chart/lib/lrndesign-pie.js";import"../../../dropdown-select/dropdown-select.js";import"../../../lrnsys-button/lrnsys-button.js";import"../../../lrndesign-avatar/lrndesign-avatar.js";import"../../../lrnsys-layout/lib/lrnsys-dialog.js";import"../lrnapp-studio-submission/lrnapp-studio-submission.js";Polymer({_template:html`
...
`,is:"lrnapp-studio-instructor",observers:["_routeChanged(route, endPoint)","_activeDataChanged(activeData.student, activeData.assignment)"],properties:{/**
     ...
     */projects:{type:Object,notify:!0/* transition */ /* transition */},/**
     ...
     */activeData:{type:Object,value:{student:{},assignment:{},submission:{}}},/**
   ...
   */_chartChanged:function(e){this.set("activeChart.title",e.detail.value);this.notifyPath("activeChart.title");this.set("activeChart.description","Chart of values relative to "+e.detail.value);this.notifyPath("activeChart.description");// calculate the valid charting options relative to selected chart style
this.set("activeChart.data",this._formatChartData(e.detail.value));this.notifyPath("activeChart.data")},/**
   ...
   */_formatChartData:function(type){var labels=[],series=[[]];const stats=this.stats.stats,assignments=this._toArray(this.assignments);let parts=type.split("-");if(typeof stats[parts[0]]!==typeof void 0&&typeof stats[parts[0]][parts[1]]!==typeof void 0){// make sure the data aligns now w/ the IDs in question
...
for(var i in assignments){let title=assignments[i].title;if(typeof stats[parts[0]][parts[1]][assignments[i].id]!==typeof void 0){series[0].push(stats[parts[0]][parts[1]][assignments[i].id]);title+=` (${stats[parts[0]][parts[1]][assignments[i].id]})`}else{series[0].push(0);title+=" (0)"}labels.push(title)}}return{labels:labels,series:series}},/**
   ...
   * and event
   */_dataTypeChanged:function(newValue,oldValue){// if type changes values and not from undefined to defined
...
this.studentParams.projectId=this.activeProject;this.$.studentrequest.generateRequest()},/**
   * attached life cycle
   */attached:function(){// listen for focus event to have fired
document.body.addEventListener("lrnsys-dialog-modal-closed",this._accessibleFocus.bind(this));this.$.selectedproject.addEventListener("change",this._projectChanged.bind(this));this.$.selectedchart.addEventListener("change",this._chartChanged.bind(this))},/**
   * detached life cycle
   */detached:function(){// listen for focus event to have fired
document.body.removeEventListener("lrnsys-dialog-modal-closed",this._accessibleFocus.bind(this));this.$.selectedproject.removeEventListener("change",this._projectChanged.bind(this));this.$.selectedchart.removeEventListener("change",this._chartChanged.bind(this))},/**
   * Set ourselves as having focus after the modal closes.
   */_accessibleFocus:function(e){document.body.classList.remove("scroll-disabled");// focus on our dialog triggering button
this.__rememberClick.focus()},/**
   * Handle response for the whole projects object.
   */_handleProjectResponse:function(event){this.$.loading.hidden=!0;this.set("projects",this._projectData.data.projects)},/**
   * Handle response for the whole projects object.
   */_handleStudentResponse:function(event){this.$.loading.hidden=!0;this.set("students",[]);this.set("students",this._studentData.data.students);this.set("assignments",[]);this.set("assignments",this._studentData.data.assignments);this.stats=this._studentData.data.stats;this.set("stats.header","Statistics for "+this.projects["project-"+this.activeProject].attributes.title);// make sure default is asc data
setTimeout(()=>{this.shadowRoot.querySelector("#sorter").direction="asc"},200)},/**
   * Pie menu operation handler based on which was clicked.
   */_changeActiveItem:function(e){document.body.classList.add("scroll-disabled");var normalizedEvent=dom(e),local=normalizedEvent.localTarget,newstudent,newassignment;// use button id in order to move around in the grid as far as active
switch(local.id){case"prevstudent":newstudent=this._getObjectByPosition(this.students,this.activeData.student.id,-1);if(-1!=newstudent&&typeof newstudent.assignments[this.activeData.assignment.id]!==typeof void 0){this.set("activeData.student",{});this.set("activeData.student",newstudent);this.set("activeData.submission",{});this.set("activeData.submission",newstudent.assignments[this.activeData.assignment.id]);this.set("route.path",this.endPoint+"/submissions/"+this.activeData.submission.id)}break;case"nextstudent":newstudent=this._getObjectByPosition(this.students,this.activeData.student.id,1);if(-1!=newstudent&&typeof newstudent.assignments[this.activeData.assignment.id]!==typeof void 0){this.set("activeData.student",{});this.set("activeData.student",newstudent);this.set("activeData.submission",{});this.set("activeData.submission",newstudent.assignments[this.activeData.assignment.id]);this.set("route.path",this.endPoint+"/submissions/"+this.activeData.submission.id)}break;case"prevassignment":newassignment=this._getObjectByPosition(this.assignments,this.activeData.assignment.id,-1);if(-1!=newassignment){this.set("activeData.assignment",{});this.set("activeData.assignment",newassignment);if(typeof this.activeData.student.assignments[newassignment.id].id!==typeof void 0){this.set("activeData.submission",this.activeData.student.assignments[newassignment.id]);this.set("activeData.submission",{});this.set("route.path",this.endPoint+"/submissions/"+this.activeData.student.assignments[newassignment.id].id)}else{this.set("activeData.submission",!1)}}break;case"nextassignment":newassignment=this._getObjectByPosition(this.assignments,this.activeData.assignment.id,1);if(-1!=newassignment){this.set("activeData.assignment",{});this.set("activeData.assignment",newassignment);if(typeof this.activeData.student.assignments[newassignment.id].id!==typeof void 0){this.set("activeData.submission",{});this.set("activeData.submission",this.activeData.student.assignments[newassignment.id]);this.set("route.path",this.endPoint+"/submissions/"+this.activeData.student.assignments[newassignment.id].id)}else{this.set("activeData.submission",!1)}}break;}},/**
   ...
   */_getObjectByPosition:function(items,key,i){var keys=Object.keys(items).sort(function(a,b){if(typeof items[a].sis!==typeof void 0){if(items[a].sis.sortable_name>items[b].sis.sortable_name){return 1}else if(items[a].sis.sortable_name<items[b].sis.sortable_name){return-1}return 0}else{return a-b}});if(key!==void 0){key=key.toString()}var index=keys.indexOf(key);// try fallback for type issues
if(-1==index){index=keys.indexOf(parseInt(key))}if(-1==i&&0<index||1==i&&index<keys.length-1){return items[keys[index+i]]}else{return-1}},/**
   ...
   */_submissionStatus:function(student,assignment,dataType){if(null!=student){if(!dataType&&typeof student.assignments[assignment.id]!==typeof void 0&&typeof student.assignments[assignment.id].id!==typeof void 0){return!0}else if(dataType&&typeof student.assignmentComments[assignment.id]!==typeof void 0&&0<this._toArray(student.assignmentComments[assignment.id]).length){return!0}}return!1},/**
   * Test student submission status relative to assignments
   */_submissionID:function(student,assignment){if(null!=student&&typeof student.assignments[assignment.id]!==typeof void 0&&typeof student.assignments[assignment.id].id!==typeof void 0){return student.assignments[assignment.id].id}return!1},/**
   * Return a piece of the submission needed for visualization bc of template scope
   */_submissionPiece:function(student,assignment,piece){if(null!=student&&typeof student.assignments[assignment.id]!==typeof void 0&&typeof student.assignments[assignment.id].id!==typeof void 0){var submission=student.assignments[assignment.id];switch(piece){case"url":return this.basePath+"lrnapp-studio-submission/submissions/"+submission.id;break;case"title":return submission.attributes.title;break;case"icon":return submission.meta.state_icon;break;case"color":return submission.meta.state_color;break;case"comments":return this._toArray(student.assignmentComments[assignment.id]);break;}}return""},/**
   * Return number of comments on an assignment for display.
   */_commentCount:function(student,assignment,dataType){if(!dataType&&null!=student&&typeof student.assignmentComments[assignment.id]!==typeof void 0){return this._toArray(student.assignmentComments[assignment.id]).length}return!1},/**
   * Set route for active submission to load
   */_setActiveSubmission:function(e){var normalizedEvent=dom(e),local=normalizedEvent.localTarget;this.__rememberClick=local;var item=local.id.split("-");// find the active elements
this.set("activeData.student",this.students[item[1]]);this.set("activeData.assignment",this.assignments[item[3]]);this.set("activeData.submission",this.students[item[1]].assignments[item[3]]);this.set("route.path",this.endPoint+"/submissions/"+item[item.length-1]);document.body.classList.add("scroll-disabled");this.$.dialog.toggleDialog()},/**
   ...
   */_setActiveComment:function(e){// disable all buttons for in modal nav
