...
tmp.academics.forEach(function(element){academics.push(element)});this.$.loading.hidden=!0;this.set("academics",academics);this.set("programs",programs)},/**
   ...
   */_makeService:function(e){var normalizedEvent=dom(e);let active=normalizedEvent.localTarget.getAttribute("data-machine-name");const network=this.activeCourse.topology.Network;let service=network.filter(service=>{if(service.machine_name!==active){return!1}return!0});// if we found one, make it the top level item
