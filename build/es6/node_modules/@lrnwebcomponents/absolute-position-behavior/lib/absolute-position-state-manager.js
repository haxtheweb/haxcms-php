...
import "../../../@polymer/iron-resizable-behavior/iron-resizable-behavior.js"; // register globally so we can make sure there is only one
...
class AbsolutePositionStateManager extends PolymerElement {
  ...
  checkMutations(mutations) {
    ...
    mutations.forEach(mutation => {
      if (update) return;
      update = update || !(mutation.type === "attributes" && mutation.attributeName === "style" && this.els.includes(mutation.target));
    });
  ...
  positionElement(el) {
    ...
    el.style.position = "absolute"; // TODO(noms): This should use IronFitBehavior if possible.
    ...
    if (el.fitToVisibleBounds) {
      ...
      if (elLeft - offset < parentRect.left || elRect.width > parentRect.width) {
        styleLeft = parentRect.left + "px"; /// if the right side is off-screen
      } else if (elRect.right > parentRect.right) {
        styleLeft = targetRect.right - elRect.width + "px";
      } //if the top is off screen


      if (elTop - offset < parentRect.top || elRect.height > parentRect.height) {
        styleTop = parentRect.top + "px"; // if the bottom is off screen
    ...
    el.style.top = styleTop; //provide positions for el and target (in case furthor positioning adjustments are needed)
