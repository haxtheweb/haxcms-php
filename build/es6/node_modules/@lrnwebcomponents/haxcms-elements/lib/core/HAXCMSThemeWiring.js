...
constructor(){super();this.HAXCMSThemeWiring=new HAXCMSThemeWiring(this)}/**
     ...
     */static get properties(){return{/**
         ...
         */color:{type:String,reflectToAttribute:!0/* transition */ /* transition */},/**
...
let name=color.replace("-text",""),tmp=new SimpleColors;if(tmp.colors[name]){return tmp.colors[name][6]}return"#000000"}/**
     ...
     */_editModeChanged(newValue){if(typeof newValue!==typeof void 0){microTask.run(()=>{// trick browser into thinking we just reized
...
if(null===this.contentContainer){this.contentContainer=this.shadowRoot.querySelector("#contentcontainer")}this.__disposer=[];afterNextRender(this,function(){// edge case, we just swapped theme faster then content loaded... lol
if(0===dom(this).getEffectiveChildNodes().length){let frag=document.createRange().createContextualFragment(store.activeItemContent);dom(this).appendChild(frag)}updateStyles();// store disposer so we can clean up later
 ...
 */class HAXCMSThemeWiring{constructor(element,load=!0){if(load){document.body.addEventListener("haxcms-edit-mode-changed",this._globalEditChanged.bind(element));document.body.addEventListener("haxcms-active-item-changed",this._activeItemUpdate.bind(element));document.body.addEventListener("haxcms-trigger-update",this._triggerUpdate.bind(element));// @todo may want to set this to sessionStorage instead...
