 ...
 */class HAXCMSEditorBuilder extends PolymerElement{/**
   ...
   * ready life cycle
   */constructor(){super();import("./haxcms-site-editor-ui.js");if(!store.cmsSiteEditor.haxCmsSiteEditorUIElement){store.cmsSiteEditor.haxCmsSiteEditorUIElement=document.createElement("haxcms-site-editor-ui");document.body.appendChild(store.cmsSiteEditor.haxCmsSiteEditorUIElement);// forces a nice fade in transition
setTimeout(()=>{store.cmsSiteEditor.haxCmsSiteEditorUIElement.painting=!1},5)}this.applyContext()}/**
...
if(typeof DatArchive!==typeof void 0){context="beaker"}else if(!0/* transition */ /* transition */===window.__haxCMSContextPublished){context="published"}else if(!0===window.__haxCMSContextNode){// @todo add support for node js based back end
context="nodejs"}else if(!0===window.__haxCMSContextDemo){context="demo"}else{context="php"}return context}applyContext(){let context=this.getContext();if("php"===context){// append the php for global scope to show up via window
// this is a unique case since it's server side generated in HAXCMS/PHP
let script=document.createElement("script");script.src=`/haxcms-jwt.php`;document.documentElement.appendChild(script)}// dynamic import if this isn't published
