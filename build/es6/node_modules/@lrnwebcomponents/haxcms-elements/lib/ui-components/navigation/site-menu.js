import{html,PolymerElement}from"../../../../../@polymer/polymer/polymer-element.js";import{store}from"../../core/haxcms-site-store.js";import{autorun,toJS}from"../../../../../mobx/lib/mobx.module.js";import"../../../../map-menu/map-menu.js";class SiteMenu extends PolymerElement{static get tag(){return"site-menu"}static get template(){return html`
      ...
      ></map-menu>
    `}static get properties(){return{routerManifest:{type:Object},activeId:{type:String},hideActiveIndicator:{type:Boolean,value:!1},preventAutoScroll:{type:Boolean,value:!1},trackIcon:{type:String,value:""}}}connectedCallback(){super.connectedCallback();this.__disposer=autorun(()=>{this.routerManifest=Object.assign({},toJS(store.routerManifest))});setTimeout(()=>{this.__disposer2=autorun(()=>{this.activeId=toJS(store.activeId)})},50);this.$.menu.addEventListener("active-item",this.mapMenuActiveChanged.bind(this))}disconnectedCallback(){super.disconnectedCallback();this.__disposer();if(this.__disposer2){this.__disposer2()}this.$.menu.removeEventListener("active-item",this.mapMenuActiveChanged.bind(this))}mapMenuActiveChanged(e){e.detail.trackIcon=this.trackIcon;let userData=JSON.parse(window.localStorage.getItem("HAXCMSSystemData"));if(userData.manifests&&typeof userData.manifests[this.routerManifest.id]===typeof void 0){userData.manifests[this.routerManifest.id]={accessData:{}}}if(!userData.manifests[this.routerManifest.id].accessData){userData.manifests[this.routerManifest.id].accessData={}}userData.manifests[this.routerManifest.id].accessData[e.detail.id]={timestamp:Math.floor(Date.now()/1e3),trackIcon:this.trackIcon};for(var i in this.routerManifest.items){if(this.routerManifest.items[i].id===e.detail.id){this.routerManifest.items[i].metadata.accessData=userData.manifests[this.routerManifest.id].accessData[e.detail.id]}}window.localStorage.setItem("HAXCMSSystemData",JSON.stringify(userData))}}window.customElements.define(SiteMenu.tag,SiteMenu);export{SiteMenu};