...
import{LitElement as t,html as e,css as i}from"../../lit/index.js";import{normalizeEventPath as n}from"../utils/utils.js";class PageContentsMenu extends t{static get styles(){return[i`
    ...
    `}keyToggle(t){["Enter","Space"].includes(t.key)&&this.toggleSettings(t)}keyScroll(t){"Enter"==t.key&&this.scrollToObject(t)}toggleSettings(t){this.mobile&&(this.hideSettings=!this.hideSettings)}scrollToObject(t){var e=n(t)[0];if(this.items&&this.contentContainer&&e.getAttribute("data-index")&&this.items[parseInt(e.getAttribute("data-index"))]){let i;t.preventDefault(),t.stopPropagation(),t.stopImmediatePropagation(),i=this.contentContainer.querySelector("#"+this.items[parseInt(e.getAttribute("data-index"))].id);void 0!==window.safari?i.scrollIntoView():i.scrollIntoView({behavior:"smooth",block:"start",inline:"nearest"}),window.history.pushState({},null,e.getAttribute("href")),window.dispatchEvent(new PopStateEvent("popstate")),this.hideSettings=!0}}renderItem(t,i){return null==t.link&&t.id?e`
        <li class="item">
          <a
