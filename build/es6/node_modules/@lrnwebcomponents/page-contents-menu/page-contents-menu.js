...
import{LitElement as e,html as t,css as i}from"../../lit/index.js";import{normalizeEventPath as n}from"../utils/utils.js";class PageContentsMenu extends e{static get styles(){return[i`
    ...
    `}keyToggle(e){["Enter","Space"].includes(e.key)&&this.toggleSettings(e)}keyScroll(e){"Enter"==e.key&&this.scrollToObject(e)}toggleSettings(e){this.mobile&&(this.hideSettings=!this.hideSettings)}scrollToObject(e){var t=n(e)[0];if(this.items&&this.contentContainer&&t.getAttribute("data-index")&&this.items[parseInt(t.getAttribute("data-index"))]){let i;e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),i=this.items[parseInt(t.getAttribute("data-index"))].item?this.items[parseInt(t.getAttribute("data-index"))].item:this.contentContainer.querySelector("#"+this.items[parseInt(t.getAttribute("data-index"))].id);void 0!==window.safari?i.scrollIntoView():i.scrollIntoView({behavior:"smooth",block:"start",inline:"nearest"}),window.history.pushState({},null,t.getAttribute("href")),this.hideSettings=!0}}renderItem(e,i){return null==e.link&&e.id?t`
        <li class="item">
          <a
