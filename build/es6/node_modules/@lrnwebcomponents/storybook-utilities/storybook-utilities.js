import { storiesOf } from "../../@storybook/polymer/dist/client/index.js";
import * as storybookBridge from "../../@storybook/addon-knobs/polymer.js";
...
export class StorybookUtilities {
  ...
  getBindings(props) {
    ...
    keys.forEach(key => {
      ...
      let editable = key.startsWith("__") === false && (props[key].computed === undefined || props[key].computed === "undefined") && props[key].readOnly !== true;

      if (editable) {
        let keyType = props[key].type.name || props[key].type; // convert typed props
  ...
  addLiveDemo(story, escape = false) {
    ...
    story.demo.add(story.alias, () => {
      story.attrBindings = ``;
      Object.values(this.getBindings(story.props)).forEach(prop => {
        if (prop.value !== false && prop.value !== "") {
          story.attrBindings += typeof prop.value !== "string" ? ` ${prop.id}="${prop.value}"` : ` ${prop.id}="${prop.value.replace(/'/g, "&apos;").replace(/"/g, "&quot;")}"`;
        }
      });
      story.slotBindings = ``;
      Object.values(this.getBindings(story.slots)).forEach(slot => {
        story.slotBindings += slot.id !== "slot" ? `<div slot="${slot.id}">${slot.value}</div>` : `${slot.value}`;
      });
      return `
        <h1>${story.alias}</h1>
        ${story.before}
