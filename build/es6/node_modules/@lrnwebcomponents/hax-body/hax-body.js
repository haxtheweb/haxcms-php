 ...
 */let HaxBody=Polymer({is:"hax-body",_template:html`
    ...
    <hax-input-mixer
      ...
      class="hax-context-menu ignore-activation"
    ></hax-input-mixer>
  `,listeners:{focusin:"_focusIn",mousedown:"_focusIn","hax-context-item-selected":"_haxContextOperation","hax-input-mixer-update":"_haxInputMixerOperation","place-holder-replace":"replacePlaceholder"},properties:{__self:{type:Object,value:this},/**
...
info.removedNodes.map(node=>{if(this._haxElementTest(node)&&!node.classList.contains("hax-active")){this.fire("hax-body-tag-removed",{node:node})}})}})})},/**
   ...
   */attached:function(){afterNextRender(this,function(){// try to normalize paragraph insert on enter
...
this.hideContextMenus();var nodeClone=dom(node).cloneNode(!0);if("webview"===nodeClone.tagName.toLowerCase()&&window.HaxStore.instance._isSandboxed&&typeof nodeClone.guestinstance!==typeof void 0){delete nodeClone.guestinstance}// shouldn't be possible but might as well check
...
this._hideContextMenu(this.$.haxinputmixer)},/**
   ...
   */_focusIn:function(e){var normalizedEvent=dom(e);if(this.__focusLogic(normalizedEvent.localTarget)){e.stopPropagation()}},/**
...
if(!1===newValue){this.removeAttribute("contenteditable");this.hideContextMenus()}},/**
   ...
   */_haxResolvePreviousElement:function(node){node=dom(node).previousElementSibling;while(null!=node&&typeof node.tagName!==typeof void 0&&"HAX-"===node.tagName.substring(0,4)){node=dom(node).previousElementSibling}return node},/**
   * Test if this is a HAX element or not
   ...
   */_applyContentEditable:function(status,target=this.$.body){let children=dom(target).getDistributedNodes();// fallback for content nodes if not polymer managed nodes above
...
while(null!=node&&!this._haxElementTest(node)){node=dom(node).previousSibling}}if(null!=node){setTimeout(()=>{node.focus()},50)}}}catch(e){console.log(e)}}});export{HaxBody};