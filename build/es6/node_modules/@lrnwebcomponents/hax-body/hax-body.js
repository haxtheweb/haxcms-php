...
import { encapScript, wipeSlot } from "./lib/haxutils.js";
...
class HaxBody extends PolymerElement {
        ...
        if (info.addedNodes.length > 0) {
          info.addedNodes.map(node => {
            if (this._haxElementTest(node)) {
              ...
              let i = window.HaxStore.instance.gizmoList.findIndex(j => j.tag === node.tagName.toLowerCase());
  ...
  hoverEvent(e) {
    if (this.editMode) {
      ...
      if (!this.$.platecontextmenu.classList.contains("hax-active-hover")) {
        ...
        let local = normalizedEvent.localTarget; // see if the target is relevent when showing the edit menu operations
  ...
  elementInViewport(el) {
    let top = el.offsetTop - 32 - window.HaxStore.instance.haxPanel.$.drawer.offsetHeight;
  ...
  replacePlaceholder(e) {
    ...
    if (e.detail === "text") {
      ...
      p.innerHTML = "<br/>";
      this.haxReplaceNode(this.activeNode, p, dom(this.activeNode).parentNode); // allow swap out to happen
  ...
  haxInsert(tag, content, properties = {}, waitForLock = true) {
    ...
    if (window.HaxStore.instance.activePlaceHolder !== null && typeof window.HaxStore.instance.activePlaceHolder.style !== typeof undefined) {
      ...
      this.haxReplaceNode(window.HaxStore.instance.activePlaceHolder, newNode, dom(window.HaxStore.instance.activePlaceHolder).parentNode);
      window.HaxStore.instance.activePlaceHolder = null;
        ...
        if (newNode.tagName !== "GRID-PLATE" && this.activeContainerNode.tagName === "GRID-PLATE" && this.activeContainerNode !== this.activeNode) {
          ...
          dom(this).insertBefore(newNode, this.activeContainerNode.nextElementSibling);
        }
      } else {
  ...
  haxChangeTagName(node, tagName, newNode) {
    ...
    try {
      ...
      setTimeout(() => {
        let children = dom(replacement).getEffectiveChildNodes(); // see if there's a child element and focus that instead if there is
  ...
  haxDeleteNode(node, parent = this) {
    ...
    if (this.activeContainerNode != null && this.activeContainerNode.previousElementSibling !== null) {
      ...
      if (this.activeContainerNode != null && window.HaxStore.instance.isTextElement(this.activeContainerNode) && dom(this.activeContainerNode).textContent !== "") {
        try {
  ...
  _haxContextOperation(e) {
    ...
    switch (detail.eventName) {
      ...
      case "hax-edit-property":
        ...
        let style = this.$.cecontextmenu.currentStyle || window.getComputedStyle(this.$.cecontextmenu); // force input mixes to match width of the ce context menu currently
  ...
  _haxElementTest(node) {
    if (typeof node.tagName !== typeof undefined && node.tagName.substring(0, 4) !== "HAX-") {
      return true;
    }
  ...
  _applyContentEditable(status, target = this.$.body) {
    let children = dom(target).getDistributedNodes(); // fallback for content nodes if not polymer managed nodes above
    ...
    for (var i = 0, len = children.length; i < len; i++) {
      ...
      if (this._haxElementTest(children[i])) {
        if (status) {
          ...
          let l = window.HaxStore.instance.gizmoList.findIndex(j => j.tag === children[i].tagName.toLowerCase());
  ...
  _activeContainerNodeChanged(newValue, oldValue) {
    if (this.editMode && typeof newValue !== typeof undefined && newValue != null && newValue.tagName != null) {
      ...
      let tag = newValue.tagName.toLowerCase(); // special case for the grid plate since it brings in dom nodes
  ...
  _activeNodeChanged(newValue, oldValue) {
    ...
    if (this.editMode && typeof newValue !== typeof undefined && newValue !== null) {
      ...
      setTimeout(() => {
        this.positionContextMenus(newValue, window.HaxStore.instance.activeContainerNode);
      }, 100);
