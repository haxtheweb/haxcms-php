...
*/Polymer({_template:html`
   ...
   */attached:function(){// fire an event that this is the manager
...
document.body.addEventListener("place-holder-file-drop",this._placeHolderFileDrop.bind(this));this.$.dialog.addEventListener("iron-overlay-canceled",this.close.bind(this));this.$.dialog.addEventListener("iron-overlay-closed",this.close.bind(this));this.$.closedialog.addEventListener("tap",this.close.bind(this));this.$.newassetconfigure.addEventListener("tap",this.newAssetConfigure.bind(this));this.$.fileupload.addEventListener("upload-before",this._fileAboutToUpload.bind(this));this.$.fileupload.addEventListener("upload-response",this._fileUploadResponse.bind(this))},/**
   ...
   * Toggle panel size
   */togglePanelSize:function(e){this.$.dialog.classList.toggle("grow");this.updateStyles();window.dispatchEvent(new Event("resize"))},/**
   * compute title for edit mode
   */_computeEditTitle:function(updateExisting){if(typeof this.activeHaxElement!==typeof void 0&&updateExisting){return"Update"}else{return"Insert"}},/**
   * A file event was detected from a drag and drop in the interface, most likely
   * from a place-holder tag
   */_placeHolderFileDrop:function(e){// reset the manager back to the first page
...
this.$.fileupload._onDrop(e.detail)},/**
   ...
   */_fileAboutToUpload:function(e){if(!this.__allowUpload){// cancel the event so we can jump in
...
if(1===targets.length){this._haxAppPickerSelection({detail:targets[0]})}else if(0!==targets.length){window.HaxStore.instance.haxAppPicker.presentOptions(targets,type,"Where would you like to upload this "+type+"?","app")}else{window.HaxStore.toast("Sorry, you don't have a storage location that can handle "+type+" uploads!",5e3)}}else{this.__allowUpload=!1}},/**
   ...
   */_haxAppPickerSelection:function(e){// details for where to upload the file
let connection=e.detail.connection;this.__appUsed=e.detail;this.$.fileupload.method=connection.operations.add.method;let requestEndPoint=connection.protocol+"://"+connection.url;// ensure we build a url correctly
...
this.__allowUpload=!0;this.$.fileupload.uploadFiles()},/**
   ...
   */_fileUploadResponse:function(e){// convert response to object
...
// gizmo type needed
if(typeof map.gizmo.type!==typeof void 0){item.type=this._resolveObjectPath(map.gizmo.type,data)}this.$.url.value=item.url;// @todo put in logic to support the response actually
...
this.newAssetConfigure()},/**
   * Notice page changed.
   */_activePageChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0){this.searching=!1;this.updateStyles();if(1===newValue){this.$.appbrowser.resetBrowser()}else if(2===newValue){this.$.gizmobrowser.resetBrowser()}}},/**
   * Store updated, sync.
   ...
   */_activeHaxElementChanged:function(newValue,oldValue){if(typeof oldValue!==typeof void 0){this.$.preview.advancedForm=!1;if(typeof newValue.tag===typeof void 0){this.resetManager()}else{// reset files so it doesn't bloat up
this.$.fileupload.set("files",[]);this.$.dialog.scrollTop=0;this.selectStep("configure")}}},/**
   * Bubble up insert event.
   */insertHaxElement:function(e){// bubble up the inject event / element to the body
let previewNode=this.$.preview.previewNode;previewNode.removeAttribute("hax-preview-mode");let element=window.HaxStore.nodeToHaxElement(previewNode);element.replace=this.editExistingNode;if(typeof this.activeHaxElement.__type!==typeof void 0){element.__type=this.activeHaxElement.__type}element.replacement=previewNode;this.fire("hax-insert-content",element);let toast="New element added!";if(this.editExistingNode){toast="Element updated!"}window.HaxStore.toast(toast,2e3);// close window
this.close()},/**
   * Reset things on the display to their defaults.
   */resetManager:function(activePage=0){this.selectStep("select");this.activePage=activePage;document.body.style.overflow=null;this.appList=window.HaxStore.instance.appList;this.searching=!1;window.HaxStore.write("activeApp",null,this);window.dispatchEvent(new Event("resize"));this.editExistingNode=!1;this.$.url.value="";this.$.fileupload.headers="";this.$.fileupload.method="";this.$.fileupload.target="";this.__allowUpload=!1},/**
   * Cancel interaction with the modal
   */cancel:function(e){// reset and close dialog
this.close()},/**
   * Open state change.
   */_openedChanged:function(newValue,oldValue){if(oldValue&&!newValue){document.body.style.overflow=null}else if(newValue&&!oldValue){document.body.style.overflow="hidden"}},/**
   * Respond to the modal closing
   */close:function(e){var normalizedEvent=dom(e),local=normalizedEvent.localTarget;if(typeof e===typeof void 0||local===this.$.dialog||local===this.$.closedialog){// reset the active element which will force this to reset the manager
window.HaxStore.write("activeHaxElement",{},this);this.opened=!1;this.resetManager()}},/**
   * Open the dialog
   */open:function(e){this.opened=!0},/**
   * Configure asset after upload or URL passed in.
   */newAssetConfigure:function(){let values={source:this.$.url.value};// we have no clue what this is.. let's try and guess..
...
window.HaxStore.instance.haxAppPicker.presentOptions(haxElements,type,"Pick how to present the "+type,"gizmo")}}else{window.HaxStore.toast("Sorry, HAX doesn't know how to handle that type of link yet.")}},/**
   * Toggle ourselves.
   */toggleDialog:function(toggle=!0){if(this.opened&&toggle){this.close()}else{window.HaxStore.instance.closeAllDrawers(this)}},/**
   * Set step to configure or insert
   */selectStep:function(step){if("configure"==step){this.activeStep=!0}else{this.activeStep=!1}},/**
   * Notice step changes
   */_activeStepChanged:function(newValue,oldValue){if(newValue||!newValue){this.updateStyles();window.dispatchEvent(new Event("resize"))}},/**
   * Helper to take a multi-dimensional object and convert
   * it's reference into the real value. This allows for variable input defined
   * in a string to actually hit the deeper part of an object structure.
   */_resolveObjectPath:function(path,obj){return path.split(".").reduce(function(prev,curr){return prev?prev[curr]:null},obj||self)}});