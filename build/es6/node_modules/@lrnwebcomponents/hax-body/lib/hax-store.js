 ...
 */window.HaxStore=window.HaxStore||{};window.HaxStore.instance=null;window.HaxStore.requestAvailability=function(){if(!window.HaxStore.instance){window.HaxStore.instance=document.createElement("hax-store");document.body.appendChild(window.HaxStore.instance)}return window.HaxStore.instance};Polymer({_template:html`
    <style>
      :host {
        display: none;
      }
...
this._handleDynamicImports(items,haxAutoloader)}},/**
   ...
   * it came from.
   */_handleDynamicImports:async function(items,haxAutoloader){const basePath=pathFromUrl(decodeURIComponent(import.meta.url));for(var i in items){await import(`${basePath}../../../${items[i]}`).then(response=>{for(var cVal in response){// get the custom element definition we used to add that file
...
dom(haxAutoloader).appendChild(document.createElement(i))}}}).catch(error=>{/* Error handling */console.log(error)})}},_editModeChanged:function(newValue){if(newValue&&this.globalPreferences.haxVoiceCommands){this.$.hal.auto=!0}else{this.$.hal.auto=!1}},_globalPreferencesChanged:function(newValue,oldValue){// regardless of what it is, reflect it globally but only after setup
...
window.localStorage.setItem("haxUserData",JSON.stringify(this.storageData))},/**
   * ready life cycle
   */ready:function(){afterNextRender(this,function(){// see if a global was used to prevent this check
...
try{let globalData=window.sessionStorage.getItem("haxUserData")?JSON.parse(window.sessionStorage.getItem("haxUserData")):{};this.set("storageData",globalData)}catch(e){}}else{try{let globalData=window.localStorage.getItem("haxUserData")?JSON.parse(window.localStorage.getItem("haxUserData")):{};this.set("storageData",globalData)}catch(e){}}}})},/**
   * attached.
   */attached:function(){afterNextRender(this,function(){// register built in primitive definitions
...
let sel=window.HaxStore.getSelection();var range;if(range=window.HaxStore.getRange()){range.deleteContents();range.insertNode(document.createTextNode(text))}}},/**
   ...
   */created:function(){setPassiveTouchGestures(!0);// claim the instance spot. This way we can easily
