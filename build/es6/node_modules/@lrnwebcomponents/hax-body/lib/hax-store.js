...
import { encapScript, wipeSlot, stripMSWord } from "./haxutils.js";
...
import { CodeSample } from "../../code-sample/code-sample.js";
...
class HaxStore extends HAXElement(MediaBehaviorsVideo(PolymerElement)) {
  ...
  async _handleDynamicImports(items, haxAutoloader) {
    ...
    for (var i in items) {
      await import(`${basePath}../../../${items[i]}`).then(response => {
  ...
  connectedCallback() {
    ...
    afterNextRender(this, function () {
      ...
      import("../../hal-9000/hal-9000.js"); // set this global flag so we know it's safe to start trusting data
  ...
  _onPaste(e) {
    ...
    if (window.HaxStore.instance.isTextElement(window.HaxStore.instance.activeNode) && !window.HaxStore.instance.haxManager.opened) {
      ...
      pasteContent = pasteContent.replace(/<span><\/span>/g, "");
      pasteContent = stripMSWord(pasteContent); // edges that some things preserve empty white space needlessly
  ...
  constructor() {
    ...
    import("./hax-blox-browser.js"); // claim the instance spot. This way we can easily
...
window.HaxStore.getHAXSlot = node => {
  ...
  let content = "";
  var slotnodes = dom(node).children; // ensure there's something inside of this
