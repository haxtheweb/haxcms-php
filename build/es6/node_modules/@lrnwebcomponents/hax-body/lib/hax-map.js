import{html as t,css as i,LitElement as a}from"../../../lit/index.js";import"../../simple-icon/lib/simple-icon-lite.js";import"./hax-toolbar-item.js";import"../../hax-iconset/lib/simple-hax-iconset.js";import"../../simple-icon/lib/simple-icon-button.js";import{HAXStore as l}from"./hax-store.js";import{normalizeEventPath as n,nodeToHaxElement as o}from"../../utils/utils.js";import{HaxTrayDetailHeadings as s}from"./hax-ui-styles.js";import{I18NMixin as r}from"../../i18n-manager/lib/I18NMixin.js";import{autorun as h,toJS as c}from"../../../mobx/dist/mobx.esm.js";class HaxMap extends(r(a)){static get styles(){return[...s,i`
      ...
      `]}constructor(){super(),this.elementList=[],this.t={contentStatistics:"Content Statistics",words:"Words",headings:"Headings",paragraphs:"Paragraphs",widgets:"Widgets",characters:"Characters",listView:"List view"},this.registerLocalization({context:this,namespace:"hax"}),h((()=>{l.editMode&&(this.activeNode=c(l.activeNode),setTimeout((()=>{this.requestUpdate()}),0))}))}async updateHAXMap(t){let e=[];for(var i=0;i<l.activeHaxBody.childNodes.length;i++){const t=l.activeHaxBody.childNodes[i];let n=await o(t,null);if(n.parent=null,n.node=t,e.push(n),t.children&&t.children.length>0)for(var a=0;a<t.children.length;a++){let i=await o(t.children[a],null);i.parent=n.tag,i.node=t.children[a],["span","strong","b","sup","sub","i","em","div","strike"].includes(i.tag)||e.push(i)}}this.calcStats(e);let n=[];for(i=0;i<e.length;i++){let t=l.haxSchemaFromTag(e[i].tag);t.gizmo?n.push({node:e[i].node,tag:e[i].tag,parent:e[i].parent,icon:t.gizmo.icon,name:t.gizmo.title}):e[i].tag&&e[i].tag.includes("-")?n.push({tag:e[i].tag,parent:e[i].parent,icon:"hax:templates",name:"Widget"}):n.push({tag:e[i].tag,parent:e[i].parent,icon:"hax:paragraph",name:"HTML block"})}this.elementList=[...n]}calcStats(t){if(t&&l.activeHaxBody.innerText){let i={c:l.activeHaxBody.innerText.length,w:parseInt(l.activeHaxBody.innerText.split(/\s+/g).length-1),h:0,b:0,p:0,e:0};for(var e in t.forEach((t=>{switch(t.tag){case"blockquote":case"div":case"span":case"p":case"ul":case"ol":case"strong":case"em":i.p++;break;case"h1":case"h2":case"h3":case"h4":case"h5":case"h6":i.h++;break;default:i.e++}})),i)this[`${e}Count`]=i[e]}}render(){return t`
            ...
            >
              ...
              ${"page-break"!=e.tag?t`
                    <simple-icon-button
                      ...
                      title="Lock / Unlock"
                    ></simple-icon-button>
                    ${l.isTextElement(e.node)||"grid-plate"==e.tag?t``:t`
                          <simple-icon-button
