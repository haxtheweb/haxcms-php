...
*/Polymer({is:"cms-views",behaviors:[HAXBehaviors.PropertiesBehaviors],properties:{/**
     * Loading state
   ...
   * Generate body data.
   */_generateBodyData:function(name,display){if(null!==name&&""!==name){return{name:`${name}`,display:`${display}`}}},/**
   * Handle the response from the views processing endpoint
   */_handleviewsResponse:function(newValue,oldValue){if(null!==newValue&&typeof newValue.content!==typeof void 0){// store the text and url callbacks
...
async.microTask.run(()=>{let frag=document.createElement("span");frag.innerHTML=newValue.content;let newNode=frag.cloneNode(!0);dom(this).appendChild(newNode);setTimeout(()=>{this.loading=!1},600)})}},/**
   ...
   */_viewsChanged:function(newValue,oldValue){// ensure we have something and are not loading currently
if(typeof newValue!==typeof void 0&&""!==newValue&&!this.loading){// support going from a null element to a real one
if(typeof this.viewsEndPoint===typeof void 0&&typeof window.cmsviewsEndPoint!==typeof void 0){this.viewsEndPoint=window.cmsviewsEndPoint}if(this.viewsEndPoint){this.loading=!0;async.microTask.run(()=>{this.$.viewsrequest.generateRequest()})}}},/**
   * Attached to the DOM, now fire.
   */attached:function(){if(typeof this.viewsName!==typeof void 0&&null!==this.viewsName&&""!==this.viewsName){let slot=dom(this).getEffectiveChildNodes();// only kick off request if there's nothing in it
