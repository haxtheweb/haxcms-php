...
*/Polymer({is:"cms-token",behaviors:[HAXBehaviors.PropertiesBehaviors],properties:{/**
     * Loading state
   ...
   */_displayModeChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&""!=newValue&&typeof this.token!==typeof void 0){// @todo need more sanity checks then this to get default and replace better
this.token=this.token.replace(oldValue,newValue)}},/**
   * Generate body data.
   */_generateBodyData:function(token,$editingState){if(null!==token&&""!==token){let tokenPrefix=this.tokenPrefix,tokenSuffix=this.tokenSuffix;return{token:`${tokenPrefix}${token}${tokenSuffix}`,cachedResponse:$editingState}}},/**
   * Handle the response from the token processing endpoint
   */_handleTokenResponse:function(newValue,oldValue){if(null!==newValue&&typeof newValue.content!==typeof void 0){// store the text and url callbacks in the event we're in an editing mode
...
async.microTask.run(()=>{let template=document.createElement("template");template.innerHTML=newValue.content;dom(this).appendChild(document.importNode(template.content,!0));this.loading=!1})}},/**
   * wipe out the slot
   */wipeSlot:function(element){while(null!==element.firstChild){element.removeChild(element.firstChild)}},/**
   * Token end point changed
   */_tokenChanged:function(newValue,oldValue){// ensure we have something and are not loading currently
if(typeof newValue!==typeof void 0&&""!==newValue&&!this.loading){// support going from a null element to a real one
if(typeof this.tokenEndPoint===typeof void 0&&typeof window.cmstokenEndPoint!==typeof void 0){this.tokenEndPoint=window.cmstokenEndPoint}if(this.tokenEndPoint){this.loading=!0;async.microTask.run(()=>{this.$.tokenrequest.generateRequest()})}}},/**
   * Window visibility callback to monitor when we are being seen
   */_windowVisibilityChanged:function(e){// ensure we aren't already loading
...
this._clickInvoked=!1}},/**
   ...
   */__tokenClicked:function(e){// set flag so we know to generate a new request when we come back into focus
this._clickInvoked=!0},/**
   * Detatched life cycle.
   */detached:function(){// remove event listener for clean up
document.removeEventListener("visibilitychange",this._windowVisibilityChanged.bind(this))},/**
   * Attached to the DOM, now fire.
   */attached:function(){// notice visibilty change
document.addEventListener("visibilitychange",this._windowVisibilityChanged.bind(this));if(typeof this.token!==typeof void 0&&null!==this.token&&""!==this.token){let slot=dom(this).getEffectiveChildNodes();// only kick off request if there's nothing in it
...
if(typeof this.tokenEndPoint===typeof void 0&&typeof window.cmstokenEndPoint!==typeof void 0){this.tokenEndPoint=window.cmstokenEndPoint}if(this.tokenEndPoint){this.loading=!0;async.microTask.run(()=>{this.$.tokenrequest.generateRequest()})}}}// Establish hax property binding
let props={canScale:!0,canPosition:!0,canEditSource:!1,gizmo:{title:"CMS Token",description:"CMS token rendered on the backend",icon:"icons:code",color:"light-blue",groups:["CMS"],handles:[{type:"cmstoken",token:"token"}],meta:{author:"LRNWebComponents"}},settings:{quick:[],configure:[{property:"token",title:"Token",description:"Token from our CMS",inputMethod:"textfield",icon:"editor:title"}],advanced:[]},saveOptions:{wipeSlot:!0,unsetAttributes:["loading","token-data","body-data","token-end-point"]}};this.setHaxProperties(props)},/**
   ...
   */postProcessgetHaxJSONSchema:function(schema){let href="",slot="Edit";// if we have values populate them
