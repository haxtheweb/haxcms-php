import{html,Polymer}from"../../../@polymer/polymer/polymer-legacy.js";import{dom}from"../../../@polymer/polymer/lib/legacy/polymer.dom.js";import"./map-styles.js";/* styles scoped to inside a custom element must be in a style module */Polymer({_template:html`
...
// See issue #26
this._layer._map.fire("moveend")},attached:function(){if("MAP"!==dom(this).parentNode.nodeName){console.log("ERROR: "+this.localName+"#"+this.id+" must be a child of a map element");return}// if the map has been attached, set this layer up wrt Leaflet map
if(dom(this).parentNode._map){this._attachedToMap()}},_removeEvents:function(){if(this._layer){this._layer.off("loadstart",!1,this)}},_setUpEvents:function(){this._layer.on("loadstart",function(){this.fire("loadstart",{target:this})},this)}});