...
import "../simple-colors/simple-colors.js"; // need to make this an object so that HAX can listen for it correctly
...
class GridPlate extends PolymerElement {
  ...
  canMoveOrder(item, before) {
    let nodes = this.shadowRoot.querySelector('slot[name="' + item.getAttribute("slot") + '"').assignedNodes({
      flatten: true
    });
    let target = null,
        position = 0;
  ...
  dropEvent(e) {
    if (this.editMode) {
      ...
      if (typeof this.activeItem !== typeof undefined && this.activeItem !== null && typeof local !== typeof undefined && local.getAttribute("slot") != null && this.activeItem !== local) {
        this.activeItem.setAttribute("slot", local.getAttribute("slot"));
        dom(this).insertBefore(this.activeItem, local); // ensure that if we caught this event we process it
      ...
      // which could involve a miss on the column
          ...
          dom(this).appendChild(this.activeItem); // ensure that if we caught this event we process it

          e.preventDefault();
          e.stopPropagation();
        }

      let children = dom(this).children; // walk the children and apply the draggable state needed
