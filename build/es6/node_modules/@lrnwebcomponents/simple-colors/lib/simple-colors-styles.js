...
utils.getContrastingColors = (colorName, colorShade, isLarge) => {
  ...
  Object.keys(utils.colors).forEach(color => {
    result[color] = utils.getContrastingShades(isLarge, colorName, colorShade, color);
  });
  return result.color;
};
/**
...
utils.isContrastCompliant = (isLarge, colorName, colorShade, contrastName, contrastShade) => {
  let hasGrey = colorName === "grey" || contrastName === "grey" ? "greyColor" : "colorColor",
      aa = isLarge ? "aaLarge" : "aa",
      index = parseInt(colorShade) + 1,
      range = contrasts[hasGrey][aa][index];
...
utils.maxContrastIndex = index => {
  return parseInt(index) < utils.colors["grey"].length / 2 ? utils.colors["grey"].length - 1 : 0;
};
/**
...
utils.maxContrastShade = shade => {
  return parseInt(shade) < utils.colors["grey"].length / 2 + 1 ? utils.colors["grey"].length : 1;
};
/**
...
utils.makeClass = (color = "grey", shade = 1, theme = "default", suffix = "") => {
...
utils.addCssVariables = () => {
  ...
  str.push(utils.addStatement(":host", utils.addColorShades("default", "accent", greys, false) + utils.addThemeVariables("default", false)));
  str.push(utils.addStatement(":host", utils.addColorShades("fixed", "accent", greys, false) + utils.addThemeVariables("fixed", false)));
  /**
   * dark and light themes will be deprecated
   * in favor of default and fixed themes
   */

  str.push(utils.addStatement(":host", utils.addColorShades("light", "accent", greys, false) + utils.addThemeVariables("light", false)));
  str.push(utils.addStatement(":host", utils.addColorShades("dark", "accent", greys, true) + utils.addThemeVariables("dark", true)));
  str.push(utils.addStatement(":host([dark])", utils.addColorShades("default", "accent", greys, true) + utils.addThemeVariables("default", true)));
...
utils.addAccentVariables = () => {
  ...
  for (let color in utils.colors) {
    str.push(utils.addStatement(':host([accent-color="' + color + '"])', [utils.addColorShades("default", "accent", utils.colors[color], false), utils.addColorShades("fixed", "accent", utils.colors[color], false), utils.addColorShades("light", "accent", utils.colors[color], false), utils.addColorShades("dark", "accent", utils.colors[color], true)].join("")));
    str.push(utils.addStatement(':host([dark][accent-color="' + color + '"])', [utils.addColorShades("default", "accent", utils.colors[color], true)].join("")));
  }
...
utils.addClasses = () => {
  ...
  for (let i = 0; i < themes.length; i++) {
    for (let j = 0; j < utils.colors["grey"].length; j++) {
      ...
      str.push([utils.addStatement(bg, "background-color: var(" + cssvar + ");"), utils.addStatement(bg + "-text", "color: var(" + cssvar + ");"), utils.addStatement(bg + "-border", "border: 1px solid var(" + cssvar + ");")].join(""));
...
template.innerHTML = utils.addCssVariables() + utils.addAccentVariables() + utils.addClasses();
