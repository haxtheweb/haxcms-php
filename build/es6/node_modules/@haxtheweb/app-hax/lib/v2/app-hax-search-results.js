import{SimpleColors as e}from"../../../simple-colors/simple-colors.js";import{html as t,css as a}from"../../../../lit/index.js";import{autorun as r,toJS as d}from"../../../../mobx/dist/mobx.esm.js";import{varGet as s}from"../../../utils/utils.js";import{store as o}from"./AppHaxStore.js";import"./app-hax-site-bar.js";import"./app-hax-site-details.js";import"../../../simple-tooltip/simple-tooltip.js";export class AppHaxSearchResults extends e{static get tag(){return"app-hax-search-results"}constructor(){super(),this.searchItems=[],this.displayItems=[],this.searchTerm="",this.dark=!1,this.isPointerDown=!1,this.startX=0,this.scrollLeftVal=0,this.isDragging=!1,this.currentIndex=1,this.totalItems=0,r((()=>{this.searchTerm=d(o.searchTerm)})),r((()=>{this.dark=d(o.darkMode)})),r((()=>{const e=d(o.manifest);e&&e.items&&(this.searchItems=e.items,this.displayItems=[...this.searchItems])}))}static get properties(){return{...super.properties,searchTerm:{type:String,reflect:!0},searchItems:{type:Array},displayItems:{type:Array},siteUrl:{type:String,attribute:"site-url"},isPointerDown:{type:Boolean},startX:{type:Number},scrollLeftVal:{type:Number},isDragging:{type:Boolean},currentIndex:{type:Number},totalItems:{type:Number}}}updated(e){super.updated&&super.updated(e),e.forEach(((e,t)=>{"searchTerm"===t&&(this.displayItems=this.searchItems.filter((e=>!!(e.title.toLowerCase().includes(this.searchTerm.toLowerCase())||e.description.toLowerCase().includes(this.searchTerm.toLowerCase())||e.author.toLowerCase().includes(this.searchTerm.toLowerCase())||e.slug.toLowerCase().includes(this.searchTerm.toLowerCase())))))}))}static get styles(){return[super.styles,a`
        :host {
          display: block;
          width: 100%;
          min-width: calc(
      ...
      `]}render(){return this.totalItems=this.displayItems.length,t`
      ${this.totalItems>1?t`
            <div class="pager-container">
              ${Array.from({length:this.totalItems},((e,a)=>t`
                  <button
                    class="pager-dot ${this.currentIndex===a+1?"active":""}"
                    @click="${()=>this.goToPage(a+1)}"
                    aria-label="Go to page ${a+1}"
                    tabindex="0"
                  ></button>
                `))}
            </div>
        ...
        >
          ${this.displayItems.length>0?this.displayItems.map((e=>t` <li>
                    <app-hax-site-bar
                      ...
                      site-id="${e.id}"
                      .siteUrl="${e.slug}"
                      accent-color="${s(e,"metadata.theme.variables.cssVariable","").replace("--simple-colors-default-theme-","").replace("-7","")}"
                    >
                ...
                No results${""!==this.searchTerm?t`<strong>"${this.searchTerm}"</strong>`:", Create a new site!"}
              </div>`}
        </ul>
    ...
    `}scrollLeft(){this.currentIndex<=1||this.totalItems<=1||this.shadowRoot.querySelector("#results").scrollBy({left:-800,behavior:"smooth"})}scrollRight(){this.currentIndex>=this.totalItems||this.totalItems<=1||this.shadowRoot.querySelector("#results").scrollBy({left:800,behavior:"smooth"})}handlePointerStart(e){const t=this.shadowRoot.querySelector("#results");this.isPointerDown=!0,this.isDragging=!1,this.startX=("mousedown"===e.type?e.clientX:e.touches[0].clientX)-t.offsetLeft,this.scrollLeftVal=t.scrollLeft,e.preventDefault()}handlePointerMove(e){if(!this.isPointerDown)return;const t=this.shadowRoot.querySelector("#results"),a=2*(("mousemove"===e.type?e.clientX:e.touches[0].clientX)-t.offsetLeft-this.startX);Math.abs(a)>5&&(this.isDragging=!0,t.style.scrollBehavior="auto"),this.isDragging&&(t.scrollLeft=this.scrollLeftVal-a,e.preventDefault())}handlePointerEnd(e){this.isPointerDown=!1;const t=this.shadowRoot.querySelector("#results");t&&(t.style.scrollBehavior="smooth"),setTimeout((()=>{this.isDragging=!1}),100)}handleScroll(e){if(this.totalItems<=1)return;const t=e.target.scrollLeft;this.currentIndex=Math.min(Math.max(1,Math.round(t/288)+1),this.totalItems)}goToPage(e){const t=288*(e-1);this.shadowRoot.querySelector("#results").scrollTo({left:t,behavior:"smooth"}),this.currentIndex=e}getItemDetails(e){return{created:s(e,"metadata.site.created",new Date/1e3),updated:s(e,"metadata.site.updated",new Date/1e3),pages:s(e,"metadata.pageCount",0),url:e.slug}}openedChanged(e){o.appEl.playSound("click"),e.detail.value?this.shadowRoot.querySelector("app-hax-site-details").removeAttribute("tabindex"):this.shadowRoot.querySelector("app-hax-site-details").setAttribute("tabindex","-1")}}customElements.define(AppHaxSearchResults.tag,AppHaxSearchResults);