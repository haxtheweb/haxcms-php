 ...
 */
import{LitElement as t,html as e,css as i}from"../../lit/index.js";import{HAXCMSLitElementTheme as a}from"../haxcms-elements/lib/core/HAXCMSLitElementTheme.js";import{store as o}from"../haxcms-elements/lib/core/haxcms-site-store.js";import{autorun as d,toJS as r}from"../../mobx/dist/mobx.esm.js";import"../haxcms-elements/lib/ui-components/active-item/site-active-title.js";import"../haxcms-elements/lib/ui-components/active-item/site-active-media-banner.js";import"../scroll-button/scroll-button.js";import"../simple-icon/lib/simple-icon-button-lite.js";import"../simple-icon/lib/simple-icon-lite.js";import{DDDSuper as s}from"../d-d-d/d-d-d.js";import{DDDVariables as l}from"../d-d-d/lib/DDDStyles.js";import{DDDAllStyles as n}from"../d-d-d/lib/DDDStyles.js";import{licenseList as c}from"../license-element/license-element.js";import{UserScaffoldInstance as p}from"../user-scaffold/user-scaffold.js";const h=["https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400..900;1,400..900&family=Source+Code+Pro:ital,wght@0,200..900;1,200..900&family=Work+Sans:ital,wght@0,100..900;1,100..900&display=swap"];function getPostLogo(t){return t.metadata.image?t.metadata.image:o.manifest.metadata.theme.variables.image?r(o.manifest.metadata.theme.variables.image):r(o.manifest.metadata.site.logo)}export class CleanPortfolioTheme extends(s(a)){static get tag(){return"clean-portfolio-theme"}constructor(){super(),this.dataPalette=p.readMemory("HAXCMSSitePalette")||"",this.selectedTag="",this.activeLayout="text",this.menuOpen=!1,this.menuOverflow=[],this.lastUpdated="",this.copyrightYear=0,this.HAXSiteCustomRenderRoutes={"x/tags":{items:this.HAXSiteRenderXTagsItems}},this.HAXCMSThemeSettings.autoScroll=!0,this.categoryTags=[],this.allTags=[],this.items=[],this.__disposer=this.__disposer||[],this.__disposer.push(d(t=>{this.homeLink=r(o.homeLink)})),this.__disposer.push(d(t=>{this.siteTitle=r(o.siteTitle)})),this.__disposer.push(d(t=>{this.ancestorItem=r(o.ancestorItem)})),this.__disposer.push(d(t=>{let e=r(o.activeTags);e&&e.length>0?this.activeTags=e.split(","):this.activeTags=[]})),this.__disposer.push(d(t=>{let e=o.getItemChildren(null);e&&e.length>0&&(this.topItems=[...e])})),this.__disposer.push(d(t=>{const e=r(o.activeItem);if(e){let t=o.manifest.items.find(t=>e.parent===t.id);if(t){const t=e.metadata.tags&&e.metadata.tags.split(",").map(t=>t.trim()),i=t&&t[0]||null,a=o.manifest.items.filter(t=>{const a=t.metadata&&t.metadata.tags&&t.metadata.tags.split(",").map(t=>t.trim()),o=a&&a[0]||null;return t.parent===e.parent&&o===i}),d=a.findIndex(t=>t.id===e.id);this.prevSibling=a[d-1]||null,this.nextSibling=a[d+1]||null}else t="";this.menuOpen&&(this.menuOpen=!1),globalThis.document&&globalThis.document.startViewTransition?globalThis.document.startViewTransition(()=>{this.activeItem=e,this.activeParent=t}):(this.activeItem=e,this.activeParent=t);const i=o.getItemChildren(o.activeId);if(i)if(i.length>0){this.setLayout("listing");const t=[],e=[];if(i.forEach(i=>{let a=r(i.metadata.tags);if(a){const i=a.split(",");i[0]&&!t.includes(i[0])&&t.push(i[0]),i.forEach(t=>{t&&!e.includes(t)&&e.push(t)})}}),globalThis.document&&globalThis.document.startViewTransition?globalThis.document.startViewTransition(()=>{this.items=[...i],this.categoryTags=[...t],this.allTags=[...e]}):(this.items=[...i],this.categoryTags=[...t],this.allTags=[...e]),this.selectedTag="",this.shadowRoot){let t=this.shadowRoot.querySelector("#listing-filter");t&&(t.value="")}}else e.parent?this.setLayout("media"):this.setLayout("text")}})),this.__disposer.push(d(t=>{if(r(o.editMode)){(this.shadowRoot.querySelector("#slot")||globalThis.document.querySelector("#slot")).scrollIntoView({behavior:"smooth",block:"start"})}})),this.__disposer.push(d(t=>{this.manifest=r(o.manifest);let e=new c;this.manifest.license&&e[this.manifest.license]&&(this.licenseName=e[this.manifest.license].name,this.licenseLink=e[this.manifest.license].link,this.licenseImage=e[this.manifest.license].image)})),this.__disposer.push(d(t=>{const e=r(o.pageCounter);this.pageCurrent=e.current,this.pageTotal=e.total}))}firstUpdated(t){super.firstUpdated(t);let e=globalThis.DesignSystemManager.requestAvailability();e.addDesignSystem({name:"clean-portfolio-theme",styles:[...CleanPortfolioTheme.styles,l],fonts:h,hax:!0}),e.active="clean-portfolio-theme",this.lastUpdated=new Date(1e3*o.manifest.metadata.site.updated).toDateString(),this.copyrightYear=new Date(1e3*o.manifest.metadata.site.created).getFullYear();const i=this.renderRoot.querySelector("nav");i&&(this._resizeObserver=new ResizeObserver(()=>{this._checkOverflow()}),this._resizeObserver.observe(i)),requestAnimationFrame(()=>this._checkOverflow())}updated(t){super.updated(t),t.has("dataPalette")&&p.writeMemory("HAXCMSSitePalette",this.dataPalette,"long")}disconnectedCallback(){if(this._resizeObserver&&this._resizeObserver.disconnect(),this.__disposer)for(var t in this.__disposer)this.__disposer[t].dispose();super.disconnectedCallback()}_checkOverflow(){const t=this.renderRoot.querySelector("nav");if(t){const e=Array.from(t.children),i=t.clientWidth-100;let a=0;const o=[];for(const t of e)t.style.display="inline-block",a+=t.offsetWidth+25,a>i&&(t.style.display="none",o.push(this.topItems.find(e=>e.slug===t.getAttribute("href"))));this.menuOverflow=o,this.requestUpdate()}}static get properties(){return{...super.properties,dataPalette:{type:Number,reflect:!0,attribute:"data-palette"},activeItem:{type:Object},activeParent:{type:Object},ancestorItem:{type:Object},prevSibling:{type:Object},nextSibling:{type:Object},topItems:{type:Array},items:{type:Array},activeTags:{type:Array},categoryTags:{type:Array},allTags:{type:Array},menuOverflow:{type:Array},menuOpen:{type:Boolean},copyrightYear:{type:Number},pageCurrent:{type:Number},pageTotal:{type:Number},siteTitle:{type:String},homeLink:{type:String},activeLayout:{type:String},selectedTag:{type:String},lastUpdated:{type:String},licenseName:{type:String},licenseLink:{type:String},licenseImage:{type:String}}}HAXCMSGlobalStyleSheetContent(){return[...super.HAXCMSGlobalStyleSheetContent(),n,i`
        :root, html, body {
...
`]}static get styles(){return[n,super.styles,i`
      /* Semantic elements */
 ...
 */
    ...
    `]}testEditMode(t){this.editMode?(t.preventDefault(),t.stopPropagation(),t.stopImmediatePropagation()):t.currentTarget.blur()}_renderListing(){const t=this.selectedTag?this.items.filter(t=>{let e=r(t.metadata.tags);return!!e&&e.includes(this.selectedTag)}):this.items,i=this.selectedTag?this.categoryTags.filter(e=>t.some(t=>{let i=r(t.metadata.tags);return!!i&&i.includes(e)})):this.categoryTags;return e`
      <!-- Render cards based on filtered tags -->
      ${i.length>0?i.map(i=>e`
              <h2 class="listing-category"><a tabindex="${this.editMode?"-1":"0"}" ?disabled="${this.editMode}" @click="${this.testEditMode}" href="x/tags?tag=${i.trim()}">${i}</a></h2>
              <div class="listing-grid">
                ${t.filter(t=>{let e=r(t.metadata.tags);return!!e&&e.includes(i)}).map(t=>{let i=r(t.metadata.tags).split(",")[1];return e`
      ...
      ${!this.selectedTag&&this.items.some(t=>!r(t.metadata.tags))?e`
        <div class="listing-category"></div>
        <div class="listing-grid">
          ${this.items.filter(t=>!r(t.metadata.tags)).map(t=>e`
    ...
    </div>`}setLayout(t){globalThis.document&&globalThis.document.startViewTransition?globalThis.document.startViewTransition(()=>{this.activeLayout=t}):this.activeLayout=t}togglePalette(t){this.dataPalette++,this.dataPalette>10&&(this.dataPalette=0)}render(){return e`
      <header>
      ...
      <div id="contentcontainer" class="container">
        ${"listing"==this.activeLayout?e`
              <div class="listing-titlecontainer">
                ...
                ${this.categoryTags.length>1||this.categoryTags.length>0&&this.items.some(t=>!t.metadata.tags)?e`
          ...
          `:e`
                ...
                ${this.activeTags&&this.activeTags.length>0?e`
                      <ul class="tag-list">
                        ${("listing"===this.activeLayout?this.activeTags:this.activeTags.slice(1)).map(t=>e`<li><a tabindex="${this.editMode?"-1":"0"}" @click="${this.testEditMode}" ?disabled="${this.editMode}" href="x/tags?tag=${t.trim()}">${t}</a></li>`)}
                      </ul>
                  `:""}
          `}
