import{html as e,css as i}from"../../../../../lit/index.js";import{autorun as t,toJS as s}from"../../../../../mobx/dist/mobx.esm.js";import{DDD as o}from"../../../../d-d-d/d-d-d.js";import{store as a}from"../haxcms-site-store.js";import{HAXStore as r}from"../../../../hax-body/lib/hax-store.js";import{HAXCMSKeyboardShortcutsInstance as d}from"../utils/HAXCMSKeyboardShortcuts.js";import"../../../../simple-icon/lib/simple-icon-lite.js";class HAXCMSAboutDialogUI extends o{static get tag(){return"haxcms-about-dialog-ui"}static get properties(){return{haxVersion:{type:String,attribute:"hax-version"},keyboardShortcuts:{type:Array},markdownShortcuts:{type:Array}}}constructor(){super(),this.haxVersion="Loading…",this.keyboardShortcuts=[],this.markdownShortcuts=[],this.__disposer=[],this._loadKeyboardShortcuts(),this._loadMarkdownShortcuts()}connectedCallback(){super.connectedCallback();const e=t(()=>{const e=s(a.version);this.haxVersion=e||"Loading…"});this.__disposer.push(e),this._loadKeyboardShortcuts(),this._loadMarkdownShortcuts()}disconnectedCallback(){for(;this.__disposer.length;){const e=this.__disposer.pop();e&&"function"==typeof e?e():e&&"function"==typeof e.dispose&&e.dispose()}super.disconnectedCallback()}_loadKeyboardShortcuts(){const e=d.getShortcutsForDisplay();this.keyboardShortcuts=e.slice().sort((e,i)=>`${e.label} ${e.description}`.localeCompare(`${i.label} ${i.description}`))}_loadMarkdownShortcuts(){const e=r.keyboardShortcuts||{},i=Object.keys(e),t=["#","##","###","####","#####","######","1.","-","*","+",">","```","---","***","___"];i.forEach(e=>{t.includes(e)||t.push(e)}),this.markdownShortcuts=t.filter(i=>e[i]).map(i=>({trigger:i,description:this._markdownShortcutDescription(e[i])}))}_markdownShortcutDescription(e){if(!e||!e.tag)return"Insert block";const i=e.tag.toLowerCase();switch(i){case"h2":return"Heading 2";case"h3":return"Heading 3";case"h4":return"Heading 4";case"h5":return"Heading 5";case"h6":return"Heading 6";case"ol":return"Numbered list";case"ul":return"Bulleted list";case"hr":return"Horizontal rule";case"code":return"Code block";case"blockquote":return"Blockquote";default:return`Insert ${i}`}}_shortcutsForContext(e){return this.keyboardShortcuts.filter(i=>i.context===e)}_renderShortcutList(i){return i&&0!==i.length?e`<ul class="shortcut-list">
      ${i.map(i=>e`<li>
          <span class="shortcut-key">${i.label}</span>
      ...
      ${this.markdownShortcuts.map(i=>e`<li>
          <span class="shortcut-key"><code>${i.trigger}</code></span>
          <span>${i.description}</span>
        </li>`)}
    </ul>`:e`<p class="section-description">
        No editor shortcuts are registered.
      </p>`}static get styles(){return[super.styles,i`
        :host {
      ...
      `]}render(){const i=this._shortcutsForContext("edit"),t=this._shortcutsForContext("view"),s=this._shortcutsForContext("global");return e`
