 ...
 */
import{html as t,css as e}from"../../lit/index.js";import{QuestionElement as r}from"../multiple-choice/lib/QuestionElement.js";import"../simple-icon/lib/simple-icon-button-lite.js";class MatchingQuestion extends r{static get tag(){return"matching-question"}static get styles(){return[super.styles,e`
        :host {
          display: block;
        }
      ...
      `]}constructor(){super(),this.guessDataValue="matchAnswers",this.__activeOption={},this.dragEnter=!1,this.dragging=!1,this.matchAnswers=[],this.matchTarget=!1}processInput(t,e,r){let s=super.processInput(t,e);if(!1===s.correct){s.matchOption=!0;for(let t=r.length;t>=0;t--)!s.match&&r[t]&&!0===r[t].target&&(s.match=r[t].order)}else s.target=!0;return s}cleanAnswerData(t){this.matchAnswers=[],this.showAnswer=!1;let e=[];for(let r in t){let s={...this.answerPrototype(),...t[r]};s.order=parseInt(r),s.match=!1,s.matchOption=!1,s.target=!1,e.push({...this.cleanAnswerDataBeforeSend(s,parseInt(r),e)})}return e}cleanAnswerDataBeforeSend(t,e,r){if(!1===t.correct){t.matchOption=!0;for(let s=e;s>=0;s--)!t.match&&r[s]&&!0===r[s].target&&(t.match=r[s].order)}else t.target=!0;return t}resetAnswer(){if(this.showAnswer=!1,globalThis.dispatchEvent(new CustomEvent("simple-toast-hide",{bubbles:!0,composed:!0,cancelable:!1,detail:!1})),this.isCorrect()){this.displayedAnswers=[],this.matchAnswers=[];const t=JSON.parse(JSON.stringify(this.answers));this.answers=[...t]}setTimeout(()=>{this.requestUpdate()},0)}isCorrect(){let t=!0;for(var e=0;e<this.matchAnswers.length;e++)this.matchAnswers[e].guess!==this.matchAnswers[e].match?(t=!1,this.matchAnswers[e].correct=!1):this.matchAnswers[e].correct=!0;for(e=0;e<this.displayedAnswers.length;e++)this.displayedAnswers[e].match&&(t=!1);return t}renderInteraction(){return t` <dialog id="selecttarget">
        <simple-icon-button-lite
          icon="close"
      ...
      </div>`}renderOption(e,r){return t`<button
      ?disabled="${this.disabled||this.showAnswer}"
      class="tag-option ${"possible"!=r&&this.showAnswer?e.correct?"correct":"incorrect":""}"
    ...
    </p>`}focusActive(t){this.shadowRoot.querySelector(`button[data-label="${t.label}"]`).focus()}handleTagClick(t){let e=0;this.__activeOption={...t};let r=Array.from(this.shadowRoot.querySelectorAll("dialog option"));for(var s in r)parseInt(r[s].value)===t.guess&&(e=s);this.shadowRoot.querySelector("dialog select").selectedIndex=e,this.shadowRoot.querySelector("dialog").showModal()}selectTargetChange(t){let e=parseInt(t.target.value);this.__activeOption.guess=e;let r=this.matchAnswers.findIndex(t=>t.order===this.__activeOption.order);r>-1&&this.matchAnswers.splice(r,1),r=this.displayedAnswers.findIndex(t=>t.order===this.__activeOption.order),r>-1&&this.displayedAnswers.splice(r,1),""===t.target.value?(this.__activeOption.guess=null,this.displayedAnswers.push(JSON.parse(JSON.stringify(this.__activeOption)))):this.matchAnswers.push(JSON.parse(JSON.stringify(this.__activeOption))),this.shadowRoot.querySelector("dialog select").selectedIndex=0,this.shadowRoot.querySelector("dialog").close(),this.requestUpdate(),setTimeout(()=>{this.focusActive(this.__activeOption),this.__activeOption={guess:null}},0)}static get properties(){return{...super.properties,dragging:{type:Boolean,reflect:!0},dragEnter:{type:Boolean,reflect:!0,attribute:"drag-enter"},matchTarget:{type:Boolean,reflect:!0,attribute:"match-target"},matchAnswers:{type:Array},__activeOption:{type:Object}}}static get haxProperties(){return new URL(`./lib/${this.tag}.haxProperties.json`,import.meta.url).href}}globalThis.customElements.define(MatchingQuestion.tag,MatchingQuestion);export{MatchingQuestion};