...
*/(0,_polymerLegacy.Polymer)({is:"cms-entity",behaviors:[HAXBehaviors.PropertiesBehaviors],properties:{/**
     * Loading state
   ...
   * Generate body data.
   */_generateBodyData:function(entityType,entityId,entityDisplayMode){if(null!==entityType&&""!==entityType&&null!==entityId&&""!==entityId){return{type:`${entityType}`,id:`${entityId}`,display_mode:`${entityDisplayMode}`}}},/**
   * Handle the response from the entity processing endpoint
   */_handleEntityResponse:function(newValue,oldValue){if(null!==newValue&&typeof newValue.content!==typeof void 0){// store the text and url callbacks
...
async.microTask.run(()=>{let frag=document.createElement("span");frag.innerHTML=newValue.content;let newNode=frag.cloneNode(!0);(0,_polymerDom.dom)(this).appendChild(newNode);setTimeout(()=>{this.loading=!1},600)})}},/**
   * wipe out the slot
   */wipeSlot:function(element){while(null!==element.firstChild){element.removeChild(element.firstChild)}},/**
   * entity end point changed
   */_entityChanged:function(newValue,oldValue){// ensure we have something and are not loading currently
if(typeof newValue!==typeof void 0&&""!==newValue&&!this.loading){// support going from a null element to a real one
if(typeof this.entityEndPoint===typeof void 0&&typeof window.cmsentityEndPoint!==typeof void 0){this.entityEndPoint=window.cmsentityEndPoint}if(this.entityEndPoint){this.loading=!0;async.microTask.run(()=>{this.$.entityrequest.generateRequest()})}}},/**
   * Attached to the DOM, now fire.
   */attached:function(){if(typeof this.entity!==typeof void 0&&null!==this.entity&&""!==this.entity){let slot=(0,_polymerDom.dom)(this).getEffectiveChildNodes();// only kick off request if there's nothing in it
