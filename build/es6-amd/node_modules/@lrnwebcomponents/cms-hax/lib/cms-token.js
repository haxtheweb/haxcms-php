...
async.microTask.run(()=>{let template=document.createElement("template");template.innerHTML=newValue.content;(0,_polymerDom.dom)(this).appendChild(document.importNode(template.content,!0));this.loading=!1})}},/**
   * wipe out the slot
...
document.addEventListener("visibilitychange",this._windowVisibilityChanged.bind(this));if(typeof this.token!==typeof void 0&&null!==this.token&&""!==this.token){let slot=(0,_polymerDom.dom)(this).getEffectiveChildNodes();// only kick off request if there's nothing in it
