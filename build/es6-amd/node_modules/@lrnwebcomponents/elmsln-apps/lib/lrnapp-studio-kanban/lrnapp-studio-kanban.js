define(["../../../../@polymer/polymer/polymer-legacy.js","../../../../@polymer/polymer/lib/legacy/polymer.dom.js","../../../../@polymer/iron-ajax/iron-ajax.js","../../../../@polymer/app-route/app-location.js","../../../../@polymer/app-route/app-route.js","../../../../@polymer/iron-icon/iron-icon.js","../../../../@polymer/paper-badge/paper-badge.js","../../../../@polymer/paper-toggle-button/paper-toggle-button.js","../../../../@polymer/app-layout/app-toolbar/app-toolbar.js","../../../../@polymer/paper-card/paper-card.js","../../../../@polymer/iron-list/iron-list.js","../../../../@polymer/paper-toast/paper-toast.js","../../../../@polymer/paper-button/paper-button.js","../../../../@polymer/paper-dialog/paper-dialog.js","../../../lrnsys-render-html/lrnsys-render-html.js","../../../lrnsys-layout/lib/lrnsys-dialog.js","../../../lrnsys-button/lrnsys-button.js","../../../elmsln-loading/elmsln-loading.js","../../../materializecss-styles/materializecss-styles.js","./lrnapp-studio-project-button.js","./lrnapp-studio-assignment-button.js"],function(_polymerLegacy,_polymerDom,_ironAjax,_appLocation,_appRoute,_ironIcon,_paperBadge,_paperToggleButton,_appToolbar,_paperCard,_ironList,_paperToast,_paperButton,_paperDialog,_lrnsysRenderHtml,_lrnsysDialog,_lrnsysButton,_elmslnLoading,_materializecssStyles,_lrnappStudioProjectButton,_lrnappStudioAssignmentButton){"use strict";(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
  ...
  `,is:"lrnapp-studio-kanban",properties:{/**
     * Tracks the active assignment.
     */activeAssignment:{type:String,value:null,notify:!0/* transition */ /* transition */},projectToDelete:{type:String,value:null,notify:!0},assignmentToDelete:{type:String,value:null,notify:!0},sourcePath:{type:String,notify:!0},/**
     ...
     */backendResponse:{type:Object,notify:!0}},listeners:{"project-created":"_handleProjectCreated","assignment-created":"_handleAssignmentCreated"},observers:["_routeChanged(route, endPoint)","_deleteToast(queryParams.deletetoast)"],// If the current route is outside the scope of our app
// then allow the website to break out of the single page
// application routing
_routeChanged:function(route,endPoint){if("string"===typeof route.path){if("string"===typeof endPoint){if(route.path.startsWith(endPoint)){return}}window.location=route.path}},/**
   * Support having a toast message because of delete or error elsewhere.
   */_deleteToast:function(deletetoast,old){if(typeof deletetoast!==typeof void 0){if("error"==deletetoast){this.$.toast.show("That submission on longer exists!")}else{this.$.toast.show("Submission deleted successfully!")}this.set("queryParams.deletetoast",void 0);this.notifyPath("queryParams.deletetoast")}},/**
   * @todo  remove this once we have a modal for it
   */_makeProjectEditLink:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");window.location.href=this.basePath+"../node/"+parts[1]+"/edit?destination=apps/lrnapp-studio-kanban"},/**
   * @todo  remove this once we have a modal for it
   */_makeAssignmentEditLink:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");window.location.href=this.basePath+"../node/"+parts[2]+"/edit?destination=apps/lrnapp-studio-kanban"},/**
   * Handle the push to delete a project, pop up the modal.
   */_deleteProjectDialog:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");// set values so we know what to drop
this.projectToDelete=parts[1];this._deleteTitle="Delete project";this._deleteText="Are you sure you want to delete this project and all related assignments!?";this._deleteType="project";this.$.delete.open()},/**
   ...
   */_handleDelete:function(){if("project"==this._deleteType){this.$.backend.method="DELETE";this.$.backend.body=this.projectToDelete;this.$.backend.url=this.endPoint+"/api/projects/"+this.projectToDelete+"?token="+this.csrfToken;this.$.backend.generateRequest()}else if("assignment"==this._deleteType){this.$.backend.method="DELETE";this.$.backend.body=this.assignmentToDelete;this.$.backend.url=this.endPoint+"/api/assignments/"+this.assignmentToDelete+"?token="+this.csrfToken;this.$.backend.generateRequest()}},/**
   * Handle the push to delete an assignment, pop up the modal.
   */_deleteAssignmentDialog:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");this.assignmentToDelete=parts[2];// set values so we know what to drop
this._deleteTitle="Delete assignment";this._deleteText="Are you sure you want to delete this assignment?";this._deleteType="assignment";this.$.delete.open()},/**
   ...
   */displayNewBadge:function(count){if(0==count){return!0}return!1},/**
   * Handle toggling the status for the submission.
   */statusToggle:function(e){// find our xhr for callbacks
...
if(!this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled){// hide the loading screen
this.$.loading.hidden=!1;// queue of the request parameters
xhr.params={submissionid:submission.submission.id,status:this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked};// send the request
xhr.generateRequest()}},/**
   * Handle toggle for mouse class and manage classList array for paper-button.
   */assignmentFocusIn:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;if(null!=this.activeAssignment&&this.activeAssignment!=local.id){this.shadowRoot.querySelector("#"+this.activeAssignment).nextElementSibling.classList.remove("show");this.shadowRoot.querySelector("#"+this.activeAssignment).classList.remove("active")}this.activeAssignment=local.id;this._setToggle(!1);local.nextElementSibling.classList.add("show");local.classList.add("active")},/**
   * Handle response for the whole projects object.
   */_handleProjectResponse:function(event){this.$.loading.hidden=!0;this._setToggle(!0)},/**
   * buildSubmissionPath for generating a new submission or linking to existing.
   */buildSubmissionPath:function(path){return path+"lrnapp-studio-submission"},/**
   * Handle a response from updating an item
   */_handleUpdateResponse:function(event){if(200==this.backendResponse.status){this.$.toast.text="Updated successfully";this.$.toast.toggle();this.$.projectbackend.generateRequest()}else{// this would imply an error
this.$.loading.hidden=!0}},/**
   * set the toggle state when assignment gets focus
   */_setToggle:function(update){if(null!=this.activeAssignment){var parts=this.activeAssignment.split("-"),submission=this.projectResponse.data.projects["project-"+parts[1]].relationships.assignments["assignment-"+parts[2]].meta.relatedSubmissions.complete;// focus in on the submissions / assignment meta
// not finished but also not started
if(0==submission.status&&0==submission.submission.length){if(!update){this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled=!0;this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked=!1}this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").title="Submission not started";this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle-text").textContent="Submission not started"}else if(0==submission.status){if(!update){this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled=!1;this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked=!1}this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").title="Submission started";this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle-text").textContent="Submission in progress"}else{if(!update){this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled=!1;this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked=!0}this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").title="Submission ready";this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle-text").textContent="Submission ready"}}},/**
   * Event came from the project button to indicate it was successful.
   */_handleProjectCreated:function(e){this.$.toast.text="Project added";this.$.toast.toggle();this.$.projectbackend.generateRequest()},/**
   * Event came from the assignment button to indicate it was successful.
   */_handleAssignmentCreated:function(e){this.$.toast.text="Assignment added";this.$.toast.toggle();this.$.projectbackend.generateRequest()},/*
