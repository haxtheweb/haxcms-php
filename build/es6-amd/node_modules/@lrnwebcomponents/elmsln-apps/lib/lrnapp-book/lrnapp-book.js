...
*/(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
  ...
  `,is:"lrnapp-book",behaviors:[HAXCMSBehaviors.Theme],properties:{/**
   ...
   * Ready event.
   */ready:function(e){this.setupHAXTheme(!0,this.$.contentcontainer);window.dispatchEvent(new CustomEvent("haxcms-router-manifest-subscribe",{detail:{callback:"_haxcmsRouterManifestSubscribeHandler",scope:this,setup:!0}}));window.SimpleToast.requestAvailability();this.$.bodyloading.hidden=!0;// scroll top into view
setTimeout(()=>{this._resetScroll()},500)},/**
   * attached life cycle
   */attached:function(){this.$.progress.addEventListener("node-percent-milestone",this.testMilestone.bind(this));this.$.mapmenu.addEventListener("link-clicked",this._menuItemClicked.bind(this))},/**
   * detached life cycle
   ...
   */_fullWidthChanged:function(newValue,oldValue){this.updateStyles()},/**
   * Handle click on dashboard to trigger loading data.
   */progressdashboardopen:function(e){this.$.progressdashboard.showProgress=!0},/**
   * Generate path to point to the right endpoint for updating items.
   */_computePageUpdatePath:function(data,sourcePath){return sourcePath.replace("%",data.id)},/**
   * React when state changes for editMode
   */_editModeChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0){if(!0===newValue){this.$.currenttitle.contentEditable=!0;// block scroll tracking during edit mode
...
if(this.$.currenttitle.innerHTML!==this.currentPageData.title){this.currentPageData.title=this.$.currenttitle.innerHTML}const evt=new CustomEvent("simple-toast-show",{bubbles:!0,cancelable:!0,detail:{text:"Saved",duration:0}});this.dispatchEvent(evt)}}}},/**
   ...
   */_activeOutlineChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&typeof oldValue!==typeof void 0){// trigger loading state
this.rebuildOutline=!0}},/**
   * Reset scroll position visually and internally data wise.
   */_resetScroll:function(){this.resetScroll=!0;this.scrollPosition=0;this.$.anchor.scrollIntoView({block:"start",behavior:"smooth",inline:"nearest"})},/**
   * React to active page being changed.
   */_activePageChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0){if(typeof this.outlineItems!==typeof void 0&&0<this.outlineItems.length){this.fire("json-outline-schema-active-item-changed",this.outlineItems[newValue])}// scroll into view the container that's about to be swapped out
...
if(typeof this.outlineItems!==typeof void 0&&newValue+1==this.outlineItems.length){this.hasNextPage=!1}else{this.hasNextPage=!0;if(typeof this.outlineItems!==typeof void 0){this.nextLabel=this.outlineItems[newValue+1].title}}}},/**
   ...
   */_outlineItemsChanged:function(newValue,oldValue){// these need set immediately
...
if(this.activePage+1!=newValue.length){this.nextLabel=newValue[this.activePage+1].title}}},/**
   ...
   */testMilestone:function(e){// we should preload the next page
if(75==e.detail.percentage){console.log("@todo preload the next page and present grayed out right of UI.")}},/**
   ...
   */_scrollChanged:function(newValue,oldValue){// only evaluate scroll if value is greater then previous
...
if(75<=newValue){this.outlineItems[this.activePage].metadata.value=this.outlineItems[this.activePage].metadata.max;this.set("outlineItems."+this.activePage+".metadata.value",this.outlineItems[this.activePage].metadata.max)}else{this.outlineItems[this.activePage].metadata.value=newValue;this.set("outlineItems."+this.activePage+".metadata.value",newValue)}}},/**
   ...
   */_nextBtn:function(e){// make sure we are able to move forward more
if(this.activePage<this.outlineItems.length-1){this.set("outlineItems."+this.activePage+".metadata.value",this.outlineItems[this.activePage].metadata.max);this.activePage=this.activePage+1}},/**
   ...
   */_prevBtn:function(e){if(0<this.activePage){this.activePage=this.activePage-1}},/**
   * Toggle the book drawer
   */toggleBook:function(e){// if we are in edit mode then we ned to close this
this.$.bookdrawer.toggle();this.fullWidth=!this.$.bookdrawer.opened},/**
   * Handle the response.
   */_manifestChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0){const items=newValue.items,outlineTitle=newValue.title;// set active to 0 because once we update the outlineItems it will try to
...
this.set("outlineTitle",outlineTitle);this.$.outlineloading.hidden=!0;this.pageParams.load=!0}},/**
   ...
   */_currentPageDataUpdated:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&typeof newValue.content!==typeof void 0){// set page title; easiest for sure
