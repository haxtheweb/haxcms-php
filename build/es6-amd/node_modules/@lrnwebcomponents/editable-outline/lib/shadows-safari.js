define(["exports"],function(_exports){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0/* transition */ /* transition */});_exports.getRange=getRange;_exports.internalGetShadowSelection=internalGetShadowSelection;/**
...
}const cachedRange=new Map;function getRange(root){if(root.getSelection){const s=root.getSelection();return s.rangeCount?s.getRangeAt(0):null}const thisFrame=cachedRange.get(root);if(thisFrame){return thisFrame}const initialText=window.getSelection().toString(),result=internalGetShadowSelection(root),rs=result.range&&result.range.toString()||null;if(null!==rs&&rs!==initialText){// TODO: sometimes triggers on single-char hack etc
...
console.warn("invalid range, initial text:",initialText);console.warn("vs",rs,result.mode,result.range)}}cachedRange.set(root,result.range);window.setTimeout(()=>{cachedRange.delete(root)},0);debug&&console.debug("getRange got",result);return result.range}const fakeSelectionNode=document.createTextNode("");function internalGetShadowSelection(root){const range=document.createRange(),s=window.getSelection();if(!s.containsNode(root.host,!0)){return{range:null,mode:"none"}}// TODO: inserting fake nodes isn't ideal, but containsNode doesn't work on nearby adjacent
