define(["../../../@polymer/polymer/polymer-legacy.js","./paper-search-bar.js","./paper-filter-dialog.js"],function(_polymerLegacy,_paperSearchBar,_paperFilterDialog){"use strict";(0,_polymerLegacy.Polymer)({is:"paper-search-panel",/**
   ...
   */properties:{/**
     ...
     */noValuesLabel:String,_hasItems:{type:Boolean,computed:"_computeHasItems(items)",value:!1}},getPaperSearchBarInstance:function(){return this.$.paperSearchBar},// Private methods
_loadMore:function(){this.count+=20;this._updateData()},_computeHasItems:function(items){return"undefined"!==typeof items&&0<items.length},_showNoResults:function(_hasItems,loading){return!_hasItems&&!loading},_onChangeRequest:function(newValue,oldValue){// Ignore initial setting of properties (caller is supposed to trigger this call automatically)
if("undefined"!==typeof oldValue){// Set back to default to avoid endless listers
this.count=20;this._updateData()}},_updateData:function(){this.fire("change-request-params")},_onFilter:function(){this.$.filterDialog.open()},_onSearch:function(){this.fire("search")},// Counts the selected filters
_getNrSelectedFilters:function(selectedFilters){if(0>=Object.keys(selectedFilters).length){return 0}var nrSelectedFilters=Object.keys(selectedFilters).map(function(key){// Returns number of selected value for a filter
return selectedFilters[key].length}).reduce(function(sum,value){// Sum up the selected values across filters
return sum+value});return nrSelectedFilters},_disableFilterButton:function(filters){return!(filters&&0<filters.length)}})});