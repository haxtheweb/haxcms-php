 ...
 */(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
  ...
  `,is:"lrnsys-progress-circle",listeners:{tap:"tapEventOn",mouseover:"focusOn",mouseout:"focusOff","focused-changed":"focusEvent"},properties:{/**
     * Current value.
   ...
   * Ready state
   */ready:function(e){// seems odd but if we don't do this we can get issues with
...
this._bubbleProgress={25:!1,50:!1,75:!1}},/**
   ...
   */_testValueComplete:function(newValue,oldValue){// ensure we were previously available before marking complete
...
// over reported percentages upstream
   ...
   */focusEvent:function(e){// see if it has hover classes
...
this.$.icon.classList.remove("activeIcon")}this.focusState=!this.focusState}},/**
   * Focus event for UX consistency.
   */focusOn:function(e){// see if it has hover classes
...
this.$.icon.icon=this.icon;this.$.icon.classList.add("activeIcon")}},/**
   * Focus event for UX consistency.
   */focusOff:function(e){// see if it has hover classes
if(!this.disabled&&"loading"!=this.status){// if complete set it back to what it was
if("complete"==this.status||"finished"==this.status){this.$.icon.icon=this.activeIcon}// drop the class for active step
this.$.icon.classList.remove("activeIcon")}},/**
   * Tapped on the item.
   */tapEventOn:function(e){let target=e.target;this.fire("node-is-active",{target})},/**
   * Calculate which icon to present.
   */_getActiveIcon:function(icon,iconComplete,status){if(typeof icon!==typeof void 0){var tmp=icon;// support for a loading icon while loading
if("loading"==status){tmp=this.loadingIcon;this.$.icon.classList.add("transition")}else if("finished"==status){tmp=this.finishedIcon}else if("complete"==status&&typeof iconComplete!==typeof void 0){if(this.playSound&&!this.__chimed){this._playSound()}tmp=iconComplete}else{this.$.icon.classList.remove("transition")}return tmp}return!1},/**
   ...
   */_playSound:function(){// calculate sound to use and ensure it only ever happens once
...
window.audio=new Audio}window.audio.play()},/**
   ...
   */_loadingStatus:function(status){if("loading"==status){return!0}return!1},/**
   * Calculate loading based on the status of the item.
   */_finishedStatus:function(status){if("finished"==status){if(this.playFinishSound&&!this.__finishchimed){this._playSound()}return!0}return!1},/**
   * Notice when state changes to fire up an event for others to respond to.
   */_statusChange:function(newValue,oldValue){// verify this isn't loading up
if(typeof oldValue!==typeof void 0&&newValue!==oldValue){this.fire("node-status-change",{status:newValue})}},/**
   * Calculate disable based on the status of the item.
   */_disableStatus:function(status){if("disabled"==status){return!0}return!1}})});