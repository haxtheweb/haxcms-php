 ...
 * @customElement
 * @polymer
 */class AbsolutePositionStateManager extends _polymerElement.PolymerElement{/* REQUIRED FOR TOOLING DO NOT TOUCH */ /**
   * Store the tag name to make it easier to obtain directly.
   * @notice function name must be here for tooling to operate correctly
   */static get tag(){return"absolute-position-state-manager"}// properties available to the custom element for data binding
...
if(!window.AbsolutePositionStateManager.instance){window.AbsolutePositionStateManager.instance=this;return this}}/**
   ...
   */loadElement(el){let root=this;root.__on=root.__on!==void 0?root.__on:!1;root.elements.push(el);root.updatePosition(el);if(!root.__on)root.addEventListeners();root.__on=!0}/**
   * Unloads element from array
   */unloadElement(el){let root=this;root.elements.filter(element=>element===el);root.__on=0<root.elements;if(!root.__on)root.removeEventListeners()}/**
...
if(parentRect.top+tooltipTop+elementRect.height>window.innerHeight){element.style.bottom=parentRect.height-targetRect.top+offset+"px";element.style.top="auto"}else{element.style.top=Math.max(-parentRect.top,tooltipTop)+"px";element.style.bottom="auto"}}else{element.style.left=tooltipLeft+"px";element.style.top=tooltipTop+"px"}}/**
   ...
   */disconnectedCallback(){this.removeEventListeners();super.disconnectedCallback()}}_exports.AbsolutePositionStateManager=AbsolutePositionStateManager;window.customElements.define(AbsolutePositionStateManager.tag,AbsolutePositionStateManager)});