...
styleLeft=elLeft+"px";styleTop=elTop+"px";styleRight="unset";styleBottom="unset";if(el.fitToVisibleBounds){/// Clip the left/right side
if(parentRect.left+elLeft+elRect.width>window.innerWidth){el.style.right="0px";el.style.left="auto"}else{el.style.left=Math.max(0,elLeft)+"px";el.style.right="auto"}// Clip the top/bottom side.
if(parentRect.top+elTop+elRect.height>window.innerHeight){el.style.bottom=parentRect.height-targetRect.top+offset+"px";el.style.top="auto"}else{el.style.top=Math.max(-parentRect.top,elTop)+"px";el.style.bottom="auto"}}else{}el.style.left=styleLeft;el.style.top=styleTop;el.style.right=styleRight;el.style.bottom=styleBottom;//provide positions for el and target (in case furthor positioning adjustments are needed)
el.__positions={self:elRect,target:targetRect}}/**
