define(["exports","../../../@polymer/polymer/polymer-element.js","./rich-text-editor-button.js","../../es-global-bridge/es-global-bridge.js","../../../@polymer/polymer/lib/utils/resolve-url.js","../../simple-picker/simple-picker.js"],function(_exports,_polymerElement,_richTextEditorButton,_esGlobalBridge,_resolveUrl,_simplePicker){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0/* transition */ /* transition */});_exports.RichTextEditorPicker=void 0;/**
 ...
 * @customElement
 * @polymer
 */class RichTextEditorPicker extends _richTextEditorButton.RichTextEditorButton{// render function
...
static get properties(){return{/**
   ...
   *
   */_isToggled(selection){let toggled=!1;if(null!==selection){if("formatBlock"===this.command){let ancestor=selection.commonAncestorContainer,parent=ancestor.parentNode,temp=[];this.options.forEach(function(row){row.forEach(function(option){temp.push(option.value)})});this.$.button.value=null!==parent.closest(temp.join(","))?parent.closest(temp.join(",")).tagName.toLowerCase():null}}return!1}/**
   ...
   */_getPickerOptions(options=[],allowNull=!1,icon=null){let items=!1===allowNull&&null===icon?[{alt:"null",icon:icon,value:null}]:[],cols=11>Math.sqrt(options.length)?Math.ceil(Math.sqrt(options.length)):10;for(let i=0;i<options.length;i++){let row=Math.floor(i/cols),col=i-row*cols,data=this._getOptionData(options[i]);if(items[row]===void 0||null===items[row])items[row]=[];if(0===row&&!1===allowNull&&null!==icon){items[0][0]={alt:"null",icon:icon,value:null};col++}items[row][col]=data}console.log("items",items);return items}}_exports.RichTextEditorPicker=RichTextEditorPicker;window.customElements.define(RichTextEditorPicker.tag,RichTextEditorPicker)});