...
static get properties(){return{/**
   ...
   */_getPickerOptions(options=[],allowNull=!1,icon=null){let items=!1===allowNull&&null===icon?[{alt:"null",icon:icon,value:null}]:[],cols=11>Math.sqrt(options.length)?Math.ceil(Math.sqrt(options.length)):10;for(let i=0;i<options.length;i++){let row=Math.floor(i/cols),col=i-row*cols,data=this._getOptionData(options[i]);if(items[row]===void 0||null===items[row])items[row]=[];if(0===row&&!1===allowNull&&null!==icon){items[0][0]={alt:"null",icon:icon,value:null};col++}items[row][col]=data}return items}}_exports.RichTextEditorPicker=RichTextEditorPicker;window.customElements.define(RichTextEditorPicker.tag,RichTextEditorPicker)});