 ...
 */window.MaterialProgressBehaviorImpl={properties:{/**
     ...
     */animated:{type:Boolean,value:!1,reflectToAttribute:!0/* transition */ /* transition */},/**
     ...
     */bars:{type:Array,value:function(){return[]},notify:!0,readOnly:!0},/**
     ...
     */_legendItems:{type:Array,value:function(){return[]},readOnly:!0},/**
   ...
   * Refresh the element.
   */refresh:function(){if(this._attached){// Reading and computing fundamental data
...
if(!this._oldBarHeight||this._oldBarHeight!==this.barHeight){this.customStyle["--material-progress-bar-height"]=this.barHeight+"px";this.updateStyles();this._oldBarHeight=this.barHeight}this._initialized=foundOneValue}},ready:function(){// Setting up the mutation observer
...
return this._mutationIsChildList(mutation,this.$.barsContainer)||this._mutationIsChildAttributes(mutation,this.$.barsContainer)}},attached:function(){this._attached=!0;this._refresh()},_refresh:function(){this.debounce("refresh",this.refresh,10)},_computeSumAndBars:function(){var sum=0,value=0,withValueCount=0,max=0,validBars=[],allChildren=(0,_polymerDom.dom)(this.$.content).getDistributedNodes();if(allChildren){allChildren.forEach(function(child){if(child.classList&&child.classList.contains("bar")&&child.hasAttribute("data-value")){validBars.push(child);value=this._getBarMeta(child).value;sum+=value;withValueCount+=0<value?1:0;max=Math.max(max,value)}},this)}this._setBars(validBars);this._setSum(sum);this._maxBarValue=max;this._barWithValueCount=withValueCount;return sum},_getBarMeta:function(bar){var meta={value:0,legend:void 0},val;if(bar&&bar.getAttribute){val=+bar.getAttribute("data-value");meta.value=isNaN(val)?0:Math.max(0,val);meta.legend=bar.getAttribute("data-legend");meta.legendAlwaysVisible=bar.hasAttribute("data-legend-always-visible")}return meta},_getWidthForBar:function(barValue,barValuesSum,maxBarValue,barHeight){// To implement
return"0px"},_playFirstAnimation:function(node,index,animationDelay){this.toggleClass("entry",!0,node);(node,index,animationDelay)=>{async.microTask.run(()=>{this.toggleClass("entry",!1,node)},500+animationDelay*index)}},_computeLegendNeeded:function(legendItems){return legendItems&&0<legendItems.length}};/** @polymerBehavior */window.MaterialProgressBehavior=[window.MaterialProgressBehaviorImpl]});