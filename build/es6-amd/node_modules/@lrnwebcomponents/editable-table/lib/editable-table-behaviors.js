...
*/window.editableTableBehaviors=window.editableTableBehaviors||{};editableTableBehaviors.displayBehaviors={properties:{/**
   ...
   * Return table data
   */getData:function(){let data={accentColor:this.accentColor,bordered:!this.hideBordered?this.bordered:null,caption:this.caption,columnHeader:this.columnHeader,condensed:!this.hideCondensed?this.condensed:null,dark:!this.hideDark?this.dark:null,data:this.data,filter:!this.hideFilter?this.filter:null,footer:this.footer,rowHeader:this.rowHeader,scroll:!this.hideScroll?this.scroll:null,sort:!this.hideSort?this.sort:null,striped:!this.hideStriped?this.striped:null,summary:this.summary};return data}};window.editableTableBehaviors.cellBehaviors={/**
   * Get the row or column label
   */_getLabel:function(index,type){if("Column"===type){let numerals="ABCDEFGHIJKLMNOPQRSTUVWXYZ".split(""),results=this._getLetter(index).split("-").reverse(),label="";for(let i=0;i<results.length;i++){if(""!==results[i])label+=numerals[results[i]]}return label}else{return index+1}},/**
   * Get the row or column label
   */_getLetter:function(index){let place=Math.floor(index/26),multiplier=26*place,remainder=index-multiplier,letters="";letters+=remainder+"-";if(0<place&&26>place){letters+=place-1+"-"}else if(26<=place){letters+=this._getLetter(place-1)}return letters}};window.editableTableBehaviors.editBehaviors={properties:{/**
