define(["../../../@polymer/polymer/polymer-legacy.js","../../dropdown-select/dropdown-select.js","../../../@polymer/paper-item/paper-item.js","../../simple-colors/simple-colors.js","../../responsive-utility/lib/responsive-utility-behaviors.js","./editable-table-behaviors.js","./editable-table-sort.js","./editable-table-filter.js","./editable-table-styles.js"],function(_polymerLegacy,_dropdownSelect,_paperItem,_simpleColors,_responsiveUtilityBehaviors,_editableTableBehaviors,_editableTableSort,_editableTableFilter,_editableTableStyles){"use strict";/**
...
*/(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
  ...
  `,is:"editable-table-display",listeners:{"change-sort-mode":"_changeSortMode","toggle-filter":"toggleFilter","dropdown-select-changed":"_onColumnChange"},behaviors:[ResponsiveUtilityBehaviors,editableTableBehaviors.displayBehaviors],properties:{/**
   ...
   */_getTbody:function(data,columnHeader,footer){if(data!==void 0&&null!==data&&0<data.length){let ch=columnHeader?1:0,tbody;if(footer){tbody=data.slice(ch,data.length-1);this.__tfoot=data.slice(data.length-1)}else{tbody=data.slice(ch,data.length);this.__tfoot=[]}return tbody}return[]},/**
   * Get the columns in <thead>
   */_getThead:function(data,columnHeader){let root=this;if(data!==void 0&&null!==data&&0<data.length&&columnHeader){return data.slice(0,1)}return[]},/**
   * sets a column's cells to filtered when in filtered mode so that filter can toggle
   */_isFiltered:function(column,filterColumn,filtered){return null!==filterColumn&&filterColumn===column&&filtered},/**
   * sets a cell's numeric style
   */_isNegative:function(cell){return this._isNumeric(cell)&&0===cell.trim().indexOf("-")},/**
   * sets a cell's numeric style
   */_isNumeric:function(cell){return null!==cell&&!isNaN(cell.trim().replace(/\$/g,""))},/**
   * sets a cell's numeric style
   */_isNumericColumn:function(col){let numeric=!0;for(let i=0;i<this.tbody.length;i++){if(!this._isNumeric(this.tbody[i][col]))numeric=!1}return numeric},/**
   * Calculate if the cell is a th or td
   */_isRowHeader:function(rowHeader,index){return 0===index&&rowHeader},/**
   * Handle column dropdown-select change
   */_onColumnChange:function(e){this.selected=e.detail.value;this._updateCols(parseInt(e.detail.value))},/**
   * Handle sort button click
   */_changeSortMode:function(e){if(this.sortColumn===e.detail.columnNumber&&"asc"===this.sortMode){this.sortMode="desc"}else if(this.sortColumn===e.detail.columnNumber&&"desc"===this.sortMode){this.sortMode="none"}else{this.sortMode="asc";this.sortColumn=e.detail.columnNumber}e.detail.setSortMode(this.sortMode);this.sortData(this.sortMode,e.detail.columnNumber)},/**
   * update the responsive columns menu
   */_updateCols:function(selected){this.$.table.removeAttribute("default-xs-display");let cols=this.$.table.querySelectorAll("th,td");this.$.table.setAttribute("transition",!0);setTimeout(function(){for(let i=0;i<cols.length;i++){let col=cols[i],index=col.cellIndex,delay;if(0===index||index===selected){col.removeAttribute("xs-hidden")}else{col.setAttribute("xs-hidden",!0)}}},200);this.$.table.removeAttribute("transition")},/**
   * Handle filter based on collumn and text of cell that is clicked
   */filterRows:function(filterColumn,filterText){if(filterText!==void 0&&null!==filterText){return function(tr){return tr[filterColumn].toLowerCase().trim()===filterText.toLowerCase().trim()}}else{return null}},/**
   * initialize the responsive columns menu
   */sortData:function(type,column){if("none"!==type&&!1!==type){let temp=this.tbody.slice();for(let i=0;i<temp.length;i++){temp[i].unshift(temp[i][column])}if("asc"===type){temp.sort()}else{temp.reverse()}for(let i=0;i<temp.length;i++){this.set("tbody."+i,[]);this.set("tbody."+i,temp[i].slice(1))}}else{let temp=this.tbody.slice();for(let i=0;i<temp.length;i++){this.set("data."+(i+1),[]);this.set("data."+(i+1),temp[i].slice())}}},/**
   * Handle filter button click
   */toggleFilter:function(e){if(e===void 0||this.filterColumn==e.detail.columnNumber&&this.filtered){this.filtered=!1;this.filterText=null;this.filterColumn=null}else{this.filterText=e.detail.text;this.filterColumn=e.detail.columnNumber;this.filtered=!0}}})});