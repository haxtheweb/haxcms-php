...
*/(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
   ...
   * click detection
   */ready:function(){this.onclick=function(e){let el=null!==e.srcElement&&null!==e.srcElement.tagName&&"td"===e.srcElement.tagName.toLowerCase()?e.srcElement:!1;if(el&&null!==el.getElementsByTagName("editable-table-cell")){console.log(el.children[0]);el.children[0].focus()}}},/**
   * Get the current accent color
   */_getAccentSelected:function(accentColor){return null!==accentColor?accentColor:"none"},/**
   * Gets the row data for a given row index
   */_getCurrentRow:function(index,data){let row=null;if(data!==void 0&&null!==data&&data[index]!==void 0&&null!==data[index]){row=data[index]}return row},/**
   * Handles delete column event
   */_handleDeleteColumn:function(e){this.deleteColumn(e.detail)},/**
   * Handles delete row event
   */_handleDeleteRow:function(e){this.deleteRow(e.detail)},/**
   * Handles insert column event
   */_handleInsertColumn:function(e){this.insertColumn(e.detail)},/**
   * Handles insert row event
   */_handleInsertRow:function(e){this.insertRow(e.detail)},/**
   * Tests for first row of data. Workaround to restamp column headers.
   */_isFirstRow:function(index){return 0===index},/**
   * Handle accent dropdown-select change
   */_onAccentChange:function(e){this.accentColor="none"!==e.detail.value?e.detail.value:null},/**
   * Move the focus/cursor to the correct cell when navigation keys are pressed
   */_onCellMove:function(e){let dir=e.detail.direction,cell=e.detail.cell,row=cell.parentNode,body=this.$.tbody,x=Array.prototype.indexOf.call(row.children,cell),y=Array.prototype.indexOf.call(body.children,row);if("down"===dir){if(y+1<body.children.length-1){body.children[y+1].children[x].children[0].setFocus()}else{this.insertRow(y)}}else if("up"===dir){if(0<y){body.children[y-1].children[x].children[0].setFocus()}}else if("right"===dir){if(x+1<row.children.length-1){row.children[x+1].children[0].setFocus()}else if(y+1<body.children.length-1){body.children[y+1].children[1].children[0].setFocus()}}else if("left"===dir){if(1<x){row.children[x-1].children[0].setFocus()}else if(0<y){body.children[y-2].children[body.children[y-2].children.length-2].children[0].setFocus()}}},/**
   * Updates data when cell value changes
   */_onCellValueChange:function(e){this.set("data."+e.detail.row+"."+e.detail.column,e.detail.value)},/**
   * Updates table properties when setting changes
   */_onTableSettingChange:function(e){this[e.detail.prop]=e.detail.value},/**
   * Makes sure there is always on cell to work from
   */_setMinimumData:function(data){if(1>data.length||1>data[0].length){this.set("data",[["","",""],["","",""],["","",""]])}},/**
   * Are all of the table style choices hidden?
   */_tableStylesHidden:function(hideBordered,hideCondensed,hideStriped,hideScroll){return hideBordered&&hideCondensed&&hideStriped&&hideScroll},/**
   * Are all of the theme choices hidden?
   */_tableThemeHidden:function(hideAccentColor,hideDarkTheme){return hideAccentColor&&hideDarkTheme},/**
   * Are all of the theme choices hidden?
   */_tableSortHidden:function(hideSort,hideFilter){return hideSort&&hideFilter},/**
   * Delete a column at the given index
   */deleteColumn:function(index){if(confirm("Delete entire column?")){for(let i=0;i<this.data.length;i++){this.splice("data."+i,index,1)}}},/**
   * Delete a row at the given index
   */deleteRow:function(index){if(confirm("Delete entire row?")){this.splice("data",index,1)}},/**
   * Insert a column at the given index
   */insertColumn:function(index){let temp=[],first=null;for(let i=0;i<this.data.length;i++){this.splice("data."+i,index,0,"")}},/**
   * Insert a row at the given index
   */insertRow:function(index){let temp=[];for(let i=0;i<this.data[0].length;i++){temp.push("")}this.splice("data",index+1,0,temp)}})});