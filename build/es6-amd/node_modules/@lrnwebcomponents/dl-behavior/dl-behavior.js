...
const link=document.createElement("a");link.href=(window.URL||window.webkitURL).createObjectURL(blob);link.download=filename;link.target=newTab?"_blank":"_self";(0,_polymerDom.dom)(this.root).appendChild(link);link.click();(0,_polymerDom.dom)(this.root).removeChild(link)}}/**
 ...
 */_exports.MtzFileDownloadBehaviors=MtzFileDownloadBehaviors;window.mtz=window.mtz||{};mtz.FileDownloadBehaviors={properties:{/* MIME type lookup for file extensions */fileTypes:{type:Object,value(){return{CSV:"text/csv",JSON:"text/json",PDF:"application/pdf",TXT:"text/plain"}}}},/**
   * Converts the data to a blob then uses navigator to save blob if it’s available, otherwise
   * creates an <a> with [download] attribute then simulates a click.
   * @param {String} data - data to encode.
   * @param {String} type - type of file to generate (i.e, JSON or CSV).
