...
this._feature.on("click",function(){if(this.href){window.open(this.href)}},this)}}},detached:function(){this._map.removeLayer(this._feature);delete this._feature},_coordsToArray:function(containerPoints){// returns an array of arrays of coordinate pairs coordsToArray("1,2,3,4") -> [[1,2],[3,4]]
for(var i=1,points=[],coords=containerPoints.split(",");i<coords.length;i+=2){points.push([parseInt(coords[i-1]),parseInt(coords[i])])}return points},_pointsToLatLngs:function(points){// points should be an array of nested container coordinates [[x1,y1],[x2,y2](,[xN,yN])]
var latLngArray=[];if(this._map){for(var i=0,map=this._map;i<points.length;i++){latLngArray.push(map.containerPointToLatLng(points[i]))}}return latLngArray},_styleToPathOptions:function(style){var options={};if("none"!==style.stroke){options.stroke=!0;options.color=style.stroke;options.opacity=style.strokeOpacity;options.weight=parseInt(style.strokeWidth);options.dashArray=style.strokeDasharray;options.lineCap=style.strokeLinecap;options.lineJoin=style.strokeLinejoin}else{options.stroke=!1}if("none"!==style.fill){options.fill=!0;options.fillColor=style.fill;options.fillOpacity=style.fillOpacity;options.fillRule=style.fillRule}else{options.fill=!1}return options}})});