define(["../../../@polymer/polymer/polymer-legacy.js","../../../@polymer/polymer/lib/legacy/polymer.dom.js","./map-styles.js"],function(_polymerLegacy,_polymerDom,_mapStyles){"use strict";/* styles scoped to inside a custom element must be in a style module */(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
    ...
    <slot></slot>
  `,is:"layer-",properties:{src:{type:String,reflectToAttribute:!0/* transition */ /* transition */,observer:"_srcChanged"},label:{type:String,value:"Layer",reflectToAttribute:!0,observer:"_labelChanged"},// changing checked turns the layer on or off, and reflects that in the
...
value:function(){return[]}}},_srcChanged:function(){// this should only be invoked after the ready callback
if(this._layer){this._layer.initialize(this.src,this)}},_labelChanged:function(){var map=this.parentElement&&this.parentElement._map;if(map){this._toggleHidden(!1)}},_apiToggleChecked:!0,_toggleChecked:function(){var map=this.parentElement&&this.parentElement._map;if(map){if(this._apiToggleChecked){if(map.hasLayer(this._layer)){map.removeLayer(this._layer)}else{map.addLayer(this._layer)}this._validateDisabled()}this._apiToggleChecked=!0}},_onLayerExtentLoad:function(e){// the mapml document associated to this layer can in theory contain many
...
if(this._layer._map){this._layer.fire("attached",this._layer)}},_validateDisabled:function(){var layer=this._layer,map=layer._map;if(map){var zoomBounds=layer.getZoomBounds(),zoom=map.getZoom(),withinZoomBounds=zoomBounds&&zoomBounds.min<=zoom&&zoom<=zoomBounds.max,projectionMatches=layer._projectionMatches(map),visible=projectionMatches&&withinZoomBounds&&map.getPixelBounds().intersects(layer.getLayerExtentBounds(map));this.disabled=!visible}},_onLayerChange:function(){if(this._layer._map){// can't disable observers, have to set a flag telling it where
// the 'event' comes from: either the api or a user click/tap
this._apiToggleChecked=!1;this.checked=this._layer._map.hasLayer(this._layer)}},_toggleHidden:function(hide){var map=this.parentElement&&this.parentElement._map;if(map&&this.parentElement.controls){if(hide){this.parentElement._layerControl.removeLayer(this._layer)}else{this._layerControl=this.parentElement._layerControl;this._layerControl.addOrUpdateOverlay(this._layer,this.label)}this._validateDisabled()}},detached:function(){// if the map-layer node is removed from the dom, the layer should be
// removed from the map and the layer control
if(this._layer._map){this._layer._map.removeLayer(this._layer)}if(this._layerControl&&!this.hidden){this._layerControl.removeLayer(this._layer)}this._removeEvents()},ready:function(){// the layer might not be attached to a map
...
this._layer.fire("attached");if(this.checked){this._layer.addTo(this._layer._map)}// add the handler which toggles the 'checked' property based on the
...
// See issue #26
this._layer._map.fire("moveend")},attached:function(){if("MAP"!==(0,_polymerDom.dom)(this).parentNode.nodeName){console.log("ERROR: "+this.localName+"#"+this.id+" must be a child of a map element");return}// if the map has been attached, set this layer up wrt Leaflet map
if((0,_polymerDom.dom)(this).parentNode._map){this._attachedToMap()}},_removeEvents:function(){if(this._layer){this._layer.off("loadstart",!1,this)}},_setUpEvents:function(){this._layer.on("loadstart",function(){this.fire("loadstart",{target:this})},this)}})});