...
*/let GridPlate=(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
    <custom-style>
    ...
    ></iron-a11y-keys>
  `,is:"grid-plate",listeners:{focusin:"_focusIn",focusout:"_focusOut"},behaviors:[HAXBehaviors.PropertiesBehaviors],properties:{/**
     ...
     * For example:```
{
   ...
   */cancelActive:function(e){this.__activeItem=null},/**
   ...
   */canMoveSlot:function(item,before){let dir=before?-1:1,max=this.shadowRoot.querySelectorAll(".column").length,col=item.getAttribute("slot").split("-"),dest=parseInt(col[1])+dir;return 1<=dest&&dest<=max},/**
   * Moves an item a set number of slots.
   *
   * @param {object} the item
   * @param {number} -1 for left or +1 for right
   */moveSlot:function(item,before){let dir=before?-1:1,col=item.getAttribute("slot").split("-"),dest=parseInt(col[1])+dir;if(this.canMoveSlot(item,dir)){item.setAttribute("slot","col-"+dest)}},/**
   * Determines if the item can move a set number of slots.
   *
   * @param {object} the item
   * @param {boolean} move item before previous? (false for move item after next)
   * @returns {boolean} if the item can move a set number of slots
   */canMoveOrder:function(item,before){let target=before?item.previousElementSibling:item.nextElementSibling;return null!==target&&target.getAttribute("slot")===item.getAttribute("slot")},/**
   * Moves an item's order within a slot.
   *
   * @param {object} the item
   * @param {boolean} move item before previous? (false for move item after next)
   */moveOrder:function(item,before=!0){let dir=before?-1:1;if(this.canMoveOrder(item,before)){if(before){(0,_polymerDom.dom)(this).insertBefore(this.__activeItem,this.__activeItem.previousElementSibling)}else{(0,_polymerDom.dom)(this).insertBefore(this.__activeItem.nextElementSibling,this.__activeItem)}}},/**
   * Move the active element based on which button got pressed.
   */moveActiveElement:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// see if this was an up down left or right movement
...
setTimeout(()=>{if(this.__activeItem&&"function"===typeof this.__activeItem.focus){this.positionArrows(this.__activeItem);this.__activeItem.focus()}},100)},/**
   ...
   */_activeItemChanged:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&null!=newValue){// position arrows
...
if(typeof oldValue!==typeof void 0&&null!=oldValue){oldValue.classList.remove("active-item");oldValue.blur()}},/**
   ...
   */setActiveElement:function(e){this.$.right.focus();e.preventDefault();e.stopPropagation()},/**
...
return layouts["1-1"][size]}},/**
   ...
   */_focusIn:function(e){if(this.editMode){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// only activate if we touch something that's in the slot of the grid plate
if((0,_polymerDom.dom)(local).parentNode===this){this.__activeItem=local}}},/**
   * Focus / tab / click event normalization
   */_focusOut:function(e){if(this.editMode){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// @todo need to correctly de-focus when the element loses focus entirely
...
this.$.up.style.top=topOffset-20+"px";this.$.down.style.top=topOffset+elemRect.height+"px";this.$.left.style.top=topOffset+elemRect.height/2+"px";this.$.right.style.top=topOffset+elemRect.height/2+"px";this.$.up.style.left=leftOffset+elemRect.width/2-10+"px";this.$.down.style.left=leftOffset+elemRect.width/2-10+"px";this.$.left.style.left=leftOffset-20+"px";this.$.right.style.left=leftOffset+elemRect.width+"px"}},/**
   ...
   */_editModeChanged:function(newValue,oldValue){// flipping from false to true
...
for(var i in children){if(typeof children[i].tagName!==typeof void 0){children[i].removeEventListener("drop",this.dropEvent.bind(this));children[i].removeEventListener("dragstart",this.dragStart.bind(this));children[i].removeEventListener("dragend",this.dragEnd.bind(this));children[i].removeEventListener("dragover",function(e){e.preventDefault()});children[i].removeAttribute("draggable");children[i].removeAttribute("data-draggable");children[i].removeAttribute("tabindex")}}async.microTask.run(()=>{for(var j=1;j<=this.columns.length;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).removeEventListener("drop",this.dropEvent.bind(this));this.shadowRoot.querySelector("#col"+j).removeEventListener("dragstart",this.dragStart.bind(this));this.shadowRoot.querySelector("#col"+j).removeEventListener("dragend",this.dragEnd.bind(this));this.shadowRoot.querySelector("#col"+j).removeEventListener("dragover",function(e){e.preventDefault()});this.shadowRoot.querySelector("#col"+j).removeAttribute("data-draggable")}}})}}},/**
   ...
   */dropEvent:function(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// if we have a slot on what we dropped into then we need to mirror that item
...
setTimeout(()=>{if(this.__activeItem&&"function"===typeof this.__activeItem.focus){this.positionArrows(this.__activeItem);this.__activeItem.focus()}},100)},/**
   ...
   */dragStart:function(e){let children=(0,_polymerDom.dom)(this).children;// walk the children and apply the draggable state needed
for(var i in children){if(typeof children[i].classList!==typeof void 0){children[i].classList.add("mover")}}for(var j=1;j<=this.columns.length;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.add("mover")}}},/**
   ...
   */dragEnd:function(e){let children=(0,_polymerDom.dom)(this).children;// walk the children and apply the draggable state needed
for(var i in children){if(typeof children[i].classList!==typeof void 0){children[i].classList.remove("mover")}}for(var j=1;j<=this.columns.length;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.remove("mover")}}},/**
   ...
   */attached:function(){let root=this;// listen for HAX if it's around
...
setTimeout(()=>{this.editMode=!0;if(this.__activeItem&&"function"===typeof this.__activeItem.focus){this.positionArrows(this.__activeItem);this.__activeItem.focus()}},100)}},/**
   * Store updated, sync.
   */_haxStorePropertyUpdated:function(e){if(e.detail&&typeof e.detail.value!==typeof void 0&&e.detail.property){if("object"===typeof e.detail.value){this.set(e.detail.property,null)}this.set(e.detail.property,e.detail.value)}}});_exports.GridPlate=GridPlate});