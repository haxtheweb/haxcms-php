...
document.body.addEventListener("hax-store-property-updated",this._haxStorePropertyUpdated.bind(this));this.$.download.addEventListener("tap",this.download.bind(this));this.$.downloadfull.addEventListener("tap",this.downloadfull.bind(this));this.$.import.addEventListener("tap",this.importContent.bind(this));this.$.copy.addEventListener("tap",this.selectBody.bind(this));this.$.closedialog.addEventListener("tap",this.close.bind(this));this.$.elementexport.addEventListener("tap",this.htmlToHaxElements.bind(this))},/**
   ...
   * Download file.
   */downloadfull:function(e){const data=this.contentToFile(!0/* transition */ /* transition */);this.downloadFromData(data,"html","my-new-webpage");window.HaxStore.toast("Working offline copy downloaded")},/**
   * Download file.
   */importContent:function(e){// import contents of this text area into the activeHaxBody
const htmlBody=this.$.textarea.value;window.HaxStore.toast("Content updated");return window.HaxStore.instance.activeHaxBody.importContent(htmlBody)},/**
   * selectBody
   */selectBody:function(e){this.$.hiddentextarea.value=this.$.textarea.value;this.$.hiddentextarea.removeAttribute("hidden");this.$.hiddentextarea.focus();this.$.hiddentextarea.select();document.execCommand("copy");this.$.hiddentextarea.setAttribute("hidden","hidden");window.HaxStore.toast("Copied HTML content")},/**
   * HTML to HAX Elements
   */htmlToHaxElements:function(e){let elements=window.HaxStore.htmlToHaxElements(this.$.textarea.value);var str=JSON.stringify(elements,null,2);let val=this.$.textarea.value;this.$.hiddentextarea.removeAttribute("hidden");this.$.hiddentextarea.value=str;this.$.hiddentextarea.focus();this.$.hiddentextarea.select();document.execCommand("copy");this.$.hiddentextarea.value=val;this.$.hiddentextarea.setAttribute("hidden","hidden");window.HaxStore.toast("Copied hax elements to clipboard")},/**
...
content=`
   ...
   * Toggle ourselves.
   */toggleDialog:function(){if(this.$.dialog.opened){this.close()}else{this.$.textarea.editorValue=this.contentToFile(!1);window.HaxStore.instance.closeAllDrawers(this)}},/**
   * open the dialog
   */open:function(){this.$.dialog.open();this.$.buttons.style.display="none";this.$.loading.setAttribute("loading","loading");this.$.wrapper.appendChild(this.$.textarea);// silly but we need the code editor to figure itself out real quick as to sizing
setTimeout(()=>{this.$.loading.removeAttribute("loading");this.$.buttons.style.display="unset"},800)},/**
   * close the dialog
