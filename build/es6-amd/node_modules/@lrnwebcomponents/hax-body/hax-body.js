 ...
 */let HaxBody=(0,_polymerLegacy.Polymer)({is:"hax-body",_template:_polymerLegacy.html`
    ...
    <hax-input-mixer
      ...
      class="hax-context-menu ignore-activation"
    ></hax-input-mixer>
  `,listeners:{focusin:"_focusIn",mousedown:"_focusIn","hax-context-item-selected":"_haxContextOperation","hax-input-mixer-update":"_haxInputMixerOperation","place-holder-replace":"replacePlaceholder"},properties:{__self:{type:Object,value:void 0},/**
   ...
   */ready:function(){this.fire("hax-register-body",this);this.polyfillSafe=window.HaxStore.instance.computePolyfillSafe();// mutation observer that ensures state of hax applied correctly
...
node.setAttribute("data-editable",this.editMode);let haxRay=node.tagName.replace("-"," ").toLowerCase(),i=window.HaxStore.instance.gizmoList.findIndex(j=>j.tag===node.tagName.toLowerCase());if(-1!==i){haxRay=window.HaxStore.instance.gizmoList[i].title}node.setAttribute("data-hax-ray",haxRay);this.fire("hax-body-tag-added",{node:node})}})}// if we dropped nodes via the UI (delete event basically)
if(0<info.removedNodes.length){// handle removing items... not sure we need to do anything here
info.removedNodes.map(node=>{if(this._haxElementTest(node)&&!node.classList.contains("hax-active")){this.fire("hax-body-tag-removed",{node:node})}})}})})},/**
   * Attached to the DOM; now we can fire event to the store that
   * we exist and are the thing being edited.
   */attached:function(){(0,_renderStatus.afterNextRender)(this,function(){// try to normalize paragraph insert on enter
...
if(e.clipboardData||e.originalEvent.clipboardData){text=(e.originalEvent||e).clipboardData.getData("text/plain")}else if(window.clipboardData){text=window.clipboardData.getData("Text")}try{let range=window.HaxStore.getRange(),sel=window.HaxStore.getSelection(),newNode=document.createTextNode(text),newRange=document.createRange();if(range&&sel){range.deleteContents();range.insertNode(newNode);newRange.setStart(newNode,text.length);newRange.collapse(!0);sel.removeAllRanges();sel.addRange(newRange)}}catch(e){console.log(e)}}});document.body.removeEventListener("hax-store-property-updated",this._haxStorePropertyUpdated.bind(this));window.removeEventListener("scroll",this._keepContextVisible.bind(this))},/**
   ...
   */_keepContextVisible:function(e){// see if the text context menu is visible
...
else if(rng.commonAncestorContainer&&rng.commonAncestorContainer.parentNode&&this.activeNode!==rng.commonAncestorContainer.parentNode&&"function"===typeof rng.commonAncestorContainer.parentNode.focus){if("HAX-BODY"!==rng.commonAncestorContainer.parentNode.tagName){if(window.HaxStore.instance.isTextElement(rng.commonAncestorContainer.parentNode)){this.setAttribute("contenteditable",!0)}else{this.removeAttribute("contenteditable")}setTimeout(()=>{rng.commonAncestorContainer.parentNode.focus();this.__focusLogic(rng.commonAncestorContainer.parentNode)},900)}}break;}},100);if(this.$.platecontextmenu.classList.contains("hax-active-hover")){this.__dropActiveHover()}}},_onKeyPress:function(e){if(this.editMode&&this.$.platecontextmenu.classList.contains("hax-active-hover")){this.__dropActiveHover()}},/**
   * on mouse over then fire the hax ray value if we have one
   */hoverEvent:function(e){if(this.editMode){if(e.target&&null!=e.target.getAttribute("data-hax-ray")){this.__activeHover=e.target;this.fire("hax-active-hover-name",e.target.getAttribute("data-hax-ray"))}else if(e.target&&e.target.parentNode&&null!=e.target.parentNode.getAttribute("data-hax-ray")){this.__activeHover=e.target.parentNode;this.fire("hax-active-hover-name",e.target.parentNode.getAttribute("data-hax-ray"))}if(!this.$.platecontextmenu.classList.contains("hax-active-hover")){let normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// see if the target is relevent when showing the edit menu operations
...
p.focus();this.__focusLogic(p)},210)}else{this.replaceElementWorkflow()}},/**
   ...
   */replaceElementWorkflow:function(){let element=window.HaxStore.nodeToHaxElement(this.activeNode,null),type="*",skipPropMatch=!1;// special support for place holder which defines exactly
...
let tag=this.activeNode.tagName.toLowerCase(),humanName=tag.replace("-"," ");if(typeof window.HaxStore.instance.elementList[tag]!==typeof void 0&&!1!==window.HaxStore.instance.elementList[tag].gizmo){humanName=window.HaxStore.instance.elementList[tag].gizmo.title}window.HaxStore.instance.haxAppPicker.presentOptions(haxElements,"__convert",`Transform ${humanName} to..`,"gizmo")}else{window.HaxStore.toast("Sorry, this can not be transformed!",5e3)}},/**
   ...
   */_globalPreferencesUpdated:function(newValue,oldValue){if(typeof newValue!==typeof void 0&&null!=newValue){this.haxRayMode=newValue.haxRayMode}},/**
   * Store updated, sync.
   ...
   * Clear area.
   */haxClearBody:function(confirm=!0){let status=!0;// only confirm if asked so we can support wipes without it
...
if(status){(0,_haxutils.wipeSlot)(this)}},/**
   ...
   */haxInsert:function(tag,content,properties={},waitForLock=!0){var tags=window.HaxStore.instance.validTagList;this.__activeHover=null;// verify this tag is a valid one
...
if(waitForLock){setTimeout(()=>{this.breakUpdateLock()},150)}return!0}return!1},breakUpdateLock:function(){window.HaxStore.write("activeContainerNode",this.__updateLockFocus,this);window.HaxStore.write("activeNode",this.__updateLockFocus,this);// attempt to focus on the new node, may not always work
...
if("function"===typeof this.__updateLockFocus.scrollIntoViewIfNeeded){this.__updateLockFocus.scrollIntoViewIfNeeded(!0)}else{this.__updateLockFocus.scrollIntoView({behavior:"smooth",inline:"center"})}},/**
   ...
   */haxToContent:function(){this.hideContextMenus();var __active=this.activeNode;// null this to drop hax based classes
...
content=(0,_haxutils.encapScript)(content);if(this.globalPreferences.haxDeveloperMode){console.log(content)}return content},/**
   ...
   */haxDuplicateNode:function(node,parent=this){// move the context menu before duplicating!!!!
this.hideContextMenus();var nodeClone=(0,_polymerDom.dom)(node).cloneNode(!0);if("webview"===nodeClone.tagName.toLowerCase()&&window.HaxStore.instance._isSandboxed&&typeof nodeClone.guestinstance!==typeof void 0){delete nodeClone.guestinstance}// shouldn't be possible but might as well check
...
if(parent===this){window.HaxStore.write("activeContainerNode",nodeClone,this)}window.HaxStore.write("activeNode",nodeClone,this)},50);return!0},/**
   ...
   */hideContextMenus:function(){// primary context menus
...
this.$.textcontextmenu.highlightOps=!1},/**
   ...
   */positionContextMenus:function(node,container){let tag=node.tagName.toLowerCase();if(window.HaxStore.instance._isSandboxed&&"webview"===tag){tag="iframe"}let props=window.HaxStore.instance.elementList[tag];// try and work against anything NOT a P tag
...
if(container&&!this._HTMLPrimativeTest(node)&&node!==container){container.contentEditable=!1}else if(container&&this._HTMLPrimativeTest(container)){container.contentEditable=!0}},/**
   ...
   */haxMoveGridPlate:function(direction,node,container){// menu is actually in the element for render purposes
...
if(null!==container.previousElementSibling){(0,_polymerDom.dom)(this).insertBefore(container,container.previousElementSibling)}break;case"down":if(null!==container.nextElementSibling){(0,_polymerDom.dom)(this).insertBefore(container.nextElementSibling,container)}break;case"last":if(null!==container.nextElementSibling){(0,_polymerDom.dom)(this).appendChild(container)}break;// @todo support other directions for when inside of an element
   ...
   */haxReplaceNode:function(node,replacement,parent=this){// ensure we're not in the document tree for the replaced node
...
if(null!=node.getAttribute("slot")){replacement.setAttribute("slot",node.getAttribute("slot"))}(0,_polymerDom.dom)(parent).replaceChild(replacement,node)}catch(e){console.log(e)}return replacement},/**
   * Convert an element from one tag to another.
   */haxChangeTagName:function(node,tagName,newNode){// ensure we're not in the document tree for the replaced node
...
if(children[0]&&children.tagName){children[0].focus()}else{replacement.focus()}},50);return replacement},/**
   ...
   */haxDeleteNode:function(node,parent=this){// move the context menu before deleting!!!!
...
try{return(0,_polymerDom.dom)(parent).removeChild(node)}catch(e){console.log(e)}},/**
   ...
   * internal whitelist.
   */importContent:function(html,clear=!0){// kill the slot of the active body, all of it
...
fragment.removeChild(fragment.firstChild)}}},50)},/**
   ...
   */_haxContextOperation:function(e){let detail=e.detail;var haxElement;// support a simple insert event to bubble up or everything else
...
setTimeout(()=>{window.HaxStore.instance.haxManager.$.preview.$.configurebutton.focus()},100);break;}},/**
   ...
   */_haxInputMixerOperation:function(e){// this is a big deal how simple this part is in the end
...
this._hideContextMenu(this.$.haxinputmixer)},/**
   ...
   */_focusIn:function(e){var normalizedEvent=(0,_polymerDom.dom)(e);if(this.__focusLogic(normalizedEvent.localTarget)){e.stopPropagation()}},/**
   ...
   */__focusLogic:function(target){let stopProp=!1;// only worry about these when we are in edit mode
...
if(this.activeNode!==activeNode&&tags.includes(containerNode.tagName.toLowerCase())&&!activeNode.classList.contains("ignore-activation")){this.activeNode=activeNode;window.HaxStore.write("activeNode",activeNode,this);this.positionContextMenus(activeNode,containerNode);stopProp=!0}}}else{this.__tabTrap=!1}return stopProp},/**
   ...
   */_editModeChanged:function(newValue,oldValue){// fire above that we have changed states so things can react if needed
...
if(!1===newValue){this.removeAttribute("contenteditable");this.hideContextMenus()}},/**
   ...
   */_haxResolvePreviousElement:function(node){node=(0,_polymerDom.dom)(node).previousElementSibling;while(null!=node&&typeof node.tagName!==typeof void 0&&"HAX-"===node.tagName.substring(0,4)){node=(0,_polymerDom.dom)(node).previousElementSibling}return node},/**
   * Test if this is a HAX element or not
   */_haxElementTest:function(node){if(typeof node.tagName!==typeof void 0&&"HAX-"!==node.tagName.substring(0,4)){return!0}return!1},/**
   * Test if this is an HTML primative
   */_HTMLPrimativeTest:function(node){if(null!=node&&typeof node.tagName!==typeof void 0&&-1==node.tagName.indexOf("-")){return!0}return!1},/**
   * Walk everything we find and either enable or disable editable state.
   */_applyContentEditable:function(status,target=this.$.body){let children=(0,_polymerDom.dom)(target).getDistributedNodes();// fallback for content nodes if not polymer managed nodes above
...
if("grid-plate"===tag){newValue.editMode=this.editMode;this._applyContentEditable(this.editMode,newValue)}}},/**
   ...
   */_activeNodeChanged:function(newValue,oldValue){// clean up the older one
...
setTimeout(()=>{this.positionContextMenus(newValue,window.HaxStore.instance.activeContainerNode)},100);if("left"==newValue.style.textAlign){this.$.textcontextmenu.justifyIcon="editor:format-align-left";this.$.textcontextmenu.justifyValue="text-align-left"}else if("left"==newValue.style.float){this.$.cecontextmenu.justifyIcon="editor:format-align-left";this.$.cecontextmenu.justifyValue="hax-align-left"}else if("0 auto"==newValue.style.margin){this.$.cecontextmenu.justifyIcon="editor:format-align-center";this.$.cecontextmenu.justifyValue="hax-align-center"}}// just hide menus if we don't have an active item
   ...
   */_positionContextMenu:function(menu,target,xoffset,yoffset){// make it account for the offset if it's floated over to one side
...
if(this.__activeHover){menu.classList.add("hax-active-hover");menu.style.marginLeft="";this.__typeLock=!1}setTimeout(()=>{async.microTask.run(this._keepContextVisible())},100)},/**
   ...
   */_hideContextMenu:function(menu){menu.classList.remove("hax-context-visible","hax-context-pin-top","hax-context-pin-bottom");menu.style.left="-100px"},/**
   * Find the next thing to tab forward to.
   */_tabKeyPressed:function(){let focus=!1,node=this.activeContainerNode;const activeNodeTagName=this.activeContainerNode.tagName;// try selection / tab block since range can cause issues
...
if(null==(0,_polymerDom.dom)(node).nextSibling){focus=!0}else if("function"===(0,_polymerDom.dom)(node).nextSibling.focus){(0,_polymerDom.dom)(node).nextSibling.focus();focus=!0}else{node=(0,_polymerDom.dom)(node).nextSibling}}}}catch(e){console.log(e)}},/**
   ...
   */_tabBackKeyPressed:function(){let node=this.activeContainerNode;const activeNodeTagName=this.activeContainerNode.tagName;// try selection / tab block since range can cause issues
...
while(null!=node&&!this._haxElementTest(node)){node=(0,_polymerDom.dom)(node).previousSibling}}if(null!=node){setTimeout(()=>{node.focus()},50)}}}catch(e){console.log(e)}}});_exports.HaxBody=HaxBody});