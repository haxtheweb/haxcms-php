define(["exports","meta","require","../../@polymer/polymer/polymer-element.js","../../@polymer/polymer/lib/utils/render-status.js","../../@polymer/polymer/lib/utils/resolve-url.js","../es-global-bridge/es-global-bridge.js","../../@polymer/polymer/lib/elements/dom-if.js"],function(_exports,meta,_require,_polymerElement,_renderStatus,_resolveUrl,_esGlobalBridge,_domIf){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.ImgPanZoom=void 0;meta=babelHelpers.interopRequireWildcard(meta);_require=babelHelpers.interopRequireWildcard(_require);/**
...
visibilityRatio:{type:Number,value:1}}}/**
   * life cycle
   */constructor(){super();new Promise((res,rej)=>_require.default(["../../@polymer/paper-spinner/paper-spinner.js"],res,rej));new Promise((res,rej)=>_require.default(["./lib/img-loader.js"],res,rej));const basePath=(0,_resolveUrl.pathFromUrl)(decodeURIComponent(meta.url));let location=`${basePath}lib/openseadragon/build/openseadragon/openseadragon.js`;window.addEventListener("es-bridge-openseadragon-loaded",this._openseadragonLoaded.bind(this));window.ESGlobalBridge.requestAvailability();window.ESGlobalBridge.instance.load("openseadragon",location)}_openseadragonLoaded(){this.__openseadragonLoaded=!0;if(this.dzi){this._initOpenSeadragon()}}/**
   * life cycle
   */connectedCallback(){super.connectedCallback();this.animationConfig={fade:{name:"fade-in-animation",node:this.shadowRoot.querySelector("#viewer")}};(0,_renderStatus.afterNextRender)(this,function(){// Init openseadragon if we are using a deep zoom image
