...
constructor(){super();this.HAXCMSThemeWiring=new HAXCMSThemeWiring(this)}/**
...
let name=color.replace("-text",""),tmp=new _simpleColors.SimpleColors;if(tmp.colors[name]){return tmp.colors[name][6]}return"#000000"}/**
     ...
     */_editModeChanged(newValue){if(typeof newValue!==typeof void 0){_async.microTask.run(()=>{// trick browser into thinking we just reized
...
if(null===this.contentContainer){this.contentContainer=this.shadowRoot.querySelector("#contentcontainer")}this.__disposer=[];(0,_renderStatus.afterNextRender)(this,function(){// edge case, we just swapped theme faster then content loaded... lol
if(0===(0,_polymerDom.dom)(this).getEffectiveChildNodes().length){let frag=document.createRange().createContextualFragment(_haxcmsSiteStore.store.activeItemContent);(0,_polymerDom.dom)(this).appendChild(frag)}(0,_elementMixin.updateStyles)();// store disposer so we can clean up later
 ...
 */_exports.HAXCMSTheme=HAXCMSTheme;class HAXCMSThemeWiring{constructor(element,load=!0){if(load){document.body.addEventListener("haxcms-edit-mode-changed",this._globalEditChanged.bind(element));document.body.addEventListener("haxcms-active-item-changed",this._activeItemUpdate.bind(element));document.body.addEventListener("haxcms-trigger-update",this._triggerUpdate.bind(element));// @todo may want to set this to sessionStorage instead...
...
_haxcmsSiteStore.store.cmsSiteEditorAvailability(element,injector)}/**
   ...
   */disconnect(element){document.body.removeEventListener("haxcms-active-item-changed",this._activeItemUpdate.bind(element));document.body.removeEventListener("haxcms-edit-mode-changed",this._globalEditChanged.bind(element));document.body.removeEventListener("haxcms-trigger-update",this._triggerUpdate.bind(element))}/**
