...
new Promise((res,rej)=>_require.default(["@lrnwebcomponents/haxcms-elements/lib/core/haxcms-editor-builder.js"],res,rej)).then(response=>{this.editorBuilder=document.createElement("haxcms-editor-builder");// attach editor builder after we've appended to the screen
...
setTimeout(()=>{let frag=document.createRange().createContextualFragment(html);(0,_polymerDom.dom)(this.themeElement).appendChild(frag);this.dispatchEvent(new CustomEvent("json-outline-schema-active-body-changed",{bubbles:!0,composed:!0,cancelable:!1,detail:html}))},5);// if there are, dynamically import them
