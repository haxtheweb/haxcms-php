...
class SiteSearch extends r.LitElement{static get styles(){return[r.css`
      ...
      ${this.__results.map(e=>r.html`
        ...
        `)}
    `}_searchValueChanged(e){this.search=e.detail.value}__resultsChanged(e){e.detail.value?this.__results=[...e.detail.value]:this.__results=[]}static get properties(){return{dataSource:{type:String,attribute:"data-source"},search:{type:String},__results:{type:Array}}}firstUpdated(e){this.shadowRoot.querySelector("#lunr").addEventListener("results-changed",this.__resultsChanged.bind(this))}updated(e){e.forEach((e,t)=>{"search"==t&&(this._searchChanged(this[t],e),this.shadowRoot.querySelector("#lunr").search=this[t]),"dataSource"==t&&(this.shadowRoot.querySelector("#lunr").dataSource=this[t])})}_searchChanged(e,t){e.length>=3&&void 0===this.dataSource&&(this.dataSource="lunrSearchIndex.json")}}e.SiteSearch=SiteSearch,window.customElements.define(SiteSearch.tag,SiteSearch)}));