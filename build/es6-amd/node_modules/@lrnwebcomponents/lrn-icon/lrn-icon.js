...
*/class LrnIcon extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
    ...
    `}static get tag(){return"lrn-icon"}static get properties(){return{/**
       ...
       */src:{type:String},_meta:{value:document.createElement("iron-meta",{type:"iconset"})}}}static get observers(){return["_updateIcon(_meta, isAttached)","_updateIcon(theme, isAttached)","_srcChanged(src, isAttached)","_iconChanged(icon, isAttached)"]}_iconChanged(icon){this._iconName=icon;this._iconsetName="lrn";this._updateIcon()}_srcChanged(src){this._updateIcon()}_usesIconset(){return this.icon||!this.src}/** @suppress {visibility} */_updateIcon(){if(this._usesIconset()){if(this._img&&this._img.parentNode){(0,_polymerDom.dom)(this.root).removeChild(this._img)}if(""===this._iconName){if(this._iconset){this._iconset.removeIcon(this)}}else if(this._iconsetName&&this._meta){this._iconset=this._meta.byKey(this._iconsetName);if(this._iconset){this._iconset.applyIcon(this,this._iconName,this.theme);this.unlisten(window,"lrn-iconset-added","_updateIcon")}else{this.listen(window,"lrn-iconset-added","_updateIcon")}}}else{if(this._iconset){this._iconset.removeIcon(this)}if(!this._img){this._img=document.createElement("img");this._img.style.width="100%";this._img.style.height="100%";this._img.draggable=!1}this._img.src=this.src;(0,_polymerDom.dom)(this.root).appendChild(this._img)}}}_exports.LrnIcon=LrnIcon;window.customElements.define(LrnIcon.tag,LrnIcon)});