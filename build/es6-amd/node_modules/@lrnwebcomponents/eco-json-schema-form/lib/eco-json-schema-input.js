define(["../../../@polymer/polymer/polymer-legacy.js","../../../@polymer/iron-flex-layout/iron-flex-layout-classes.js","../../../@polymer/app-localize-behavior/app-localize-behavior.js","../../../@polymer/paper-input/paper-input.js","../../../@polymer/paper-styles/typography.js"],function(_polymerLegacy,_ironFlexLayoutClasses,_appLocalizeBehavior,_paperInput,_typography){"use strict";/**
...
*/(0,_polymerLegacy.Polymer)({is:"eco-json-schema-input",_template:_polymerLegacy.html`
    <custom-style>
    ...
    <paper-input
      ...
      auto-validate
    ></paper-input>
  `,behaviors:[_appLocalizeBehavior.AppLocalizeBehavior],properties:{language:{value:"en"},resources:{value:function(){return{}}},schema:{type:Object,observer:"_schemaChanged"},value:{type:String,notify:!0,value:function(){return""}},error:{type:String,observer:"_errorChanged",value:null}},ready:function(){},detached:function(){},_schemaChanged:function(){var schema=this.schema,inputEl=this.$.input;if(schema.required){inputEl.required=!0}if(this._isSchemaNumber(schema.type)){inputEl.type="number";if(schema.multipleOf){inputEl.step=schema.multipleOf}if(!isNaN(schema.maximum)){if(schema.exclusiveMaximum){inputEl.max=schema.maximum-(schema.multipleOf||1)}else{inputEl.max=schema.maximum}}if(!isNaN(schema.minimum)){if(schema.exclusiveMinimum){inputEl.min=schema.minimum+(schema.multipleOf||1)}else{inputEl.min=schema.minimum}}}if(this._isSchemaString(schema.type)){if("date-time"===schema.format){inputEl.type="datetime-local";inputEl.alwaysFloatLabel=!0;// label doesn't float when value not set
...
if(schema.title){inputEl.label=schema.title}},_errorChanged:function(){if(this.error){this.$.input.errorMessage=this.error;this.$.input.invalid=!0}else{this.$.input.invalid=!1;this.$.input.errorMessage=null}},_isSchemaValue:function(type){return this._isSchemaBoolean(type)||this._isSchemaNumber(type)||this._isSchemaString(type)},_isSchemaBoolean:function(type){if(Array.isArray(type)){return-1!==type.indexOf("boolean")}else{return"boolean"===type}},_isSchemaNumber:function(type){if(Array.isArray(type)){return-1!==type.indexOf("number")||-1!==type.indexOf("integer")}else{return"number"===type||"integer"===type}},_isSchemaString:function(type){if(Array.isArray(type)){return-1!==type.indexOf("string")}else{return"string"===type}},_isSchemaObject:function(type){return"object"===type},_isSchemaArray:function(type){return"array"===type}})});