...
*/(0,_polymerLegacy.Polymer)({is:"eco-json-schema-file",_template:_polymerLegacy.html`
  ...
  `,behaviors:[_appLocalizeBehavior.AppLocalizeBehavior],properties:{language:{value:"en",notify:!0},resources:{value:function(){return{}},notify:!0},schema:{type:Object,observer:"_schemaChanged"},value:{type:Object,notify:!0,value:function(){return{}},observer:"_valueChanged"},/**
    error: {
      type: String,
   ...
   */clear:function(){this.set("files",[]);this.$.fileInput.value="";this._showDropText()},ready:function(){if(this.raised){this.toggleAttribute("raised",!0,this.$.button)}if(this.noink){this.toggleAttribute("noink",!0,this.$.button)}if(this.droppable){this._showDropText();this.setupDrop()}},/**
   * A function to set up a drop area for drag-and-drop file uploads
   */setupDrop:function(){var uploadBorder=this.$.UploadBorder;this.toggleClass("enabled",!0,uploadBorder);this.ondragover=function(e){e.stopPropagation();this.toggleClass("hover",!0,uploadBorder);// Workaround for allowgin drop from Chome's download footer on OSX
...
if(""!==this.accept&&!(-1<this.accept.indexOf(mimeType)||-1<this.accept.indexOf(fileType))){continue}file.progress=0;file.error=!1;file.complete=!1;this.push("files",file);this.uploadFile(file)}}},/**
   ...
   */_fileClick:function(){var elem=this.$.fileInput;if(elem&&document.createEvent){// sanity check
var evt=document.createEvent("MouseEvents");evt.initEvent("click",!0,!1);elem.dispatchEvent(evt)}},/**
   ...
   */_fileChange:function(e){for(var length=e.target.files.length,i=0,file;i<length;i++){file=e.target.files[i];file.progress=0;file.error=!1;file.complete=!1;this.push("files",file);if(!this.multi&&0!==this.files.length){this.set("files",[]);this.set("value",{})}this.uploadFile(file)}},/**
   * Cancels the file upload for a specific file
   *
   * @param {object} file An element of the files array
   */cancel:function(file){if(file){if(file.xhr){file.xhr.abort()}this.splice("files",this.files.indexOf(file),1);this._showDropText()}},/**
   * Cancels the file upload
   *
   * @param {object} e An event object
   */_cancelUpload:function(e){this.cancel(e.model.__data__.item)},/**
   * Retries to upload the file
   *
   * @param {object} e An event object
   */_retryUpload:function(e){e.model.set("item.error",!1);e.model.set("item.progress",0);// The async helps give visual feedback of a retry occurring, even though it's less efficient.
var self=this;async.microTask.run(()=>{self.uploadFile(e.model.__data__.item)})},/**
   * Whether or not to display the drop text
   */_showDropText:function(){this.set("_shownDropText",!this.files.length&&this.droppable)},/**
   * Uploads a file
   *
   * @param {object} file An element of the files array
   */uploadFile:function(file){if(!file){return}this.fire("before-upload");this._showDropText();var prefix="files."+this.files.indexOf(file),self=this,reader=new FileReader;reader.addEventListener("load",function(){var r=reader.result;//self.push('value', {"name": unescape(encodeURIComponent( file.name )),"content":r});
...
},!1);if(!self.value.hasOwnProperty(self.attributes.name.value)){this.set("value."+self.attributes.name.value,{})}reader.readAsDataURL(file)},_valueChanged:function(){console.log("this.value: "+JSON.stringify(this.value))},_schemaChanged:function(){var schema=this.schema;/*
