...
*/(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
  ...
  `,is:"lrnsys-comment-list",listeners:{"comment-save":"handleSave","comment-editing":"handleEditing","comment-reply":"handleReply","comment-like":"handleLike","comment-delete-dialog":"handleDeleteDialog"},properties:{/**
     * CSRF Token
   ...
   * attached life cycle
   */attached:function(e){window.SimpleModal.requestAvailability();async.microTask.run(()=>{if(this.$.filteredcomments.querySelector("iron-list")){this.$.filteredcomments.querySelector("iron-list").fire("iron-resize")}window.dispatchEvent(new Event("resize"))});this.$.filtercomments.addEventListener("value-changed",e=>{this.$.filteredcomments.like=e.target.value});this.$.filtertype.addEventListener("change",e=>{this.$.filtercomments.value="";this.$.filteredcomments.where=e.detail.value;this.$.filteredcomments.like=""})},/**
   * detached life cycle
   */detached:function(e){this.$.filtercomments.removeEventListener("value-changed",e=>{this.$.filteredcomments.like=e.target.value});this.$.filtertype.removeEventListener("change",e=>{this.$.filtercomments.value="";this.$.filteredcomments.where=e.detail.value;this.$.filteredcomments.like=""})},/**
   * Generate the ops URL based on the active comment
   */_computeCommentOpsUrl:function(activeComment,commentOpsBase,csrfToken){if(typeof activeComment!==typeof void 0){return commentOpsBase+"/"+activeComment.id+"?token="+csrfToken}},/**
   * Handle liking a comment.
   */handleLike:function(e){this.activeComment=e.detail.comment;this.$.ajaxlikerequest.generateRequest()},/**
   * @todo not sure we need to do anything post like button
   */_handleLikeResponse:function(e){},/**
   * Handle a delete dialog to confirm.
   */handleDeleteDialog:function(e){this.activeComment=e.detail.comment;// content of dialog
...
let pb2=document.createElement("paper-button");pb2.setAttribute("dialog-confirm","dialog-confirm");pb2.setAttribute("autofocus","autofocus");pb2.addEventListener("click",this._handleDeleteConfirm.bind(this));t=document.createTextNode("Accept");pb2.appendChild(t);b.appendChild(pb2);const evt=new CustomEvent("simple-modal-show",{bubbles:!0,cancelable:!0,detail:{title:"Delete comment",elements:{content:c,buttons:b},invokedBy:e.detail.target,clone:!1}});this.dispatchEvent(evt)},/**
   ...
   */handleEditing:function(e){const evt=new CustomEvent("simple-toast-show",{bubbles:!0,cancelable:!0,detail:{text:"Be awesome to each other",duration:4e3}});this.dispatchEvent(evt)},/**
   ...
   * up above.
   */handleTopReply:function(e){// ensure nothing is set as active for when this goes out the door
this.set("newComment",[]);this.set("activeComment",[]);this.$.ajaxcreatestub.generateRequest()},/**
   * Handle a reply event bubbling up from a comment we've printed
   * via our template in this element. This allows the higher element
   * to create new lower ones which can invoke more lower ones from
   * up above.
   */handleReply:function(e){this.set("newComment",[]);this.activeComment=e.detail.comment;// shift where the response will go
this.$.ajaxcreatestub.generateRequest()},/**
   ...
   */_updateReply:function(e){var comment=this.activeComment,comments=this.comments.data;// normalize response
...
this.set("comments.data",[]);this.set("comments.data",comments);this.notifyPath("comments.data")},/**
   ...
   */_handleDeleteConfirm:function(e){this.$.ajaxdeleterequest.generateRequest()},_handleDeleteResponse:function(e){for(var comment=this.activeComment,comments=this.comments.data,index=0;index<comments.length;index++){if(comments[index].id==comment.id){comments.splice(index,1);// nulify the active comment since it's been removed
...
return!0}}},/**
   ...
   */handleSave:function(e){this.activeComment=e.detail.comment;this.$.ajaxupdaterequest.generateRequest()},_handleUpdateResponse:function(e){const evt=new CustomEvent("simple-toast-show",{bubbles:!0,cancelable:!0,detail:{text:"Comment saved!",duration:4e3}});this.dispatchEvent(evt)},/**
