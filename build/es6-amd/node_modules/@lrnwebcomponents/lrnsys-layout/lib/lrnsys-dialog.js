...
*/(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
    ...
    <paper-tooltip for="dialogtrigger" animation-delay="0" hidden$="[[!alt]]"
      >[[alt]]</paper-tooltip
    >
  `,is:"lrnsys-dialog",listeners:{mousedown:"tapEventOn",mouseover:"tapEventOn",mouseout:"tapEventOff"},properties:{/**
   ...
   */tapEventOn:function(e){const root=this;if(typeof root.hoverClass!==typeof void 0){var classes=root.hoverClass.split(" ");classes.forEach(function(item,index){if(""!=item){root.$.dialogtrigger.classList.add(item)}})}},/**
   * Handle a mouse out event and remove the hoverclasses
   * from the classList array for paper-button.
   */tapEventOff:function(e){const root=this;if(typeof root.hoverClass!==typeof void 0){var classes=root.hoverClass.split(" ");classes.forEach(function(item,index){if(""!=item){root.$.dialogtrigger.classList.remove(item)}})}},/**
   * Toggle the drawer to open / close.
   */openDialog:function(){// assemble everything in the slot
let nodes=(0,_polymerDom.dom)(this).getEffectiveChildNodes(),h=document.createElement("span"),c=document.createElement("span"),node={};for(var i in nodes){if(typeof nodes[i].tagName!==typeof void 0){switch(nodes[i].getAttribute("slot")){case"toolbar-primary":case"toolbar-secondary":case"toolbar":case"header":node=nodes[i].cloneNode(!0);h.appendChild(node);break;case"button":// do nothing
break;default:node=nodes[i].cloneNode(!0);if(this.dynamicImages&&"IRON-IMAGE"===node.tagName){node.preventLoad=!1;node.removeAttribute("prevent-load")}c.appendChild(node);break;}}}const evt=new CustomEvent("simple-modal-show",{bubbles:!0,cancelable:!0,detail:{title:this.header,elements:{header:h,content:c},invokedBy:this.$.dialogtrigger}});this.dispatchEvent(evt)},/**
   ...
   */focusToggle:function(e){const root=this;root.fire("focus-changed",{focus:root.focusState});// see if it has hover classes
if(typeof root.hoverClass!==typeof void 0){// break class into array
...
classes.forEach(function(item,index){if(""!=item){if(root.focusState){root.$.dialogtrigger.classList.add(item)}else{root.$.dialogtrigger.classList.remove(item)}}})}root.focusState=!root.focusState},/**
   * Ready lifecycle
   */ready:function(){this.__modal=window.SimpleModal.requestAvailability()},/**
   * Attached lifecycle
   */attached:function(){this.$.dialogtrigger.addEventListener("focused-changed",this.focusToggle.bind(this))},/**
   * detached lifecycle
   */detached:function(){this.$.dialogtrigger.removeEventListener("focused-changed",this.focusToggle.bind(this))}})});