...
static get properties(){return{target:{name:"target",type:"Object"},icon:{name:"icon",type:"String",value:"icons:expand-less"},label:{name:"label",type:"String",value:"Scroll to top"},position:{name:"position",type:"String",value:"top"}}}/**
   ...
   */connectedCallback(){super.connectedCallback();this.$.btn.addEventListener("click",e=>{if(this.target){this.target.scrollIntoView({behavior:"smooth",block:"end",inline:"nearest"})}else{window.scrollTo({top:0,left:0,behavior:"smooth"})}});this.HAXWiring=new _HAXWiring.HAXWiring;this.HAXWiring.setup(ScrollButton.haxProperties,ScrollButton.tag,this)}/**
   * life cycle, element is removed from the DOM
   */disconnectedCallback(){this.$.btn.removeEventListener("click",e=>{if(this.target){this.target.scrollIntoView({behavior:"smooth",block:"end",inline:"nearest"})}else{window.scrollTo({top:0,left:0,behavior:"smooth"})}});super.disconnectedCallback()}}_exports.ScrollButton=ScrollButton;window.customElements.define(ScrollButton.tag,ScrollButton)});