define(["../../../@polymer/polymer/polymer-legacy.js","../../../@polymer/polymer/lib/legacy/polymer.dom.js","./map-styles.js"],function(_polymerLegacy,_polymerDom,_mapStyles){"use strict";var $_documentContainer=document.createElement("div");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML="<dom-module id=\"map-area\">\n<style include=\"map-styles\">\n :host {\n   display: none;\n }\n</style>\n\n</dom-module>";document.head.appendChild($_documentContainer);/* styles scoped to inside a custom element must be in a style module */(0,_polymerLegacy.Polymer)({is:"map-area",extends:"area",properties:{alt:{type:String,reflectToAttribute:!0/* transition */ /* transition */},coords:{type:String,reflectToAttribute:!0},href:{type:String,reflectToAttribute:!0},shape:{type:String,/* to HTML5's default, circle, poly and rect, add marker, line */value:"default",reflectToAttribute:!0},rel:{type:String,reflectToAttribute:!0},type:{type:String,reflectToAttribute:!0},target:{type:String,reflectToAttribute:!0}},attached:function attached(){// if the map has been attached, set this layer up wrt Leaflet map
if(this.parentElement._map){this._attachedToMap()}},_attachedToMap:function _attachedToMap(){// need the map to convert container points to LatLngs
...
this._feature.on("click",function(){if(this.href){window.open(this.href)}},this)}}},detached:function detached(){this._map.removeLayer(this._feature);delete this._feature},_coordsToArray:function _coordsToArray(containerPoints){// returns an array of arrays of coordinate pairs coordsToArray("1,2,3,4") -> [[1,2],[3,4]]
for(var i=1,points=[],coords=containerPoints.split(",");i<coords.length;i+=2){points.push([parseInt(coords[i-1]),parseInt(coords[i])])}return points},_pointsToLatLngs:function _pointsToLatLngs(points){// points should be an array of nested container coordinates [[x1,y1],[x2,y2](,[xN,yN])]
var latLngArray=[];if(this._map){for(var i=0,map=this._map;i<points.length;i++){latLngArray.push(map.containerPointToLatLng(points[i]))}}return latLngArray},_styleToPathOptions:function _styleToPathOptions(style){var options={};if("none"!==style.stroke){options.stroke=!0;options.color=style.stroke;options.opacity=style.strokeOpacity;options.weight=parseInt(style.strokeWidth);options.dashArray=style.strokeDasharray;options.lineCap=style.strokeLinecap;options.lineJoin=style.strokeLinejoin}else{options.stroke=!1}if("none"!==style.fill){options.fill=!0;options.fillColor=style.fill;options.fillOpacity=style.fillOpacity;options.fillRule=style.fillRule}else{options.fill=!1}return options}})});