...
function _class(){var _this;babelHelpers.classCallCheck(this,_class);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(_class).call(this));_this.HAXCMSThemeWiring=new HAXCMSThemeWiring(babelHelpers.assertThisInitialized(babelHelpers.assertThisInitialized(_this)));return _this}/**
     ...
     *  </div>`;
     *  }
     */babelHelpers.createClass(_class,[{key:"_editModeChanged",/**
     * notice edit changed, make sure we fake a resize because of that container flyout
     */value:function _editModeChanged(newValue){if(babelHelpers.typeof(newValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){_async.microTask.run(function(){// trick browser into thinking we just reized
...
if(null===this.contentContainer){this.contentContainer=this.shadowRoot.querySelector("#contentcontainer")}this.__disposer=[];(0,_renderStatus.afterNextRender)(this,function(){var _this3=this;// edge case, we just swapped theme faster then content loaded... lol
if(0===(0,_polymerDom.dom)(this).getEffectiveChildNodes().length){var frag=document.createRange().createContextualFragment(_haxcmsSiteStore.store.activeItemContent);(0,_polymerDom.dom)(this).appendChild(frag)}(0,_elementMixin.updateStyles)();// store disposer so we can clean up later
...
for(var i in this.__disposer){this.__disposer[i].dispose()}}/**
     ...
     */},{key:"resetActive",value:function resetActive(){window.history.pushState(null,null,_haxcmsSiteStore.store.location.baseUrl);window.dispatchEvent(new PopStateEvent("popstate"));this.dispatchEvent(new CustomEvent("haxcms-active-item-changed",{bubbles:!0,composed:!0,cancelable:!0,detail:{}}))}}],[{key:"properties",get:function get(){return{/**
         ...
         */editMode:{type:Boolean,reflectToAttribute:!0,notify:!0,observer:"_editModeChanged"},/**
...
_haxcmsSiteStore.store.cmsSiteEditorAvailability(element,injector)}/**
   ...
   */},{key:"disconnect",value:function disconnect(element){document.body.removeEventListener("haxcms-active-item-changed",this._activeItemUpdate.bind(element));document.body.removeEventListener("haxcms-edit-mode-changed",this._globalEditChanged.bind(element));document.body.removeEventListener("haxcms-trigger-update",this._triggerUpdate.bind(element))}/**
