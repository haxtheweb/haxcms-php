            ...
            for (var key in _haxcmsSiteStore.store.setupSlots) {
              switch (key) {
                ...
                case "haxcms-site-editor-ui-topbar-character-button":
                  for (var key2 in _haxcmsSiteStore.store.setupSlots[key]) _haxcmsSiteStore.store.cmsSiteEditor.haxCmsSiteEditorUIElement.appendChild(_haxcmsSiteStore.store.setupSlots[key][key2]);
                  break;
              }
            }
      ...
      value: function applyContext() {
        ...
        if (!this.__appliedContext) {
          ...
          if (!["published", "11ty"].includes(context)) {
            var basePath = new URL("./haxcms-editor-builder.js", meta.url).href + "/../";
            // import and set the tag based on the context
            _haxcmsSiteStore.store.cmsSiteEditorBackend.tag = "haxcms-backend-".concat(context);
