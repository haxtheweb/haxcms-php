define(["exports", "meta", "../../../../mobx/dist/mobx.esm.js", "../../../utils/utils.js", "../../../json-outline-schema/json-outline-schema.js", "../../../replace-tag/lib/PerformanceDetect.js", "../../../course-design/lib/learning-component.js"], function (_exports, meta, _mobxEsm, _utils, _jsonOutlineSchema, _PerformanceDetect, _learningComponent) {
  "use strict";
  ...
  var Store = /*#__PURE__*/function () {
    function Store() {
      ...
      fetch(new URL("../../package.json", meta.url)).then(function (response) {
        return response.json();
      }).then(function (obj) {
        return _this.version = obj.version;
      });
      this.appReady = false;
      this.editMode = false;
      this.manifest = null;
    ...
    babelHelpers.createClass(Store, [{
      ...
      value: function getUniqueSlugName(slug) {
        ...
        while (!ready) {
          ...
          for (var key in this.manifest.items) {
            var _item = this.manifest.items[key];
      ...
      value: function _computeItems(start, end, parent, dynamicMethodology, _routerManifest) {
        ...
        if (_routerManifest) {
          ...
          items.forEach(function (item, i) {
            _this2._spiderChildren(item, data, start, end, parent, false);
          });
          return data;
        }
      }
        ...
        if (manifest && typeof manifest.items !== "undefined") {
          var manifestItems = manifest.items.map(function (i) {
            ...
            if (metadata && metadata.pageType) {
              i.metadata.icon = (0, _learningComponent.iconFromPageType)(metadata.pageType);
            }
      ...
      key: "activeItem",
      get: function get() {
        var item = this.findItem(this.activeId);
      ...
      get: function get() {
        ...
        if (this.manifest && this.activeItem) {
          var tmpItem = this.manifest.items.find(function (d) {
            return _this7.activeItem.parent === d.id;
          });
          // walk back up to the root
          while (tmpItem && tmpItem.parent != null) {
            // take the parent object of this current item
      ...
      value: function spiderChildren(item, data, start, end, parent, parentFound, noDynamicLevel) {
        ...
        if (item.id === parent || parentFound) {
          ...
          if (item.children.length > 0) {
            item.children.forEach(function (child) {
              // recursively call itself
              _this8.spiderChildren(child, data, start, end, parent, parentFound, noDynamicLevel);
            });
          }
        } else {
          if (item.children.length > 0) {
    ...
    function HAXCMSSiteStore() {
      ...
      (0, _mobxEsm.autorun)(function () {
        if (store.location && store.location.route && store.location.route.component) {
          ...
          var id = store.location.route.name;
       ...
       */
      (0, _mobxEsm.autorun)(function () {
        var editMode = (0, _mobxEsm.toJS)(store.editMode);
      ...
      value: function attributeChangedCallback(name, oldVal, newVal) {
        ...
        if (name == "source" && newVal != "") {
          fetch(this[name]).then(function (response) {
            ...
            _this10.store.loadManifest(data);
          }).catch(function (err) {
            console.warn(err);
          });
        }
