...
/**
   ...
   */open:function open(){// Attach dialog to the body to ensure it's on top of all existing overlays
...
this._selectedFilters=Object.assign({},this.selectedFilters)}.bind(this),1)},close:function close(){this.$.dialog.close()},/**
   * Handles if the user taps on a filter
   */_tapSelectFilter:function _tapSelectFilter(e){this.$.selector.select(e.model.filter);this._preselectFilterValues()},/**
   * Separate function for unit testing
   */_preselectFilterValues:function _preselectFilterValues(){// Check all values that are selected
var selectedValueIds=this._selectedFilters[this._selectedFilter.id],isSelected=function isSelected(value){return!!selectedValueIds&&0<=selectedValueIds.indexOf(value.id)};this._selectedFilterValues=this._selectedFilter.values.map(function(value){return Object.assign({},value,{selected:isSelected(value)})})},_tapReset:function _tapReset(e){this._selectedFilters={}},_tapApply:function _tapApply(e){this.selectedFilters=this._selectedFilters;this.fire("save")},_tapSelectValues:function _tapSelectValues(e){// Captured IDs of the selected items
var selectedValues=this._selectedFilterValues.filter(function(value){return value.selected}).map(function(value){return value.id});this._selectedFilters=Object.assign({},this._selectedFilters,babelHelpers.defineProperty({},this._selectedFilter.id,selectedValues));this.$.selector.deselect(this._selectedFilter)},/**
   ...
   */_hasSelectedFilters:function _hasSelectedFilters(selectedFilters){// Iterate until we find a filter that is selected
...
_getSelectedValuesNames:function _getSelectedValuesNames(filter,_selectedFilters){var selectedValueIds=_selectedFilters[filter.id];if(!filter.values||!selectedValueIds){return this.noValuesLabel}// Capture names of all selected values
