define(["../../../@polymer/polymer/polymer-legacy.js","./paper-search-bar.js","./paper-filter-dialog.js"],function(_polymerLegacy,_paperSearchBar,_paperFilterDialog){"use strict";(0,_polymerLegacy.Polymer)({is:"paper-search-panel",/**
   ...
   */properties:{/**
     ...
     */search:{type:String,observer:"_onChangeRequest",notify:!0/* transition */ /* transition */},/**
     ...
     */noValuesLabel:String,_hasItems:{type:Boolean,computed:"_computeHasItems(items)",value:!1}},getPaperSearchBarInstance:function getPaperSearchBarInstance(){return this.$.paperSearchBar},// Private methods
_loadMore:function _loadMore(){this.count+=20;this._updateData()},_computeHasItems:function _computeHasItems(items){return"undefined"!==typeof items&&0<items.length},_showNoResults:function _showNoResults(_hasItems,loading){return!_hasItems&&!loading},_onChangeRequest:function _onChangeRequest(newValue,oldValue){// Ignore initial setting of properties (caller is supposed to trigger this call automatically)
if("undefined"!==typeof oldValue){// Set back to default to avoid endless listers
this.count=20;this._updateData()}},_updateData:function _updateData(){this.fire("change-request-params")},_onFilter:function _onFilter(){this.$.filterDialog.open()},_onSearch:function _onSearch(){this.fire("search")},// Counts the selected filters
_getNrSelectedFilters:function _getNrSelectedFilters(selectedFilters){if(0>=Object.keys(selectedFilters).length){return 0}var nrSelectedFilters=Object.keys(selectedFilters).map(function(key){// Returns number of selected value for a filter
return selectedFilters[key].length}).reduce(function(sum,value){// Sum up the selected values across filters
return sum+value});return nrSelectedFilters},_disableFilterButton:function _disableFilterButton(filters){return!(filters&&0<filters.length)}})});