...
*/(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_6c0399d062ec11e9ba56c1f762879bb4()),is:"editable-table-display",listeners:{"change-sort-mode":"_changeSortMode","toggle-filter":"toggleFilter","dropdown-select-changed":"_onColumnChange"},behaviors:[ResponsiveUtilityBehaviors,editableTableBehaviors.displayBehaviors],properties:{/**
   ...
   */_getTbody:function _getTbody(data,columnHeader,footer){if(data!==void 0&&null!==data&&0<data.length){var ch=columnHeader?1:0,tbody;if(footer){tbody=data.slice(ch,data.length-1);this.__tfoot=data.slice(data.length-1)}else{tbody=data.slice(ch,data.length);this.__tfoot=[]}return tbody}return[]},/**
   * Get the columns in <thead>
   */_getThead:function _getThead(data,columnHeader){var root=this;if(data!==void 0&&null!==data&&0<data.length&&columnHeader){return data.slice(0,1)}return[]},/**
   * sets a column's cells to filtered when in filtered mode so that filter can toggle
   */_isFiltered:function _isFiltered(column,filterColumn,filtered){return null!==filterColumn&&filterColumn===column&&filtered},/**
   * sets a cell's numeric style
   */_isNegative:function _isNegative(cell){return this._isNumeric(cell)&&0===cell.trim().indexOf("-")},/**
   * sets a cell's numeric style
   */_isNumeric:function _isNumeric(cell){return null!==cell&&!isNaN(cell.trim().replace(/\$/g,""))},/**
   * sets a cell's numeric style
   */_isNumericColumn:function _isNumericColumn(col){for(var numeric=!0,i=0;i<this.tbody.length;i++){if(!this._isNumeric(this.tbody[i][col]))numeric=!1}return numeric},/**
   * Calculate if the cell is a th or td
   */_isRowHeader:function _isRowHeader(rowHeader,index){return 0===index&&rowHeader},/**
   * Handle column dropdown-select change
   */_onColumnChange:function _onColumnChange(e){this.selected=e.detail.value;this._updateCols(parseInt(e.detail.value))},/**
   * Handle sort button click
   */_changeSortMode:function _changeSortMode(e){if(this.sortColumn===e.detail.columnNumber&&"asc"===this.sortMode){this.sortMode="desc"}else if(this.sortColumn===e.detail.columnNumber&&"desc"===this.sortMode){this.sortMode="none"}else{this.sortMode="asc";this.sortColumn=e.detail.columnNumber}e.detail.setSortMode(this.sortMode);this.sortData(this.sortMode,e.detail.columnNumber)},/**
   * update the responsive columns menu
   */_updateCols:function _updateCols(selected){this.$.table.removeAttribute("default-xs-display");var cols=this.$.table.querySelectorAll("th,td");this.$.table.setAttribute("transition",!0);setTimeout(function(){for(var i=0;i<cols.length;i++){var col=cols[i],index=col.cellIndex,delay=void 0;if(0===index||index===selected){col.removeAttribute("xs-hidden")}else{col.setAttribute("xs-hidden",!0)}}},200);this.$.table.removeAttribute("transition")},/**
   * Handle filter based on collumn and text of cell that is clicked
   */filterRows:function filterRows(filterColumn,filterText){if(filterText!==void 0&&null!==filterText){return function(tr){return tr[filterColumn].toLowerCase().trim()===filterText.toLowerCase().trim()}}else{return null}},/**
   * initialize the responsive columns menu
   */sortData:function sortData(type,column){if("none"!==type&&!1!==type){for(var temp=this.tbody.slice(),i=0;i<temp.length;i++){temp[i].unshift(temp[i][column])}if("asc"===type){temp.sort()}else{temp.reverse()}for(var _i=0;_i<temp.length;_i++){this.set("tbody."+_i,[]);this.set("tbody."+_i,temp[_i].slice(1))}}else{for(var _temp=this.tbody.slice(),_i2=0;_i2<_temp.length;_i2++){this.set("data."+(_i2+1),[]);this.set("data."+(_i2+1),_temp[_i2].slice())}}},/**
