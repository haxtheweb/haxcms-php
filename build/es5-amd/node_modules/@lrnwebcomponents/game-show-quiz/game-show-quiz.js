...
this.__submitDisabled=found}/**
   ...
   */},{key:"submitAnswer",value:function submitAnswer(e){// reset attemptsData for chartist and rebuild fully throughout
...
(0,_polymerDom.dom)(this.__activeTap).appendChild(icon);// check for if we have any attempts remaining
if(0>=this.remainingAttempts){this.shadowRoot.querySelectorAll("responsive-grid-col paper-button:not([disabled])").forEach(function(item){item.setAttribute("disabled","disabled")});this.remainingAttempts=0;// open score report in a modal now
this.$.dialog.toggle();this.scoreBoardToggle({});var _evt2=new CustomEvent("simple-toast-show",{bubbles:!0,cancelable:!0,detail:{text:"Game over!",duration:5e3}});this.dispatchEvent(_evt2)}}/**
...
var qKey=Math.floor(Math.random()*level.questions.length),questionObject={uuid:this.generateUUID(),type:level.type,title:level.title,points:level.points,displayPoints:level.points,question:Object.assign({},level.questions[qKey])};// remove this record
