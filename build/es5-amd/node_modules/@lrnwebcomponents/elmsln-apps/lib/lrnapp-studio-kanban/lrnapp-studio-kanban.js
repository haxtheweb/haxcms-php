...
// application routing
_routeChanged:function _routeChanged(route,endPoint){if("string"===typeof route.path){if("string"===typeof endPoint){if(route.path.startsWith(endPoint)){return}}window.location=route.path}},/**
   * Support having a toast message because of delete or error elsewhere.
   */_deleteToast:function _deleteToast(deletetoast,old){if(babelHelpers.typeof(deletetoast)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){if("error"==deletetoast){this.$.toast.show("That submission on longer exists!")}else{this.$.toast.show("Submission deleted successfully!")}this.set("queryParams.deletetoast",void 0);this.notifyPath("queryParams.deletetoast")}},/**
   * @todo  remove this once we have a modal for it
   */_makeProjectEditLink:function _makeProjectEditLink(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");window.location.href=this.basePath+"../node/"+parts[1]+"/edit?destination=apps/lrnapp-studio-kanban"},/**
   * @todo  remove this once we have a modal for it
   */_makeAssignmentEditLink:function _makeAssignmentEditLink(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");window.location.href=this.basePath+"../node/"+parts[2]+"/edit?destination=apps/lrnapp-studio-kanban"},/**
   * Handle the push to delete a project, pop up the modal.
   */_deleteProjectDialog:function _deleteProjectDialog(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");// set values so we know what to drop
this.projectToDelete=parts[1];this._deleteTitle="Delete project";this._deleteText="Are you sure you want to delete this project and all related assignments!?";this._deleteType="project";this.$.delete.open()},/**
   ...
   */_deleteAssignmentDialog:function _deleteAssignmentDialog(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,parts=local.id.split("-");this.assignmentToDelete=parts[2];// set values so we know what to drop
this._deleteTitle="Delete assignment";this._deleteText="Are you sure you want to delete this assignment?";this._deleteType="assignment";this.$.delete.open()},/**
   ...
   */displayNewBadge:function displayNewBadge(count){if(0==count){return!0}return!1},/**
   * Handle toggling the status for the submission.
   */statusToggle:function statusToggle(e){// find our xhr for callbacks
...
xhr.generateRequest()}},/**
   ...
   */assignmentFocusIn:function assignmentFocusIn(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;if(null!=this.activeAssignment&&this.activeAssignment!=local.id){this.shadowRoot.querySelector("#"+this.activeAssignment).nextElementSibling.classList.remove("show");this.shadowRoot.querySelector("#"+this.activeAssignment).classList.remove("active")}this.activeAssignment=local.id;this._setToggle(!1);local.nextElementSibling.classList.add("show");local.classList.add("active")},/**
   * Handle response for the whole projects object.
   */_handleProjectResponse:function _handleProjectResponse(event){this.$.loading.hidden=!0;this._setToggle(!0)},/**
   * buildSubmissionPath for generating a new submission or linking to existing.
   */buildSubmissionPath:function buildSubmissionPath(path){return path+"lrnapp-studio-submission"},/**
   * Handle a response from updating an item
   */_handleUpdateResponse:function _handleUpdateResponse(event){if(200==this.backendResponse.status){this.$.toast.text="Updated successfully";this.$.toast.toggle();this.$.projectbackend.generateRequest()}else{// this would imply an error
this.$.loading.hidden=!0}},/**
   * set the toggle state when assignment gets focus
   */_setToggle:function _setToggle(update){if(null!=this.activeAssignment){var parts=this.activeAssignment.split("-"),submission=this.projectResponse.data.projects["project-"+parts[1]].relationships.assignments["assignment-"+parts[2]].meta.relatedSubmissions.complete;// focus in on the submissions / assignment meta
...
if(0==submission.status&&0==submission.submission.length){if(!update){this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled=!0;this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked=!1}this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").title="Submission not started";this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle-text").textContent="Submission not started"}else if(0==submission.status){if(!update){this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled=!1;this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked=!1}this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").title="Submission started";this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle-text").textContent="Submission in progress"}else{if(!update){this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").disabled=!1;this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").checked=!0}this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle").title="Submission ready";this.shadowRoot.querySelector("#"+this.activeAssignment+"-toggle-text").textContent="Submission ready"}}},/**
   ...
   */_handleProjectCreated:function _handleProjectCreated(e){this.$.toast.text="Project added";this.$.toast.toggle();this.$.projectbackend.generateRequest()},/**
   * Event came from the assignment button to indicate it was successful.
   */_handleAssignmentCreated:function _handleAssignmentCreated(e){this.$.toast.text="Assignment added";this.$.toast.toggle();this.$.projectbackend.generateRequest()},/*
