 ...
 */var ItemOverlayOps=(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_ae4362306a8f11e9a3fbc91c56f16a14()),is:"item-overlay-ops",listeners:{focusin:"_inFocus",focusout:"_outFocus"},hostAttributes:{tabindex:"0"},properties:{/**
   ...
   * attached life cycle
   */attached:function attached(){var _this=this;// delay is enough to get the height correct
setTimeout(function(){_this._windowResize()},5);window.addEventListener("resize",this._windowResize.bind(this))},/**
   * detached life cycle
   */detached:function detached(){window.removeEventListener("resize",this._windowResize.bind(this))},/**
   * Fixed height changed, update.
   */fixedHeightChanged:function fixedHeightChanged(newValue,oldValue){if(newValue){if(!this.disableAutoHeight){this.$.container.style.height=this.fixedHeight+"px";this.$.workingarea.style.height=this.fixedHeight-80+"px"}}},/**
   * react to window resizing
   */_windowResize:function _windowResize(e){var rect=this.getBoundingClientRect();this.$.container.style.width=rect.width+"px";if(!this.disableAutoHeight){if(!this.fixedHeight||babelHelpers.typeof(this.fixedHeight)===("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){this.$.container.style.height=rect.height+"px";this.$.workingarea.style.height=rect.height-80+"px"}else{this.$.container.style.height=this.fixedHeight+"px";this.$.workingarea.style.height=this.fixedHeight-80+"px"}}else{this.$.workingarea.style.height=rect.height-80+"px"}},/**
   * Support tapping the buttons in the top
   */_opTap:function _opTap(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;this.activeTitle=local.getAttribute("id");this.activeOp=local.getAttribute("id");this._resetActive();this.__anyOp=!0;local.classList.add("active");// we switch icons for these
...
var op={element:this,operation:this.activeOp};this.fire("item-overlay-op-changed",op)},/**
   * Set element reflected focus so we can get the whole thing
   */_inFocus:function _inFocus(e){if(this.editMode){this.focused=!0}},/**
   * Drop element reflection when all focus offs are fired
   */_outFocus:function _outFocus(e){if(this.editMode){this.focused=!1}},/**
   * fire event because an option was selected.
   */_optionSelected:function _optionSelected(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget,ops={element:this,operation:this.activeOp,option:local.getAttribute("id")};this.fire("item-overlay-option-selected",ops);// don't reset for movement, just confirm / reject actions
if("move"!=this.activeOp){this._resetActive();this.activeOp=null}},/**
   * Reset the active selections
   */_resetActive:function _resetActive(){this.$.add.classList.remove("active");this.$.edit.classList.remove("active");this.$.move.classList.remove("active");this.$.remove.classList.remove("active");this.$.duplicate.classList.remove("active")}});_exports.ItemOverlayOps=ItemOverlayOps});