...
*/(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_7f64ff0062ec11e9ba56c1f762879bb4()),is:"lrnsys-comment-list",listeners:{"comment-save":"handleSave","comment-editing":"handleEditing","comment-reply":"handleReply","comment-like":"handleLike","comment-delete-dialog":"handleDeleteDialog"},properties:{/**
     ...
     */activeComment:{type:Object,notify:!0/* transition */ /* transition */},/**
   ...
   * attached life cycle
   */attached:function attached(e){var _this=this;window.SimpleModal.requestAvailability();async.microTask.run(function(){if(_this.$.filteredcomments.querySelector("iron-list")){_this.$.filteredcomments.querySelector("iron-list").fire("iron-resize")}window.dispatchEvent(new Event("resize"))});this.$.filtercomments.addEventListener("value-changed",function(e){_this.$.filteredcomments.like=e.target.value});this.$.filtertype.addEventListener("change",function(e){_this.$.filtercomments.value="";_this.$.filteredcomments.where=e.detail.value;_this.$.filteredcomments.like=""})},/**
   * detached life cycle
   */detached:function detached(e){var _this2=this;this.$.filtercomments.removeEventListener("value-changed",function(e){_this2.$.filteredcomments.like=e.target.value});this.$.filtertype.removeEventListener("change",function(e){_this2.$.filtercomments.value="";_this2.$.filteredcomments.where=e.detail.value;_this2.$.filteredcomments.like=""})},/**
   * Generate the ops URL based on the active comment
   */_computeCommentOpsUrl:function _computeCommentOpsUrl(activeComment,commentOpsBase,csrfToken){if(babelHelpers.typeof(activeComment)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){return commentOpsBase+"/"+activeComment.id+"?token="+csrfToken}},/**
   * Handle liking a comment.
   */handleLike:function handleLike(e){this.activeComment=e.detail.comment;this.$.ajaxlikerequest.generateRequest()},/**
   ...
   */handleDeleteDialog:function handleDeleteDialog(e){this.activeComment=e.detail.comment;// content of dialog
...
var pb2=document.createElement("paper-button");pb2.setAttribute("dialog-confirm","dialog-confirm");pb2.setAttribute("autofocus","autofocus");pb2.addEventListener("click",this._handleDeleteConfirm.bind(this));t=document.createTextNode("Accept");pb2.appendChild(t);b.appendChild(pb2);var evt=new CustomEvent("simple-modal-show",{bubbles:!0,cancelable:!0,detail:{title:"Delete comment",elements:{content:c,buttons:b},invokedBy:e.detail.target,clone:!1}});this.dispatchEvent(evt)},/**
   ...
   */handleEditing:function handleEditing(e){var evt=new CustomEvent("simple-toast-show",{bubbles:!0,cancelable:!0,detail:{text:"Be awesome to each other",duration:4e3}});this.dispatchEvent(evt)},/**
   ...
   * up above.
   */handleTopReply:function handleTopReply(e){// ensure nothing is set as active for when this goes out the door
this.set("newComment",[]);this.set("activeComment",[]);this.$.ajaxcreatestub.generateRequest()},/**
   * Handle a reply event bubbling up from a comment we've printed
   * via our template in this element. This allows the higher element
   * to create new lower ones which can invoke more lower ones from
   * up above.
   */handleReply:function handleReply(e){this.set("newComment",[]);this.activeComment=e.detail.comment;// shift where the response will go
this.$.ajaxcreatestub.generateRequest()},/**
   ...
   */_updateReply:function _updateReply(e){var comment=this.activeComment,comments=this.comments.data;// normalize response
...
return!0}}},/**
   ...
   */handleSave:function handleSave(e){this.activeComment=e.detail.comment;this.$.ajaxupdaterequest.generateRequest()},_handleUpdateResponse:function _handleUpdateResponse(e){var evt=new CustomEvent("simple-toast-show",{bubbles:!0,cancelable:!0,detail:{text:"Comment saved!",duration:4e3}});this.dispatchEvent(evt)},/**
