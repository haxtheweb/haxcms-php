...
*/var GridPlate=(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_de66fa40620511e99fc533744445c188()),is:"grid-plate",listeners:{focusin:"_focusIn",focusout:"_focusOut"},behaviors:[HAXBehaviors.PropertiesBehaviors],properties:{/**
     ...
     * For example:```
{
   ...
   */cancelActive:function cancelActive(e){this.__activeItem=null},/**
   ...
   */canMoveSlot:function canMoveSlot(item,before){var dir=before?-1:1,max=this.shadowRoot.querySelectorAll(".column").length,col=item.getAttribute("slot").split("-"),dest=parseInt(col[1])+dir;return 1<=dest&&dest<=max},/**
   * Moves an item a set number of slots.
   *
   * @param {object} the item
   * @param {number} -1 for left or +1 for right
   */moveSlot:function moveSlot(item,before){var dir=before?-1:1,col=item.getAttribute("slot").split("-"),dest=parseInt(col[1])+dir;if(this.canMoveSlot(item,dir)){item.setAttribute("slot","col-"+dest)}},/**
   * Determines if the item can move a set number of slots.
   *
   * @param {object} the item
   * @param {boolean} move item before previous? (false for move item after next)
   * @returns {boolean} if the item can move a set number of slots
   */canMoveOrder:function canMoveOrder(item,before){var target=before?item.previousElementSibling:item.nextElementSibling;return null!==target&&target.getAttribute("slot")===item.getAttribute("slot")},/**
   * Moves an item's order within a slot.
   *
   * @param {object} the item
   * @param {boolean} move item before previous? (false for move item after next)
   */moveOrder:function moveOrder(item){var before=1<arguments.length&&arguments[1]!==void 0?arguments[1]:!0,dir=before?-1:1;if(this.canMoveOrder(item,before)){if(before){(0,_polymerDom.dom)(this).insertBefore(this.__activeItem,this.__activeItem.previousElementSibling)}else{(0,_polymerDom.dom)(this).insertBefore(this.__activeItem.nextElementSibling,this.__activeItem)}}},/**
   * Move the active element based on which button got pressed.
   */moveActiveElement:function moveActiveElement(e){var _this=this,normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// see if this was an up down left or right movement
...
setTimeout(function(){if(_this.__activeItem&&"function"===typeof _this.__activeItem.focus){_this.positionArrows(_this.__activeItem);_this.__activeItem.focus()}},100)},/**
   * Notice changes to what's active and ensure UX associated w/ it is visble
   */_activeItemChanged:function _activeItemChanged(newValue,oldValue){if(babelHelpers.typeof(newValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!=newValue){// position arrows
...
if(babelHelpers.typeof(oldValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!=oldValue){oldValue.classList.remove("active-item");oldValue.blur()}},/**
   ...
   */setActiveElement:function setActiveElement(e){this.$.right.focus();e.preventDefault();e.stopPropagation()},/**
   ...
   */_getColumnWidths:function _getColumnWidths(){var responsiveSize=0<arguments.length&&arguments[0]!==void 0?arguments[0]:"sm",layout=1<arguments.length&&arguments[1]!==void 0?arguments[1]:"1-1",layouts=2<arguments.length?arguments[2]:void 0,disableResponsive=3<arguments.length?arguments[3]:void 0,newl=layouts[layout],//how old layout names map to the new ones
...
return layouts["1-1"][size]}},/**
   ...
   */_getColumnWidth:function _getColumnWidth(column,columnWidths){return columnWidths!==void 0&&columnWidths[column]!==void 0?"width:"+columnWidths[column]:"min-height: unset"},/**
   * gets a given column's current width based on layout and current responsive width
   *
   * @param {string} the name of selected layout
   * @returns {number} the number of columns in this layout
   */_getColumns:function _getColumns(columnWidths){return columnWidths.length},/**
   * Focus / tab / click event normalization
   */_focusIn:function _focusIn(e){if(this.editMode){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// only activate if we touch something that's in the slot of the grid plate
if((0,_polymerDom.dom)(local).parentNode===this){this.__activeItem=local}}},/**
   * Focus / tab / click event normalization
   */_focusOut:function _focusOut(e){if(this.editMode){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// @todo need to correctly de-focus when the element loses focus entirely
if(local.parentNode===this||document.activeElement.parentNode===this||document.activeElement===this){}else{//this.__activeItem = null;
   ...
   */positionArrows:function positionArrows(item){if(null==item){this.$.up.classList.remove("active");this.$.down.classList.remove("active");this.$.left.classList.remove("active");this.$.right.classList.remove("active")}else{this.$.up.classList.add("active");this.$.down.classList.add("active");this.$.left.classList.add("active");this.$.right.classList.add("active");// ensure we disable invalid options contextually
// test for an element above us
this.$.up.disabled=!this.canMoveOrder(item,!0);// test for an element below us
this.$.down.disabled=!this.canMoveOrder(item,!1);// test for a column to the left of us
this.$.left.disabled=!this.canMoveSlot(item,!0);// test for a column to the right of us
this.$.right.disabled=!this.canMoveSlot(item,!1);// get coordinates of the page and active element
...
this.$.up.style.top=topOffset-20+"px";this.$.down.style.top=topOffset+elemRect.height+"px";this.$.left.style.top=topOffset+elemRect.height/2+"px";this.$.right.style.top=topOffset+elemRect.height/2+"px";this.$.up.style.left=leftOffset+elemRect.width/2-10+"px";this.$.down.style.left=leftOffset+elemRect.width/2-10+"px";this.$.left.style.left=leftOffset-20+"px";this.$.right.style.left=leftOffset+elemRect.width+"px"}},/**
   * Notice edit state has changed
   */_editModeChanged:function _editModeChanged(newValue,oldValue){var _this2=this,children=(0,_polymerDom.dom)(this).getEffectiveChildNodes();if("object"===babelHelpers.typeof(children)){if(newValue&&!oldValue){// walk the children and apply the draggable state needed
...
for(var i in children){if(babelHelpers.typeof(children[i].tagName)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].removeEventListener("drop",this.dropEvent.bind(this));children[i].removeEventListener("dragstart",this.dragStart.bind(this));children[i].removeEventListener("dragend",this.dragEnd.bind(this));children[i].removeEventListener("dragover",function(e){e.preventDefault()});children[i].removeAttribute("draggable");children[i].removeAttribute("data-draggable");children[i].removeAttribute("tabindex")}}async.microTask.run(function(){for(var j=1;j<=_this2.columns.length;j++){if(_this2.shadowRoot.querySelector("#col"+j)!==void 0){_this2.shadowRoot.querySelector("#col"+j).removeEventListener("drop",_this2.dropEvent.bind(_this2));_this2.shadowRoot.querySelector("#col"+j).removeEventListener("dragstart",_this2.dragStart.bind(_this2));_this2.shadowRoot.querySelector("#col"+j).removeEventListener("dragend",_this2.dragEnd.bind(_this2));_this2.shadowRoot.querySelector("#col"+j).removeEventListener("dragover",function(e){e.preventDefault()});_this2.shadowRoot.querySelector("#col"+j).removeAttribute("data-draggable")}}})}}},/**
   ...
   */dropEvent:function dropEvent(e){var _this3=this,normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// if we have a slot on what we dropped into then we need to mirror that item
...
setTimeout(function(){if(_this3.__activeItem&&"function"===typeof _this3.__activeItem.focus){_this3.positionArrows(_this3.__activeItem);_this3.__activeItem.focus()}},100)},/**
   ...
   */dragStart:function dragStart(e){var children=(0,_polymerDom.dom)(this).children;// walk the children and apply the draggable state needed
for(var i in children){if(babelHelpers.typeof(children[i].classList)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].classList.add("mover")}}for(var j=1;j<=this.columns.length;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.add("mover")}}},/**
   ...
   */dragEnd:function dragEnd(e){var children=(0,_polymerDom.dom)(this).children;// walk the children and apply the draggable state needed
for(var i in children){if(babelHelpers.typeof(children[i].classList)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].classList.remove("mover")}}for(var j=1;j<=this.columns.length;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.remove("mover")}}},/**
   * Attached to the DOM, now fire.
   */attached:function attached(){var root=this;// listen for HAX if it's around
document.body.addEventListener("hax-store-property-updated",root._haxStorePropertyUpdated.bind(root));// listen for HAX insert events if it exists
document.body.addEventListener("hax-insert-content",root.haxInsertContent.bind(root));window.ResponsiveUtility.requestAvailability();window.dispatchEvent(new CustomEvent("responsive-element",{detail:{element:root,attribute:"responsive-size",relativeToParent:!0,sm:root.breakpointSm,md:root.breakpointMd,lg:root.breakpointLg,xl:root.breakpointXl}}));// Establish hax property binding
var options={},layouts=Object.keys(root.layouts),getOptions=function getOptions(){//loop through all the supplied layouts to get the HAX layout options & descriptions
for(var i=0;i<layouts.length;i++){options[layouts[i]]=root.layouts[layouts[i]].columnLayout}};getOptions();var props={canScale:!0,canPosition:!0,canEditSource:!1,settings:{quick:[],configure:[{property:"layout",title:"Column Layout",description:"Style to present these items (may change for small screens)",inputMethod:"select",options:options}],advanced:[{property:"breakpointSm",title:"Small Breakpoint",description:"Anything less than this number (in pixels) will render with the smallest version of this layout",inputMethod:"textfield",validationType:"number"},{property:"breakpointMd",title:"Medium Breakpoint",description:"Anything less than this number (in pixels) will render with the small version of this layout",inputMethod:"textfield",validationType:"number"},{property:"breakpointLg",title:"Large Breakpoint",description:"Anything less than this number (in pixels) will render with the medium version of this layout.",inputMethod:"textfield",validationType:"number"},{property:"breakpointXl",title:"Extra-Large Breakpoint",description:"Anything less than this number (in pixels) will render with the large version of this layout. Anything greater than or equal to this number will display with the maximum number of columns for this layout.",inputMethod:"textfield",validationType:"number"}]},saveOptions:{unsetAttributes:["__active-item","edit-mode"]}};root.setHaxProperties(props)},/**
   * Insert event noticed by HAX
   */haxInsertContent:function haxInsertContent(e){var _this4=this;// see if WE are the thing that's active when insert was fired
...
setTimeout(function(){_this4.editMode=!0;if(_this4.__activeItem&&"function"===typeof _this4.__activeItem.focus){_this4.positionArrows(_this4.__activeItem);_this4.__activeItem.focus()}},100)}},/**
   * Store updated, sync.
