 ...
 */(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_2c1954f0685411e9b25d1735312af746()),is:"lrnsys-progress-circle",listeners:{tap:"tapEventOn",mouseover:"focusOn",mouseout:"focusOff","focused-changed":"focusEvent"},properties:{/**
   ...
   * Ready state
   */ready:function ready(e){// seems odd but if we don't do this we can get issues with
...
this._bubbleProgress={25:!1,50:!1,75:!1}},/**
   ...
   */_testValueComplete:function _testValueComplete(newValue,oldValue){// ensure we were previously available before marking complete
...
this.$.icon.classList.remove("activeIcon")}this.focusState=!this.focusState}},/**
   * Focus event for UX consistency.
   */focusOn:function focusOn(e){// see if it has hover classes
...
this.$.icon.icon=this.icon;this.$.icon.classList.add("activeIcon")}},/**
   * Focus event for UX consistency.
   */focusOff:function focusOff(e){// see if it has hover classes
if(!this.disabled&&"loading"!=this.status){// if complete set it back to what it was
if("complete"==this.status||"finished"==this.status){this.$.icon.icon=this.activeIcon}// drop the class for active step
this.$.icon.classList.remove("activeIcon")}},/**
   * Tapped on the item.
   */tapEventOn:function tapEventOn(e){var target=e.target;this.fire("node-is-active",{target:target})},/**
   * Calculate which icon to present.
   */_getActiveIcon:function _getActiveIcon(icon,iconComplete,status){if(babelHelpers.typeof(icon)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){var tmp=icon;// support for a loading icon while loading
if("loading"==status){tmp=this.loadingIcon;this.$.icon.classList.add("transition")}else if("finished"==status){tmp=this.finishedIcon}else if("complete"==status&&babelHelpers.typeof(iconComplete)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){if(this.playSound&&!this.__chimed){this._playSound()}tmp=iconComplete}else{this.$.icon.classList.remove("transition")}return tmp}return!1},/**
   ...
   */_playSound:function _playSound(){// calculate sound to use and ensure it only ever happens once
...
window.audio=new Audio}window.audio.play()},/**
   ...
   */_loadingStatus:function _loadingStatus(status){if("loading"==status){return!0}return!1},/**
   * Calculate loading based on the status of the item.
   */_finishedStatus:function _finishedStatus(status){if("finished"==status){if(this.playFinishSound&&!this.__finishchimed){this._playSound()}return!0}return!1},/**
   * Notice when state changes to fire up an event for others to respond to.
   */_statusChange:function _statusChange(newValue,oldValue){// verify this isn't loading up
if(babelHelpers.typeof(oldValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&newValue!==oldValue){this.fire("node-status-change",{status:newValue})}},/**
   * Calculate disable based on the status of the item.
   */_disableStatus:function _disableStatus(status){if("disabled"==status){return!0}return!1}})});