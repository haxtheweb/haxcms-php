 ...
 */var HaxBody=(0,_polymerLegacy.Polymer)({is:"hax-body",_template:(0,_polymerLegacy.html)(_templateObject_8a69cfd068f411e996b0af119cd7f5ea()),listeners:{focusin:"_focusIn",mousedown:"_focusIn","hax-context-item-selected":"_haxContextOperation","hax-input-mixer-update":"_haxInputMixerOperation","place-holder-replace":"replacePlaceholder"},properties:{__self:{type:Object,value:void 0},/**
   ...
   */ready:function ready(){this.fire("hax-register-body",this);this.polyfillSafe=window.HaxStore.instance.computePolyfillSafe();// mutation observer that ensures state of hax applied correctly
(0,_renderStatus.afterNextRender)(this,function(){var _this=this;this._observer=new _flattenedNodesObserver.FlattenedNodesObserver(this,function(info){// MAKE SURE WE KNOW WHAT JUST GOT ADDED HERE
(0,_flush.flush)();// if we've got new nodes, we have to react to that
if(0<info.addedNodes.length){info.addedNodes.map(function(node){if(_this._haxElementTest(node)){if(_this._HTMLPrimativeTest(node)){node.contentEditable=_this.editMode}// this does the real targetting
node.setAttribute("data-editable",_this.editMode);var haxRay=node.tagName.replace("-"," ").toLowerCase(),i=window.HaxStore.instance.gizmoList.findIndex(function(j){return j.tag===node.tagName.toLowerCase()});if(-1!==i){haxRay=window.HaxStore.instance.gizmoList[i].title}node.setAttribute("data-hax-ray",haxRay);_this.fire("hax-body-tag-added",{node:node})}})}// if we dropped nodes via the UI (delete event basically)
if(0<info.removedNodes.length){// handle removing items... not sure we need to do anything here
info.removedNodes.map(function(node){if(_this._haxElementTest(node)&&!node.classList.contains("hax-active")){_this.fire("hax-body-tag-removed",{node:node})}})}})})},/**
   * Attached to the DOM; now we can fire event to the store that
   * we exist and are the thing being edited.
   */attached:function attached(){(0,_renderStatus.afterNextRender)(this,function(){// try to normalize paragraph insert on enter
...
if(e.clipboardData||e.originalEvent.clipboardData){text=(e.originalEvent||e).clipboardData.getData("text/plain")}else if(window.clipboardData){text=window.clipboardData.getData("Text")}try{var range=window.HaxStore.getRange(),sel=window.HaxStore.getSelection(),newNode=document.createTextNode(text),newRange=document.createRange();if(range&&sel){range.deleteContents();range.insertNode(newNode);newRange.setStart(newNode,text.length);newRange.collapse(!0);sel.removeAllRanges();sel.addRange(newRange)}}catch(e){console.log(e)}}});document.body.removeEventListener("hax-store-property-updated",this._haxStorePropertyUpdated.bind(this));window.removeEventListener("scroll",this._keepContextVisible.bind(this))},/**
   ...
   */_keepContextVisible:function _keepContextVisible(e){// see if the text context menu is visible
...
_this3.activeNode=p;window.HaxStore.write("activeNode",p,_this3);_this3.activeContainerNode.setAttribute("contenteditable",!0);// focus on it
p.focus();_this3.__focusLogic(p)},210)}else{this.replaceElementWorkflow()}},/**
   ...
   */replaceElementWorkflow:function replaceElementWorkflow(){var element=window.HaxStore.nodeToHaxElement(this.activeNode,null),type="*",skipPropMatch=!1;// special support for place holder which defines exactly
...
var tag=this.activeNode.tagName.toLowerCase(),humanName=tag.replace("-"," ");if(babelHelpers.typeof(window.HaxStore.instance.elementList[tag])!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&!1!==window.HaxStore.instance.elementList[tag].gizmo){humanName=window.HaxStore.instance.elementList[tag].gizmo.title}window.HaxStore.instance.haxAppPicker.presentOptions(haxElements,"__convert","Transform ".concat(humanName," to.."),"gizmo")}else{window.HaxStore.toast("Sorry, this can not be transformed!",5e3)}},/**
   ...
   */_globalPreferencesUpdated:function _globalPreferencesUpdated(newValue,oldValue){if(babelHelpers.typeof(newValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!=newValue){this.haxRayMode=newValue.haxRayMode}},/**
   * Store updated, sync.
   ...
   * Clear area.
   */haxClearBody:function haxClearBody(){var confirm=0<arguments.length&&arguments[0]!==void 0?arguments[0]:!0,status=!0;// only confirm if asked so we can support wipes without it
...
if(status){(0,_haxutils.wipeSlot)(this)}},/**
   ...
   */haxInsert:function haxInsert(tag,content){var _this4=this,properties=2<arguments.length&&arguments[2]!==void 0?arguments[2]:{},waitForLock=3<arguments.length&&arguments[3]!==void 0?arguments[3]:!0,tags=window.HaxStore.instance.validTagList;this.__activeHover=null;// verify this tag is a valid one
...
if("function"===typeof this.__updateLockFocus.scrollIntoViewIfNeeded){this.__updateLockFocus.scrollIntoViewIfNeeded(!0)}else{this.__updateLockFocus.scrollIntoView({behavior:"smooth",inline:"center"})}},/**
   ...
   */haxToContent:function haxToContent(){this.hideContextMenus();var __active=this.activeNode;// null this to drop hax based classes
...
content=(0,_haxutils.encapScript)(content);if(this.globalPreferences.haxDeveloperMode){console.log(content)}return content},/**
   ...
   */haxDuplicateNode:function haxDuplicateNode(node){var _this5=this,parent=1<arguments.length&&arguments[1]!==void 0?arguments[1]:this;// move the context menu before duplicating!!!!
this.hideContextMenus();var nodeClone=(0,_polymerDom.dom)(node).cloneNode(!0);if("webview"===nodeClone.tagName.toLowerCase()&&window.HaxStore.instance._isSandboxed&&babelHelpers.typeof(nodeClone.guestinstance)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){delete nodeClone.guestinstance}// shouldn't be possible but might as well check
...
if(parent===_this5){window.HaxStore.write("activeContainerNode",nodeClone,_this5)}window.HaxStore.write("activeNode",nodeClone,_this5)},50);return!0},/**
   * Hide all context menus.
   */hideContextMenus:function hideContextMenus(){// primary context menus
...
this.$.textcontextmenu.highlightOps=!1},/**
   ...
   */positionContextMenus:function positionContextMenus(node,container){var tag=node.tagName.toLowerCase();if(window.HaxStore.instance._isSandboxed&&"webview"===tag){tag="iframe"}var props=window.HaxStore.instance.elementList[tag];// try and work against anything NOT a P tag
...
if(container&&!this._HTMLPrimativeTest(node)&&node!==container){container.contentEditable=!1}else if(container&&this._HTMLPrimativeTest(container)){container.contentEditable=!0}},/**
   ...
   */haxMoveGridPlate:function haxMoveGridPlate(direction,node,container){var _this6=this;// menu is actually in the element for render purposes
...
}setTimeout(function(){_this6.positionContextMenus(node,container);if("function"===typeof container.scrollIntoViewIfNeeded){container.scrollIntoViewIfNeeded(!0)}else{container.scrollIntoView({behavior:"smooth",inline:"center"})}},50);return!0},/**
   * Convert an element from one tag to another.
   */haxReplaceNode:function haxReplaceNode(node,replacement){var parent=2<arguments.length&&arguments[2]!==void 0?arguments[2]:this;// ensure we're not in the document tree for the replaced node
...
if(null!=node.getAttribute("slot")){replacement.setAttribute("slot",node.getAttribute("slot"))}(0,_polymerDom.dom)(parent).replaceChild(replacement,node)}catch(e){console.log(e)}return replacement},/**
   * Convert an element from one tag to another.
   */haxChangeTagName:function haxChangeTagName(node,tagName,newNode){// ensure we're not in the document tree for the replaced node
...
if(children[0]&&children.tagName){children[0].focus()}else{replacement.focus()}},50);return replacement},/**
   ...
   */haxDeleteNode:function haxDeleteNode(node){var parent=1<arguments.length&&arguments[1]!==void 0?arguments[1]:this;// move the context menu before deleting!!!!
...
try{return(0,_polymerDom.dom)(parent).removeChild(node)}catch(e){console.log(e)}},/**
   ...
   * internal whitelist.
   */importContent:function importContent(html){var _this7=this,clear=1<arguments.length&&arguments[1]!==void 0?arguments[1]:!0;// kill the slot of the active body, all of it
...
fragment.removeChild(fragment.firstChild)}}},50)},/**
   ...
   */_haxContextOperation:function _haxContextOperation(e){var _this8=this,detail=e.detail,haxElement;// support a simple insert event to bubble up or everything else
...
case"hax-align-left":this.activeNode.style.float=null;this.activeNode.style.margin=null;this.activeNode.style.display=null;setTimeout(function(){_this8.positionContextMenus(_this8.activeNode,_this8.activeContainerNode)},200);break;case"hax-align-center":this.activeNode.style.float=null;this.activeNode.style.margin="0 auto";this.activeNode.style.display="block";setTimeout(function(){_this8.positionContextMenus(_this8.activeNode,_this8.activeContainerNode)},200);break;case"hax-size-change":if(this.activeNode){this.activeNode.style.width=detail.value+"%";setTimeout(function(){_this8.positionContextMenus(_this8.activeNode,_this8.activeContainerNode)},200)}break;// settings button selected from hax-ce-context bar
...
setTimeout(function(){window.HaxStore.instance.haxManager.$.preview.$.configurebutton.focus()},100);break;}},/**
   ...
   */_haxInputMixerOperation:function _haxInputMixerOperation(e){// this is a big deal how simple this part is in the end
...
this._hideContextMenu(this.$.haxinputmixer)},/**
   ...
   */_focusIn:function _focusIn(e){var normalizedEvent=(0,_polymerDom.dom)(e);if(this.__focusLogic(normalizedEvent.localTarget)){e.stopPropagation()}},/**
   ...
   */__focusLogic:function __focusLogic(target){var stopProp=!1;// only worry about these when we are in edit mode
...
if(this.activeNode!==activeNode&&tags.includes(containerNode.tagName.toLowerCase())&&!activeNode.classList.contains("ignore-activation")){this.activeNode=activeNode;window.HaxStore.write("activeNode",activeNode,this);this.positionContextMenus(activeNode,containerNode);stopProp=!0}}}else{this.__tabTrap=!1}return stopProp},/**
   ...
   */_editModeChanged:function _editModeChanged(newValue,oldValue){// fire above that we have changed states so things can react if needed
...
if(!1===newValue){this.removeAttribute("contenteditable");this.hideContextMenus()}},/**
   ...
   */_haxResolvePreviousElement:function _haxResolvePreviousElement(node){node=(0,_polymerDom.dom)(node).previousElementSibling;while(null!=node&&babelHelpers.typeof(node.tagName)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&"HAX-"===node.tagName.substring(0,4)){node=(0,_polymerDom.dom)(node).previousElementSibling}return node},/**
   * Test if this is a HAX element or not
   */_haxElementTest:function _haxElementTest(node){if(babelHelpers.typeof(node.tagName)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&"HAX-"!==node.tagName.substring(0,4)){return!0}return!1},/**
   * Test if this is an HTML primative
   */_HTMLPrimativeTest:function _HTMLPrimativeTest(node){if(null!=node&&babelHelpers.typeof(node.tagName)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&-1==node.tagName.indexOf("-")){return!0}return!1},/**
   * Walk everything we find and either enable or disable editable state.
   */_applyContentEditable:function _applyContentEditable(status){var target=1<arguments.length&&arguments[1]!==void 0?arguments[1]:this.$.body,children=(0,_polymerDom.dom)(target).getDistributedNodes();// fallback for content nodes if not polymer managed nodes above
...
if("grid-plate"===tag){newValue.editMode=this.editMode;this._applyContentEditable(this.editMode,newValue)}}},/**
   ...
   */_activeNodeChanged:function _activeNodeChanged(newValue,oldValue){var _this9=this;// clean up the older one
...
setTimeout(function(){_this9.positionContextMenus(newValue,window.HaxStore.instance.activeContainerNode)},100);if("left"==newValue.style.textAlign){this.$.textcontextmenu.justifyIcon="editor:format-align-left";this.$.textcontextmenu.justifyValue="text-align-left"}else if("left"==newValue.style.float){this.$.cecontextmenu.justifyIcon="editor:format-align-left";this.$.cecontextmenu.justifyValue="hax-align-left"}else if("0 auto"==newValue.style.margin){this.$.cecontextmenu.justifyIcon="editor:format-align-center";this.$.cecontextmenu.justifyValue="hax-align-center"}}// just hide menus if we don't have an active item
   ...
   */_getPosition:function _getPosition(element){var xPosition=element.offsetLeft-element.scrollLeft+element.clientLeft,yPosition=element.offsetTop-element.scrollTop+element.clientTop;return{x:xPosition,y:yPosition}},/**
   * Handle display and position of the context menu
   */_positionContextMenu:function _positionContextMenu(menu,target,xoffset,yoffset){var _this10=this;// make it account for the offset if it's floated over to one side
...
if(this.__activeHover){menu.classList.add("hax-active-hover");menu.style.marginLeft="";this.__typeLock=!1}setTimeout(function(){async.microTask.run(_this10._keepContextVisible())},100)},/**
   * Simple hide / reset of whatever menu it's handed.
   */_hideContextMenu:function _hideContextMenu(menu){menu.classList.remove("hax-context-visible","hax-context-pin-top","hax-context-pin-bottom");menu.style.left="-100px"},/**
   * Find the next thing to tab forward to.
   */_tabKeyPressed:function _tabKeyPressed(){var focus=!1,node=this.activeContainerNode,activeNodeTagName=this.activeContainerNode.tagName;// try selection / tab block since range can cause issues
...
if(null==(0,_polymerDom.dom)(node).nextSibling){focus=!0}else if("function"===(0,_polymerDom.dom)(node).nextSibling.focus){(0,_polymerDom.dom)(node).nextSibling.focus();focus=!0}else{node=(0,_polymerDom.dom)(node).nextSibling}}}}catch(e){console.log(e)}},/**
   ...
   */_tabBackKeyPressed:function _tabBackKeyPressed(){var node=this.activeContainerNode,activeNodeTagName=this.activeContainerNode.tagName;// try selection / tab block since range can cause issues
...
while(null!=node&&!this._haxElementTest(node)){node=(0,_polymerDom.dom)(node).previousSibling}}if(null!=node){setTimeout(function(){node.focus()},50)}}}catch(e){console.log(e)}}});_exports.HaxBody=HaxBody});