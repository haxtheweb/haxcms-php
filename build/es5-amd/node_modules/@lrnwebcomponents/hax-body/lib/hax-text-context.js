...
*/(0,_polymerLegacy.Polymer)({_template:(0,_polymerLegacy.html)(_templateObject_021fbca06a9011e9a3fbc91c56f16a14()),is:"hax-text-context",listeners:{"hax-context-item-selected":"_haxContextOperation"},properties:{_showIndent:{type:Boolean,computed:"_computeShowIndent(selectedValue, polyfillSafe)"},/**
     ...
     */selectedValue:{type:String,value:"p",notify:!0/* transition */ /* transition */},/**
     * Selection tracking
     */selection:{type:Boolean,value:!1},/**
     * Is this safari
     */isSafari:{type:Boolean,notify:!0,computed:"_isSafari()"}},/**
   *
   */_computeShowIndent:function _computeShowIndent(selectedValue,polyfillSafe){if(polyfillSafe&&("ol"===selectedValue||"ul"===selectedValue)){return!0}return!1},/**
   * Ready, figure out polyfill
   */attached:function attached(){this.polyfillSafe=window.HaxStore.instance.computePolyfillSafe()},/**
   * Respond to simple modifications.
   */_haxContextOperation:function _haxContextOperation(e){var _this=this,detail=e.detail,selection=window.HaxStore.getSelection();// support a simple insert event to bubble up or everything else
switch(detail.eventName){case"close-menu":setTimeout(function(){_this.$.formatsize.$.menu.hideMenu()},200);break;case"insert-inline-gizmo":if(window.HaxStore._tmpSelection&&window.HaxStore.instance.editMode){var localRange=!1;try{if("HAX-BODY"===window.HaxStore._tmpRange.startContainer.parentNode.parentNode.tagName||"HAX-BODY"===window.HaxStore._tmpRange.startContainer.parentNode.parentNode.parentNode.tagName){window.HaxStore.write("activePlaceHolder",window.HaxStore._tmpRange,this);localRange=window.HaxStore._tmpRange}}catch(err){}}if(localRange||null!=window.HaxStore.instance.activePlaceHolder){// store placeholder because if this all goes through we'll want
...
this.$.toolbar.$.moremenu.$.menu.hideMenu();break;case"text-subscript":document.execCommand("subscript");// silly hack to account for trigging a selection from
// inside the menu that isn't from a paper-item
this.$.toolbar.$.moremenu.$.menu.hideMenu();break;case"text-superscript":document.execCommand("superscript");// silly hack to account for trigging a selection from
// inside the menu that isn't from a paper-item
this.$.toolbar.$.moremenu.$.menu.hideMenu();break;case"text-remove-format":document.execCommand("removeFormat");break;case"text-strikethrough":document.execCommand("strikeThrough");// silly hack to account for trigging a selection from
// inside the menu that isn't from a paper-item
this.$.toolbar.$.moremenu.$.menu.hideMenu();break;case"text-link":var href="";if(babelHelpers.typeof(selection.focusNode.parentNode.href)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){href=selection.focusNode.parentNode.href}// @todo put in a dialog instead of this
var url=prompt("Enter a URL:",href);if(url){document.execCommand("createLink",!1,url)}break;case"text-unlink":document.execCommand("unlink");break;/**
   ...
   */_isSafari:function _isSafari(){var ua=navigator.userAgent.toLowerCase();// test to find safari to account for it's handling
...
if(-1!=ua.indexOf("safari")){if(-1<ua.indexOf("chrome")){}else{return!0}}return!1}})});