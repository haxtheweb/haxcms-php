      ...
      babelHelpers.createClass(ColumnReorderingMixin, [{
        ...
        value: function _onTrack(e) {
          ...
          if (this._isSwapAllowed(this._draggedColumn, targetColumn) && this._isSwappableByPosition(targetColumn, e.detail.x)) {
            ...
            var columnTreeLevel = this.__getColumnTreeLevel(targetColumn);
            // Get the columns on that level in visual order
            var levelColumnsInOrder = this._getColumnsInOrder(columnTreeLevel);

            // Index of the column being dragged
            var startIndex = levelColumnsInOrder.indexOf(this._draggedColumn);
            // Index of the column being dragged over
            var endIndex = levelColumnsInOrder.indexOf(targetColumn);
        ...
        value: function _getColumnsInOrder(headerLevel) {
          if (!headerLevel && headerLevel !== 0) {
            headerLevel = this._columnTree.length - 1;
          }
          return this._columnTree[headerLevel].filter(function (c) {
            return !c.hidden;
          }).sort(function (b, a) {
        ...
        value: function _updateGhostPosition(eventClientX, eventClientY) {
          ...
          var targetTop = eventClientY - ghostRect.height / 2;
          // Current position
          var _left = parseInt(this._reorderGhost._left || 0);
          var _top = parseInt(this._reorderGhost._top || 0);
        ...
        key: "__getColumnTreeLevel",
        value: function __getColumnTreeLevel(column) {
          for (var level = 0; level < this._columnTree.length; level++) {
