define(["exports", "./lit-html.js", "./directive.js", "./directive-helpers.js"], function (_exports, _litHtml, _directive, _directiveHelpers) {
  ...
  var hydrate = function hydrate(rootValue, container) {
    ...
    console.warn('Importing `hydrate()` from `lit-html/experimental-hydrate.js` is deprecated.' + 'Import from `@lit-labs/ssr-client` instead.');
    ...
    var rootPart = undefined;
    // Used for error messages
    var rootPartMarker = undefined;
    ...
    // close marker
    var currentChildPart = undefined;
    ...
    while ((marker = walker.nextNode()) !== null) {
      ...
      if (markerText.startsWith('lit-part')) {
        if (stack.length === 0 && rootPart !== undefined) {
          throw new Error("There must be only one root part per container. " + "Found a part marker (".concat(marker, ") when we already have a root ") + "part marker (".concat(rootPartMarker, ")"));
        }
    ...
    if (rootPart === undefined) {
      var elementMessage = babelHelpers.instanceof(container, ShadowRoot) ? "{container.host.localName}'s shadow root" : babelHelpers.instanceof(container, DocumentFragment) ? 'DocumentFragment' : container.localName;
      console.error("There should be exactly one root part in a render container, " + "but we didn't find any in ".concat(elementMessage, "."));
    } // This property needs to remain unminified.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    if (state.type === 'template-instance') {
      ...
      while (true) {
        ...
        if (templatePart.type === _directive.PartType.ATTRIBUTE) {
          ...
          var value = (0, _directiveHelpers.isSingleExpression)(instancePart) ? state.result.values[state.instancePartIndex] : state.result.values;
          ...
          instance._$parts.push(_instancePart);
        }
        state.templatePartIndex++;
      }
    } else {
  ...
  var digestForTemplateResult = function digestForTemplateResult(templateResult) {
    ...
    var str = String.fromCharCode.apply(String, babelHelpers.toConsumableArray(new Uint8Array(hashes.buffer)));
